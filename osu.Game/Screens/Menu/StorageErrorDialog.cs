        ...
        public StorageErrorDialog(OsuStorage storage, OsuStorageError error)
        {
            ...
            switch (error)
            {
                case OsuStorageError.NotAccessible:
                    BodyText = StorageErrorDialogStrings.LocationIsNotAccessible(
                        ...
                        new PopupDialogButton[]
                        {
                            new PopupDialogCancelButton
                            {
                                ...
                                Action = () =>
                                {
                                    ...
                                    using (
                                        realmAccess.BlockAllOperations(
                                            @"restoration of previously unavailable storage"
                                        )
                                    )
                                    ...
                                    if (!success)
                                        dialogOverlay.Push(
                                            new StorageErrorDialog(storage, nextError)
                                        );
                                },
                            },
                            new PopupDialogCancelButton
