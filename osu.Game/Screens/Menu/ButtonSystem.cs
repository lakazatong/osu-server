            ...
            IKeyBindingHandler<GlobalAction>
    {
        ...
        public void SetOsuLogo(OsuLogo? logo)
        {
            ...
            if (this.logo != null)
            {
                ...
                buttonArea.Flow.Position = new Vector2(
                    WEDGE_WIDTH * 2 - (BUTTON_WIDTH + this.logo.SizeForFlow / 4),
                    0
                );
        ...
        public ButtonSystem()
        {
            ...
            buttonArea.AddRange(
                ...
                {
                    new MainMenuButton(
                        ButtonSystemStrings.Settings,
                        string.Empty,
                        OsuIcon.Settings,
                        new Color4(85, 85, 85, 255),
                        (_, _) => OnSettings?.Invoke(),
                        Key.O,
                        Key.S
                    )
                    {
                    ...
                    backButton = new MainMenuButton(
                        ButtonSystemStrings.Back,
                        @"back-to-top",
                        OsuIcon.PrevCircle,
                        new Color4(51, 58, 94, 255),
                        (_, _) => State = ButtonSystemState.TopLevel
                    )
                    {
                        Padding = new MarginPadding { Right = WEDGE_WIDTH },
        ...
        private void load(AudioManager audio, IdleTracker? idleTracker, GameHost host)
        {
            ...
            buttonsPlay.Add(
                new MainMenuButton(
                    ButtonSystemStrings.Multi,
                    @"button-default-select",
                    OsuIcon.Online,
                    new Color4(94, 63, 186, 255),
                    onMultiplayer,
                    Key.M
                )
            );
            ...
            buttonsEdit.Add(
                new MainMenuButton(
                    EditorStrings.BeatmapEditor.ToLower(),
                    @"button-default-select",
                    OsuIcon.Beatmap,
                    new Color4(238, 170, 0, 255),
                    (_, _) => OnEditBeatmap?.Invoke(),
                    Key.B,
                    Key.E
                )
                {
            ...
            buttonsEdit.Add(
                new MainMenuButton(
                    SkinEditorStrings.SkinEditor.ToLower(),
                    @"button-default-select",
                    OsuIcon.SkinB,
                    new Color4(220, 160, 0, 255),
                    (_, _) => OnEditSkin?.Invoke(),
                    Key.S
                )
            );
            ...
            buttonsTopLevel.Add(
                new MainMenuButton(
                    ButtonSystemStrings.Edit,
                    @"button-play-select",
                    OsuIcon.EditCircle,
                    new Color4(238, 170, 0, 255),
                    (_, _) => State = ButtonSystemState.Edit,
                    Key.E
                )
            );
            buttonsTopLevel.Add(
                new MainMenuButton(
                    ButtonSystemStrings.Browse,
                    @"button-default-select",
                    OsuIcon.Beatmap,
                    new Color4(165, 204, 0, 255),
                    (_, _) => OnBeatmapListing?.Invoke(),
                    Key.B,
                    Key.D
                )
            );
            ...
            if (host.CanExit)
                buttonsTopLevel.Add(
                    new MainMenuButton(
                        ButtonSystemStrings.Exit,
                        string.Empty,
                        OsuIcon.CrossCircle,
                        new Color4(238, 51, 153, 255),
                        (_, e) => OnExit?.Invoke(e),
                        Key.Q
                    )
                );
        ...
        private void updateIdleState(bool isIdle)
        {
            ...
            if (
                ...
                && State != ButtonSystemState.EnteringMode
            )
        ...
        public ButtonSystemState State
        {
            ...
            set
            {
                ...
                using (
                    buttonArea.BeginDelayedSequence(
                        lastState == ButtonSystemState.Initial ? 150 : 0
                    )
                )
                {
        ...
        private void updateLogoState(ButtonSystemState lastState = ButtonSystemState.Initial)
        {
            ...
            switch (state)
            {
                ...
                case ButtonSystemState.Play:
                    switch (lastState)
                    {
                        ...
                        case ButtonSystemState.Initial:
                            ...
                            logoTracking?.Dispose();
                            logoTracking = logoTrackingContainer.StartTracking(
                                logo,
                                200,
                                Easing.In
                            );
                ...
                case ButtonSystemState.EnteringMode:
                    ...
                    logoTracking = logoTrackingContainer.StartTracking(
                        logo,
                        lastState == ButtonSystemState.Initial ? MainMenu.FADE_OUT_DURATION : 0,
                        Easing.InSine
                    );
                    break;
            }
