        ...
        )
        {
            ...
            AddStep(
                ...
                () =>
                {
                    ...
                    using (
                        var writer = new StreamWriter(beatmapStream, Encoding.UTF8, leaveOpen: true)
                    )
            ...
            if (ExportLocation != null)
            {
                AddStep(
                    ...
                    () =>
                    {
                        using var stream = File.Open(
                            Path.Combine(ExportLocation, $"{beatmapName}.osu"),
                            FileMode.Create
                        );
                ...
                AddStep(
                    ...
                    {
                        using var stream = File.Open(
                            Path.Combine(ExportLocation, $@"{replayName}.osr"),
                            FileMode.Create
                        );
            ...
            AddStep(
                ...
                () =>
                {
                    ...
                    {
                        scoreStream.Position = 0;
                        decodedScore = new TestScoreDecoder(Beatmap.Value, Ruleset.Value).Parse(
                            scoreStream
                        );
                    }
                }
            ...
            AddAssert(
                @"classic mod present",
                () => decodedScore.ScoreInfo.Mods.Any(mod => mod is ModClassic)
            );
            ...
            AddAssert(
                @"classic mod present",
                () => currentPlayer.GameplayState.Mods.Any(mod => mod is ModClassic)
            );
