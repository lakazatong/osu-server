...
namespace osu.Game.Beatmaps.Formats
{
        ...
        where T : new()
    {
        ...
        private Color4 convertSettingStringToColor4(
            string[] split,
            bool allowAlpha,
        ...
        protected void HandleColours<TModel>(TModel output, string line, bool allowAlpha)
        {
            ...
            bool isCombo =
                pair.Key.StartsWith(@"Combo", StringComparison.Ordinal)
                && int.TryParse(pair.Key[5..], out int comboIndex)
                && comboIndex >= 1
                && comboIndex <= MAX_COMBO_COLOUR_COUNT;

            if (isCombo)
            {
                if (!(output is IHasComboColours tHasComboColours))
                    return;
            ...
            {
                if (!(output is IHasCustomColours tHasCustomColours))
                    return;
        ...
        protected KeyValuePair<string, string> SplitKeyVal(
            string line,
            char separator = ':',
        ...
        )
        {
            string[] split = line.Split(
                separator,
                2,
                shouldTrim ? StringSplitOptions.TrimEntries : StringSplitOptions.None
            );
        ...
        protected string CleanFilename(string path) =>
            path
        ...
        internal class LegacySampleControlPoint
            : SampleControlPoint,
                IEquatable<LegacySampleControlPoint>
        {
            ...
            public override HitSampleInfo ApplyTo(HitSampleInfo hitSampleInfo)
            {
                ...
                {
                    return legacy.With(
                        newCustomSampleBank: legacy.CustomSampleBank > 0
                            ? legacy.CustomSampleBank
                            : CustomSampleBank,
            ...
            public override int GetHashCode() =>
                HashCode.Combine(base.GetHashCode(), CustomSampleBank);
        }
    }
}
