...
namespace osu.Game.Skinning
{
    public partial class LegacyRankDisplay : CompositeDrawable, ISerialisableDrawable
    {
        ...
        [SettingSource(
            typeof(DefaultRankDisplayStrings),
            nameof(DefaultRankDisplayStrings.PlaySamplesOnRankChange)
        )]
        ...
        public LegacyRankDisplay()
        {
            ...
            AddInternal(
                rankDisplay = new Sprite { Anchor = Anchor.Centre, Origin = Anchor.Centre }
            );
        }
        ...
        private void load(SkinEditor? skinEditor, SessionStatics statics)
        {
            ...
            lastSamplePlaybackTime = statics.GetBindable<double?>(
                Static.LastRankChangeSamplePlaybackTime
            );
        }
            ...
            rank.BindValueChanged(
                r =>
                {
                    ...
                    if (texture != null)
                    {
                        ...
                        transientRank
                            .FadeOutFromOne(500, Easing.Out)
                            .ScaleTo(new Vector2(1.625f), 500, Easing.Out)
                            .Expire();
                    }

                    bool enoughTimeElapsed =
                        !lastSamplePlaybackTime.Value.HasValue
                        || Time.Current - lastSamplePlaybackTime.Value
                            >= OsuGameBase.SAMPLE_DEBOUNCE_TIME;

                    // Don't play rank-down sfx on quit/retry
                    if (
                        r.NewValue != r.OldValue
                        && r.NewValue > ScoreRank.F
                        && PlaySamples.Value
                        && enoughTimeElapsed
                    )
                    {
