...
namespace osu.Game.Utils
{
    ...
    {
        private static readonly MethodInfo get_method = typeof(BindableValueAccessor).GetMethod(
            nameof(getValue),
            BindingFlags.Static | BindingFlags.NonPublic
        )!;
        private static readonly MethodInfo set_method = typeof(BindableValueAccessor).GetMethod(
            nameof(setValue),
            BindingFlags.Static | BindingFlags.NonPublic
        )!;
        ...
        {
            Type? bindableWithValueType = bindable
                .GetType()
                .GetInterfaces()
                .FirstOrDefault(t =>
                    t.IsGenericType && t.GetGenericTypeDefinition() == typeof(IBindable<>)
                );
        ...
        private static object setValue<T>(object bindable, object value) =>
            ((Bindable<T>)bindable).Value = (T)value;
    }
}
