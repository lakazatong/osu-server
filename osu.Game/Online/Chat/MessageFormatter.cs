...
namespace osu.Game.Online.Chat
{
    public static class MessageFormatter
    {
        ...
        private static readonly Regex old_link_regex = new Regex(
            @"\((?<text>(((?<=\\)[\(\)])|[^\(\)])*(((?<open>\()(((?<=\\)[\(\)])|[^\(\)])*)+((?<close-open>\))(((?<=\\)[\(\)])|[^\(\)])*)+)*(?(open)(?!)))\)\[(?<url>[a-z]+://[^ ]+)\]"
        );

        // [https://osu.ppy.sh/b/1234 Beatmap [Hard] (poop)] -> Beatmap [hard] (poop) (https://osu.ppy.sh/b/1234)
        private static readonly Regex new_link_regex = new Regex(
            @"\[(?<url>[a-z]+://[^ ]+) (?<text>(((?<=\\)[\[\]])|[^\[\]])*(((?<open>\[)(((?<=\\)[\[\]])|[^\[\]])*)+((?<close-open>\])(((?<=\\)[\[\]])|[^\[\]])*)+)*(?(open)(?!)))\]"
        );

        // [test](https://osu.ppy.sh/b/1234) -> test (https://osu.ppy.sh/b/1234) aka correct markdown format
        private static readonly Regex markdown_link_regex = new Regex(
        ...
        public static string WebsiteRootUrl
        {
            get => websiteRootUrl;
            set =>
                websiteRootUrl = value
                    .Trim('/') // trim potential trailing slash/
                    .Split('/')
                    .Last(); // only keep domain name, ignoring protocol.
        }
        ...
        private static void handleMatches(
            ...
            string display,
            string link,
            MessageFormatterResult result,
            int startIndex = 0,
            LinkAction? linkActionOverride = null,
        ...
        )
        {
            ...
            foreach (Match m in regex.Matches(result.Text, startIndex))
            {
                ...
                if (escapeChars != null)
                    displayText = escapeChars.Aggregate(
                        displayText,
                        (current, c) => current.Replace($"\\{c}", c.ToString())
                    );
                ...
                if (!result.Links.Exists(l => l.Overlaps(index, m.Length)))
                {
                    ...
                    result.Links.ForEach(l =>
                        l.Index -= l.Index > index ? m.Length - displayText.Length : 0
                    );
                    ...
                    result.Links.Add(
                        new Link(
                            ...
                            index,
                            displayText.Length,
                            linkActionOverride ?? details.Action,
                            details.Argument
                        )
                    );
        ...
        private static void handleAdvanced(
            Regex regex,
            MessageFormatterResult result,
            int startIndex = 0
        )
        ...
        private static MessageFormatterResult format(
            string toFormat,
            int startIndex = 0,
            int space = 3
        )
        {
            ...
            // handle editor times
            handleMatches(
                EditorTimestampParser.TIME_REGEX_STRICT,
                "{0}",
                $@"{OsuGameBase.OSU_PROTOCOL}edit/{{0}}",
                result,
                startIndex,
            ...
            handleMatches(
                channel_regex,
                "{0}",
                $@"{OsuGameBase.OSU_PROTOCOL}chan/{{0}}",
                result,
                startIndex,
    ...
    public class Link : IComparable<Link>
    {
        ...
        public bool Overlaps(int otherIndex, int otherLength) =>
            Index < otherIndex + otherLength && otherIndex < Index + Length;
