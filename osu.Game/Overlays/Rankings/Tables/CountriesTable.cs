    ...
    public partial class CountriesTable : RankingsTable<CountryStatistics>
    {
        ...
        protected override RankingsTableColumn[] CreateAdditionalHeaders() =>
            new[]
            {
                ...
                new RankingsTableColumn(
                    RankingsStrings.StatAverageScore,
                    Anchor.Centre,
                    new Dimension(GridSizeMode.AutoSize)
                ),
                new RankingsTableColumn(
                ...
                new RankingsTableColumn(
                    RankingsStrings.StatAveragePerformance,
                    Anchor.Centre,
                    new Dimension(GridSizeMode.AutoSize)
                ),
            };
        ...
        protected override Drawable[] CreateFlagContent(CountryStatistics item) =>
            [new CountryName(item.Code)];

        protected override Drawable[] CreateAdditionalContent(CountryStatistics item) =>
            new Drawable[]
            {
                new ColouredRowText { Text = item.ActiveUsers.ToLocalisableString(@"N0") },
                new ColouredRowText { Text = item.PlayCount.ToLocalisableString(@"N0") },
                new ColouredRowText { Text = item.RankedScore.ToLocalisableString(@"N0") },
                new ColouredRowText
                {
                    Text = (item.RankedScore / Math.Max(item.ActiveUsers, 1)).ToLocalisableString(
                        @"N0"
                    ),
                },
                new RowText { Text = item.Performance.ToLocalisableString(@"N0") },
                new ColouredRowText
                {
                    Text = (item.Performance / Math.Max(item.ActiveUsers, 1)).ToLocalisableString(
                        @"N0"
                    ),
                },
            };
