...
namespace osu.Game.Overlays.Comments
{
    public partial class CommentReportButton : CompositeDrawable, IHasPopover, IHasLineBaseHeight
    {
        ...
        private void load()
        {
            ...
            {
                link = new LinkFlowContainer(s =>
                    s.Font = OsuFont.GetFont(size: 12, weight: FontWeight.Bold)
                )
                {
                    AutoSizeAxes = Axes.Both,
                },
                loading = new LoadingSpinner { Size = new Vector2(12f) },
            };

            link.AddLink(ReportStrings.CommentButton.ToLower(), this.ShowPopover);
        }

        public Popover GetPopover() => new ReportCommentPopover(comment) { Action = report };
        ...
        private void report(CommentReportReason reason, string comments)
        {
            ...
            request.Success += () =>
                Schedule(() =>
                {
                    ...
                    link.Clear(true);
                    link.AddText(
                        UsersStrings.ReportThanks,
                        s => s.Colour = colourProvider?.Content2 ?? Colour4.White
                    );
                    link.Show();
        ...
        public float LineBaseHeight =>
            link.ChildrenOfType<IHasLineBaseHeight>().FirstOrDefault()?.LineBaseHeight
            ?? DrawHeight;
    }
}
