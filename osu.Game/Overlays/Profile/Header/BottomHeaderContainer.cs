    ...
    public partial class BottomHeaderContainer : CompositeDrawable
    {
        ...
        private void updateDisplay(APIUser? user)
        {
            ...
            else if (user.LastVisit.HasValue)
            {
                ...
                topLinkContainer.AddText(
                    new DrawableDate(user.LastVisit.Value, italic: false),
                    embolden
                );
            ...
            if (user.PlayStyles?.Length > 0)
            {
                ...
                {
                    playStylesString = new TranslatableString(
                        @"_",
                        @"{0}{1}",
                        playStylesString,
                        CommonStrings.ArrayAndWordsConnector
                    );
                    playStylesString = new TranslatableString(
                        @"_",
                        @"{0}{1}",
                        playStylesString,
                        user.PlayStyles[i].GetLocalisableDescription()
                    );
                }
            ...
            topLinkContainer.AddLink(
                "forum post".ToQuantity(user.PostCount, "#,##0"),
                $"{api.Endpoints.WebsiteUrl}/users/{user.Id}/posts",
                creationParameters: embolden
            );
            ...
            topLinkContainer.AddLink(
                "comment".ToQuantity(user.CommentsCount, "#,##0"),
                $"{api.Endpoints.WebsiteUrl}/comments?user_id={user.Id}",
                creationParameters: embolden
            );
        ...
        private void addSpacer(OsuTextFlowContainer textFlow) =>
            textFlow.AddArbitraryDrawable(new Container { Width = 15 });
        ...
        private void embolden(SpriteText text) =>
