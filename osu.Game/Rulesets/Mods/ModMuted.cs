...
namespace osu.Game.Rulesets.Mods
{
    ...
    public abstract class ModMuted<TObject>
        : ModMuted,
        ...
        where TObject : HitObject
    {
        ...
        [SettingSource(
            ...
            SettingControlType = typeof(SettingsSlider<int, MuteComboSlider>)
        )]
        public BindableInt MuteComboCount { get; } =
            new BindableInt(100) { MinValue = 0, MaxValue = 500 };
        ...
        public void ApplyToDrawableRuleset(DrawableRuleset<TObject> drawableRuleset)
        {
            if (EnableMetronome.Value)
            {
                ...
                drawableRuleset.FrameStableComponents.Add(
                    metronomeBeat = new MetronomeBeat(
                        drawableRuleset.Beatmap.HitObjects.First().StartTime
                    )
                );
                ...
                combo =>
                {
                    double dimFactor =
                        MuteComboCount.Value == 0
                            ? 1
                            : (double)combo.NewValue / MuteComboCount.Value;

                    if (InverseMuting.Value)
                        dimFactor = 1 - dimFactor;

                    scoreProcessor.TransformBindableTo(
                        metronomeVolumeAdjust,
                        dimFactor,
                        500,
                        Easing.OutQuint
                    );
    ...
    public partial class MuteComboSlider : RoundedSliderBar<int>
    {
        public override LocalisableString TooltipText =>
            Current.Value == 0 ? "always muted" : base.TooltipText;
    }
}
