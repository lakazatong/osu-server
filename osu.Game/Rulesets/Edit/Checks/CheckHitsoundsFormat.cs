...
namespace osu.Game.Rulesets.Edit.Checks
{
    public class CheckHitsoundsFormat : ICheck
    {
        public CheckMetadata Metadata =>
            new CheckMetadata(CheckCategory.Audio, "Checks for hitsound formats.");
        ...
        public IEnumerable<Issue> Run(BeatmapVerifierContext context)
        {
            ...
            {
                if (audioFile != null && ReferenceEquals(file.File, audioFile.File))
                    continue;
                ...
                using (Stream data = context.WorkingBeatmap.GetStream(file.File.GetStoragePath()))
                {
                    ...
                    if (
                        !AudioCheckUtils.HasAudioExtension(file.Filename)
                        || !probablyHasAudioData(data)
                    )
                        continue;
                    ...
                    if (
                        (audioInfo.ChannelType & ChannelType.Wave) == 0
                        && audioInfo.ChannelType != ChannelType.OGG
                    )
        ...
        public class IssueTemplateFormatUnsupported : IssueTemplate
        {
            public IssueTemplateFormatUnsupported(ICheck check)
                : base(
                    check,
                    IssueType.Problem,
                    "\"{0}\" may be corrupt or using a unsupported audio format. Use wav or ogg for hitsounds."
                ) { }
        ...
        public class IssueTemplateIncorrectFormat : IssueTemplate
        {
            public IssueTemplateIncorrectFormat(ICheck check)
                : base(
                    check,
                    IssueType.Problem,
                    "\"{0}\" is using a incorrect format. Use wav or ogg for hitsounds."
                ) { }
