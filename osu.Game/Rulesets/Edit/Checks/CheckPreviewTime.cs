...
namespace osu.Game.Rulesets.Edit.Checks
{
    public class CheckPreviewTime : ICheck
    {
        public CheckMetadata Metadata =>
            new CheckMetadata(CheckCategory.Timing, "Inconsistent or unset preview time");
        ...
        {
            var diffList =
                context.Beatmap.BeatmapInfo.BeatmapSet?.Beatmaps ?? new List<BeatmapInfo>();
            ...
            foreach (var diff in diffList)
            {
                ...
                if (diff.Metadata.PreviewTime != previewTime)
                    yield return new IssueTemplatePreviewTimeConflict(this).Create(
                        diff.DifficultyName,
                        previewTime,
                        diff.Metadata.PreviewTime
                    );
            }
        }
        ...
        public class IssueTemplatePreviewTimeConflict : IssueTemplate
        {
            public IssueTemplatePreviewTimeConflict(ICheck check)
                : base(
                    check,
                    IssueType.Problem,
                    "Audio preview time ({1}) doesn't match the time specified in \"{0}\" ({2})"
                ) { }
