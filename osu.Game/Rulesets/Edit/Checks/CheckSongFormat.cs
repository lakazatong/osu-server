...
namespace osu.Game.Rulesets.Edit.Checks
{
    public class CheckSongFormat : ICheck
    {
        public CheckMetadata Metadata =>
            new CheckMetadata(CheckCategory.Audio, "Checks for song formats.");
        ...
        private IEnumerable<ChannelType> allowedFormats =>
            new[] { ChannelType.MP3, ChannelType.OGG };
        ...
        public IEnumerable<Issue> Run(BeatmapVerifierContext context)
        {
            ...
            using (Stream data = context.WorkingBeatmap.GetStream(audioFile.File.GetStoragePath()))
            {
                ...
                if (decodeStream == 0)
                {
                    yield return new IssueTemplateFormatUnsupported(this).Create(
                        audioFile.Filename
                    );
        ...
        public class IssueTemplateFormatUnsupported : IssueTemplate
        {
            public IssueTemplateFormatUnsupported(ICheck check)
                : base(
                    check,
                    IssueType.Problem,
                    "\"{0}\" may be corrupt or using a unsupported audio format. Use mp3 or ogg for the song's audio."
                ) { }
        ...
        public class IssueTemplateIncorrectFormat : IssueTemplate
        {
            public IssueTemplateIncorrectFormat(ICheck check)
                : base(
                    check,
                    IssueType.Problem,
                    "\"{0}\" is using a incorrect format. Use mp3 or ogg for the song's audio."
                ) { }
