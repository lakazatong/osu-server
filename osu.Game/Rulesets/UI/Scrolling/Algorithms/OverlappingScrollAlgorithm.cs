    ...
    public class OverlappingScrollAlgorithm : IScrollAlgorithm
    {
        ...
        )
        {
            ...
            double visibleDuration =
                (scrollLength + offset) * timeRange / controlPoint.Multiplier / scrollLength;
        ...
        public float PositionAt(
            ...
            (float)(
                (time - currentTime)
                / timeRange
                * controlPointAt(originTime ?? time).Multiplier
                * scrollLength
            );
        ...
        )
        {
            ...
            var relevantControlPoint =
                controlPoints.LastOrDefault(cp =>
                    PositionAt(cp.Time, currentTime, timeRange, scrollLength) <= position
                ) ?? controlPoints.First();

            float positionAtControlPoint = PositionAt(
                relevantControlPoint.Time,
                currentTime,
                timeRange,
                scrollLength
            );

            return relevantControlPoint.Time
                + (position - positionAtControlPoint)
                    * timeRange
                    / relevantControlPoint.Multiplier
                    / scrollLength;
        }
