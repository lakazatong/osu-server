    ...
    public partial class LinkFlowContainer : OsuTextFlowContainer
    {
        ...
        public void AddLinks(string text, List<Link> links)
        {
            ...
            foreach (var link in links)
            {
                ...
                {
                    Logger.Log(
                        $@"Link ""{link.Url}"" with text ""{displayText}"" overlaps previous link, ignoring."
                    );
                    continue;
                }
        ...
        public void AddLink(
            ...
            createLink(
                CreateChunkFor(text, true, CreateSpriteText, creationParameters),
                new LinkDetails(LinkAction.External, url),
                url
            );
        ...
        public void AddLink(
            ...
            createLink(
                CreateChunkFor(text, true, CreateSpriteText, creationParameters),
                new LinkDetails(LinkAction.Custom, string.Empty),
                tooltipText,
                action
            );
        ...
        public void AddLink(
            ...
            createLink(
                CreateChunkFor(text, true, CreateSpriteText, creationParameters),
                new LinkDetails(action, argument),
                tooltipText
            );

        public void AddLink(
            IEnumerable<SpriteText> text,
            LinkAction action,
            object linkArgument,
            string tooltipText = null
        )
        {
            createLink(
                new TextPartManual(text),
                new LinkDetails(action, linkArgument),
                tooltipText
            );
        }
        ...
        public void AddUserLink(IUser user, Action<SpriteText> creationParameters = null) =>
            createLink(
                CreateChunkFor(user.Username, true, CreateSpriteText, creationParameters),
                new LinkDetails(LinkAction.OpenUserProfile, user),
        ...
        protected virtual DrawableLinkCompiler CreateLinkCompiler(ITextPart textPart) =>
            new DrawableLinkCompiler(textPart);
        ...
        private partial class LinkFlow : InnerFlow
        {
            ...
            public override IEnumerable<Drawable> FlowingChildren =>
                base.FlowingChildren.Where(c => !(c is DrawableLinkCompiler));
        }
    }
}
