    ...
    public partial class BackgroundDataStoreProcessor : Component
    {
        ...
        private void clearOutdatedStarRatings()
        {
            foreach (var ruleset in rulesetStore.AvailableRulesets)
            {
                ...
                int currentVersion = ruleset
                    .CreateInstance()
                    .CreateDifficultyCalculator(gameBeatmap.Value)
                    .Version;

                if (ruleset.LastAppliedDifficultyVersion < currentVersion)
                {
                    Logger.Log(
                        $"Resetting star ratings for {ruleset.Name} (difficulty calculation version updated from {ruleset.LastAppliedDifficultyVersion} to {currentVersion})"
                    );
                    ...
                    realmAccess.Write(r =>
                    {
                        ...
                        r.Find<RulesetInfo>(ruleset.ShortName)!.LastAppliedDifficultyVersion =
                            currentVersion;
                    });
        ...
        private void populateMissingStarRatings()
        {
            ...
            Logger.Log(
                $"Found {beatmapIds.Count} beatmaps which require star rating reprocessing."
            );

            var notification = showProgressNotification(
                beatmapIds.Count,
                "Reprocessing star rating for beatmaps",
                "beatmaps' star ratings have been updated"
            );
        ...
        private void processOnlineBeatmapSetsWithNoUpdate()
        {
            ...
            realmAccess.Run(r =>
            {
                ...
                if (api.IsLoggedIn)
                {
                    foreach (
                        var b in r.All<BeatmapInfo>()
                            .Where(b =>
                                b.OnlineID > 0 && b.LastOnlineUpdate == null && b.BeatmapSet != null
                            )
                    )
            ...
            var notification = showProgressNotification(
                ...
                "beatmaps' online data have been updated"
            );
        ...
        private void processBeatmapsWithMissingObjectCounts()
        {
            ...
            var notification = showProgressNotification(
                ...
                "beatmaps have been populated with missing statistics"
            );
        ...
        private void processScoresWithMissingStatistics()
        {
            ...
            {
                foreach (
                    var score in r.All<ScoreInfo>().Where(s => !s.BackgroundReprocessingFailed)
                )
                {
                    if (
            ...
            var notification = showProgressNotification(
                ...
                "scores have been populated with missing statistics"
            );
            ...
            foreach (var id in scoreIds)
            {
                ...
                try
                {
                    ...
                    if (score != null)
                    {
                        ...
                        realmAccess.Write(r =>
                        {
                            r.Find<ScoreInfo>(id)!.MaximumStatisticsJson =
                                JsonConvert.SerializeObject(score.MaximumStatistics);
                        });
                    }
                ...
                catch (Exception e)
                {
                    ...
                    realmAccess.Write(r =>
                        r.Find<ScoreInfo>(id)!.BackgroundReprocessingFailed = true
                    );
                    ++failedCount;
                }
            }
        ...
        private void convertLegacyTotalScoreToStandardised()
        {
            ...
            var notification = showProgressNotification(
                ...
                "scores have been upgraded to the new scoring algorithm"
            );
                ...
                try
                {
                    ...
                    realmAccess.Write(r =>
                    {
                        ...
                        StandardisedScoreMigrationTools.UpdateFromLegacy(
                            s,
                            beatmapManager.GetWorkingBeatmap(s.BeatmapInfo)
                        );
                        s.TotalScoreVersion = LegacyScoreEncoder.LATEST_VERSION;
                    });
        ...
        private void upgradeScoreRanks()
        {
            ...
            var notification = showProgressNotification(
                ...
                "scores now have more correct ranks"
            );
        ...
        private void backpopulateMissingSubmissionAndRankDates()
        {
            ...
            {
                Logger.Log(
                    "Cannot backpopulate missing submission/rank dates because the local metadata cache is missing."
                );
                return;
            }
            ...
            try
            {
                ...
                {
                    Logger.Log(
                        "Cannot backpopulate missing submission/rank dates because the local metadata cache is too old."
                    );
                    return;
                }
            }
            ...
            HashSet<Guid> beatmapSetIds = realmAccess.Run(r => new HashSet<Guid>(
                r.All<BeatmapSetInfo>()
                    .Where(b =>
                        b.StatusInt > 0 && (b.DateRanked == null || b.DateSubmitted == null)
                    )
                    .AsEnumerable()
                    .Select(b => b.ID)
            ));
            ...
            Logger.Log(
                $"Found {beatmapSetIds.Count} beatmap sets with missing submission/rank date."
            );

            var notification = showProgressNotification(
                beatmapSetIds.Count,
                "Populating missing submission and rank dates",
                "beatmap sets now have correct submission and rank dates."
            );
            ...
            foreach (var id in beatmapSetIds)
            {
                ...
                try
                {
                    ...
                    bool succeeded = realmAccess.Write(r =>
                    {
                        ...
                        if (
                            beatmapSet.Beatmaps.FirstOrDefault(b =>
                                b.Status >= BeatmapOnlineStatus.Ranked
                            )
                            is not BeatmapInfo beatmap
                        )
                            return false;

                        bool lookupSucceeded = localMetadataSource.TryLookup(
                            beatmap,
                            out var result
                        );
                        ...
                        Logger.Log(
                            $"Could not find {beatmapSet.GetDisplayString()} in local cache while backpopulating missing submission/rank date"
                        );
                        return false;
                    });
                ...
                {
                    Logger.Log(
                        $"Failed to update ranked/submitted dates for beatmap set {id}: {e}"
                    );
                    ++failedCount;
                }
            }
        ...
        )
        {
            ...
            notification.Text =
                notification.Text.ToString().Split('(').First().TrimEnd()
        ...
        )
        {
            ...
            {
                notification.Text =
                    $"{processedCount} of {totalCount} {notification.CompletionText}";
        ...
        private ProgressNotification? showProgressNotification(
            int totalCount,
            string running,
            string completed
        )
