    ...
    public partial class TestSceneOpenEditorTimestampInMania : EditorTestScene
    {
        ...
        public void TestNormalSelection()
        {
            ...
            AddAssert(
                "selected group",
                () =>
                    checkSnapAndSelectColumn(
                        5_920,
                        new List<(int, int)> { (5_920, 3), (6_623, 3), (6_857, 2), (7_326, 1) }
                    )
            );
            ...
            AddStep(
                ...
                {
                    if (
                        EditorBeatmap.HitObjects.Any(x =>
                            x is ManiaHitObject m
                            && m.StartTime == 11_545
                            && m.Column is 1 or 2 or 3
                        )
                    )
                        return;

                    ManiaHitObject first = (ManiaHitObject)
                        EditorBeatmap.HitObjects.First(x =>
                            x is ManiaHitObject m && m.StartTime == 11_545 && m.Column == 0
                        );
            ...
            AddAssert(
                ...
                () =>
                    checkSnapAndSelectColumn(
                        11_545,
                        new List<(int, int)> { (11_545, 0), (11_545, 1), (11_545, 2), (11_545, 3) }
                    )
            );
        ...
        private bool checkSnapAndSelectColumn(
            double startTime,
            IReadOnlyCollection<(int, int)>? columnPairs = null
        )
        {
            bool checkColumns =
                columnPairs != null
                    ? EditorBeatmap.SelectedHitObjects.All(x =>
                        columnPairs.Any(col => isNoteAt(x, col.Item1, col.Item2))
                    )
