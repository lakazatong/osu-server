...
namespace osu.Desktop.Updater
{
    public partial class VelopackUpdateManager : UpdateManager
    {
        ...
        private bool isInGameplay =>
            localUserInfo?.PlayingState.Value != LocalUserPlayingState.NotPlaying;
        ...
        protected override async Task<bool> PerformUpdateCheck(CancellationToken cancellationToken)
        {
            ...
            IUpdateSource updateSource = new GithubSource(
                @"https://github.com/ppy/osu",
                null,
                ReleaseStream.Value == Game.Configuration.ReleaseStream.Tachyon
            );
            Velopack.UpdateManager updateManager = new Velopack.UpdateManager(
                updateSource,
        ...
        private void downloadUpdate(
            Velopack.UpdateManager updateManager,
            UpdateInfo update,
            ...
            Task.Run(
                async () =>
                {
                    ...
                    UpdateDownloadProgressNotification progressNotification =
                        new UpdateDownloadProgressNotification(cancellationToken)
                        {
                    ...
                    try
                    {
                        using (
                            var cts = CancellationTokenSource.CreateLinkedTokenSource(
                                progressNotification.CancellationToken,
                                cancellationToken
                            )
                        )
                        {
                            ...
                            await updateManager
                                .DownloadUpdatesAsync(
                                    update,
                                    p => progressNotification.Progress = p / 100f,
                                    cts.Token
                                )
                            ...
                            runOutsideOfGameplay(
                                () =>
                                    progressNotification.State =
                                        ProgressNotificationState.Completed,
                                cts.Token
                            );
                        }
                    }
        ...
        private void restartToApplyUpdate(
            ...
            {
                await updateManager
                    .WaitExitThenApplyUpdatesAsync(update.TargetFullRelease)
                    .ConfigureAwait(false);
