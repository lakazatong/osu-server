...
namespace osu.Game.Rulesets.Taiko.Tests
{
    ...
    public partial class TestSceneFlyingHits : DrawableTaikoRulesetTestScene
    {
        ...
        {
            var tick = new DrumRollTick(new DrumRoll())
            {
            ...
            DrawableRuleset.Playfield.Add(
                h = new DrawableDrumRollTick(tick) { JudgementType = hitType }
            );
            ((TaikoPlayfield)DrawableRuleset.Playfield).OnNewResult(
                h,
                new JudgementResult(tick, new TaikoDrumRollTickJudgement())
                {
