...
namespace osu.Game.Rulesets.Taiko.Tests
{
    public partial class TestSceneDrumSampleTriggerSource : OsuTestScene
    {
        ...
        public void TestNormalHit()
        {
            ...
            AddAssert(
                "most valid object is hit",
                () => triggerSource.GetMostValidObject(),
                Is.InstanceOf<Hit>
            );
            checkSamples(
                HitType.Centre,
        ...
        public void TestBetweenHits()
        {
            ...
            AddAssert(
                "most valid object is first hit",
                () => triggerSource.GetMostValidObject(),
                () => Is.EqualTo(first)
            );
            checkSamples(
            ...
            seekTo(480);
            AddAssert(
                "most valid object is second hit",
                () => triggerSource.GetMostValidObject(),
                () => Is.EqualTo(second)
            );
            checkSamples(HitType.Centre, false, HitSampleInfo.HIT_NORMAL, HitSampleInfo.BANK_SOFT);
        ...
        {
            AddStep(
                "add strong hit with drum samples",
                () =>
                {
                    var hit = new Hit
                    {
            ...
            AddAssert(
                "most valid object is nested strong hit",
                () => triggerSource.GetMostValidObject(),
                Is.InstanceOf<Hit.StrongNestedHit>
            );
            checkSamples(
                HitType.Centre,
                true,
                $"{HitSampleInfo.HIT_NORMAL},{HitSampleInfo.HIT_FINISH}",
                HitSampleInfo.BANK_DRUM
            );
            checkSamples(
                HitType.Rim,
                true,
                $"{HitSampleInfo.HIT_CLAP},{HitSampleInfo.HIT_WHISTLE}",
                HitSampleInfo.BANK_DRUM
            );
        ...
        {
            AddStep(
                "add drum roll with normal samples",
                () =>
                {
            ...
            AddAssert(
                "most valid object is drum roll tick",
                () => triggerSource.GetMostValidObject(),
                Is.InstanceOf<DrumRollTick>
            );
            checkSamples(
                HitType.Centre,
                false,
            ...
            seekTo(1200);
            AddAssert(
                "most valid object is drum roll",
                () => triggerSource.GetMostValidObject(),
                Is.InstanceOf<DrumRoll>
            );
            checkSamples(
                HitType.Centre,
                false,
        ...
        {
            AddStep(
                "add drum roll with soft samples",
                () =>
                {
                    var drumRoll = new DrumRoll
                    {
        ...
        {
            AddStep(
                "add strong drum roll with drum samples",
                () =>
                {
                    var drumRoll = new DrumRoll
                    {
        ...
        public void TestNormalSwell()
        {
            ...
            AddAssert(
                "most valid object is swell tick",
                () => triggerSource.GetMostValidObject(),
                Is.InstanceOf<SwellTick>
            );
            checkSamples(
                HitType.Centre,
                false,
        ...
        private void checkSamples(
            HitType hitType,
            bool strong,
            string expectedSamplesCsv,
            string expectedBank
        )
        {
            AddStep($"hit {hitType}", () => triggerSource.Play(hitType, strong));
            AddAssert(
                $"last played sample is {expectedSamplesCsv}",
                () =>
                    string.Join(
                        ',',
                        triggerSource.LastPlayedSamples!.OfType<HitSampleInfo>().Select(s => s.Name)
                    ),
                () => Is.EqualTo(expectedSamplesCsv)
            );
            AddAssert(
                $"last played sample has {expectedBank} bank",
                () => triggerSource.LastPlayedSamples!.OfType<HitSampleInfo>().First().Bank,
                () => Is.EqualTo(expectedBank)
            );
        }

        private void seekTo(double time) =>
            AddStep($"seek to {time}", () => gameplayClock.Seek(time));
