    ...
    public partial class TestSceneMultiplayerBeatmapAvailabilityTracker : MultiplayerTestScene
    {
                    ...
                    api.HandleRequest = req =>
                    {
                        switch (req)
                        {
                            case GetBeatmapsRequest beatmapsReq:
                                ...
                                availableApiBeatmap.OnlineBeatmapSetID = availableBeatmap
                                    .BeatmapSet!
                                    .OnlineID;
                                availableApiBeatmap.Checksum = availableBeatmap.MD5Hash;
                                availableApiBeatmap.BeatmapSet!.OnlineID = availableBeatmap
                                    .BeatmapSet!
                                    .OnlineID;

                                var unavailableApiBeatmap = CreateAPIBeatmap();
                                unavailableApiBeatmap.OnlineID = unavailableBeatmap.OnlineID;
                                unavailableApiBeatmap.OnlineBeatmapSetID = unavailableBeatmap
                                    .BeatmapSet!
                                    .OnlineID;
                                unavailableApiBeatmap.Checksum = unavailableBeatmap.MD5Hash;
                                unavailableApiBeatmap.BeatmapSet!.OnlineID = unavailableBeatmap
                                    .BeatmapSet!
                                    .OnlineID;

                                beatmapsReq.TriggerSuccess(
        ...
        public void TestEnterRoomWithNotDownloadedBeatmap()
        {
            ...
            AddUntilStep(
                ...
                () => tracker.Availability.Value.State,
                () => Is.EqualTo(DownloadState.NotDownloaded)
            );
        }
        ...
        public void TestEnterRoomWithLocallyAvailableBeatmap()
        {
            ...
            AddUntilStep(
                "beatmap is available",
                () => tracker.Availability.Value.State,
                () => Is.EqualTo(DownloadState.LocallyAvailable)
            );
        }
        ...
        public void TestAvailabilityUpdatesOnItemEdit()
        {
            ...
            AddStep(
                "change item to not downloaded beatmap",
                () =>
                {
                    PlaylistItem newItem = new PlaylistItem(
                        MultiplayerClient.ClientRoom!.CurrentPlaylistItem
                    ).With(beatmap: new Optional<IBeatmapInfo>(unavailableBeatmap));
                    MultiplayerClient
                        .EditPlaylistItem(new MultiplayerPlaylistItem(newItem))
                        .WaitSafely();
                }
            );
            ...
            AddStep(
                "change item to downloaded beatmap",
                () =>
                {
                    PlaylistItem newItem = new PlaylistItem(
                        MultiplayerClient.ClientRoom!.CurrentPlaylistItem
                    ).With(beatmap: new Optional<IBeatmapInfo>(availableBeatmap));
                    MultiplayerClient
                        .EditPlaylistItem(new MultiplayerPlaylistItem(newItem))
                        .WaitSafely();
                }
        ...
        public void TestAvailabilityUpdatesOnSettingsChange()
        {
            AddStep(
                ...
                () =>
                {
                    ...
                    room.Playlist =
                    [
                        new PlaylistItem(availableBeatmap),
                        new PlaylistItem(unavailableBeatmap),
                    ];
                    JoinRoom(room);
                }
            );
            ...
            AddStep(
                "change settings to not downloaded beatmap",
                () =>
                    MultiplayerClient
                        .ChangeServerRoomSettings(
                            new MultiplayerRoomSettings(MultiplayerClient.ClientAPIRoom!)
                            {
            ...
            AddStep(
                "change settings to downloaded beatmap",
                () =>
                    MultiplayerClient
                        .ChangeServerRoomSettings(
                            new MultiplayerRoomSettings(MultiplayerClient.ClientAPIRoom!)
