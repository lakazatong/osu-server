    ...
    public class RealmLiveTests : RealmTest
    {
        ...
        public void TestLiveEquality()
        {
            RunTestWithRealm(
                (realm, _) =>
                {
                    Live<BeatmapInfo> beatmap = realm.Run(r =>
                        r.Write(_ =>
                                r.Add(
                                    new BeatmapInfo(
                                        CreateRuleset(),
                    ...
                    Live<BeatmapInfo> beatmap2 = realm.Run(r =>
                        r.All<BeatmapInfo>().First().ToLive(realm)
                    );
        ...
        public void TestNestedWriteCalls()
        {
            RunTestWithRealm(
                (realm, _) =>
                {
                    ...
                    realm.Run(r => r.Write(_ => r.Write(_ => r.Add(beatmap))));
        ...
        public void TestTransactionRolledBackOnException()
        {
            RunTestWithRealm(
                (realm, _) =>
                {
                    ...
                    Assert.Throws<InvalidOperationException>(() =>
                        liveBeatmap.PerformWrite(l => throw new InvalidOperationException())
                    );
                    Assert.IsFalse(liveBeatmap.PerformRead(l => l.Hidden));
                ...
                (realm, _) =>
                {
                    ...
                    Task.Factory.StartNew(
                            () =>
                            {
                                ...
                                {
                                    var beatmap = threadContext.Write(r =>
                                        r.Add(
                                            new BeatmapInfo(
                                                CreateRuleset(),
                                                new BeatmapDifficulty(),
            ...
            RunTestWithRealm(
                (realm, _) =>
                {
                    ...
                    Task.Factory.StartNew(
                            () =>
                            {
                                liveBeatmap.PerformWrite(beatmap =>
                                {
                                    beatmap.Hidden = true;
                                });
                                liveBeatmap.PerformRead(beatmap =>
                                {
                                    Assert.IsTrue(beatmap.Hidden);
                                });
                            },
                            TaskCreationOptions.LongRunning | TaskCreationOptions.HideScheduler
                        )
                        ...
                        Task.Factory.StartNew(
                                () =>
                                {
                                    realm.Run(innerRealm =>
                                    {
                                        ...
                                        var beatmap = innerRealm.Write(r =>
                                            r.Add(
                                                new BeatmapInfo(
                                                    ruleset,
                                                    new BeatmapDifficulty(),
