                ...
                (realmAccess, storage) =>
                {
                    ...
                    Assert.True(
                        files.Storage.Exists(
                            "0/05/054edec1d0211f624fed0cbca9d4f9400b0e491c43742af2c5b0abebf0c990d8"
                        )
                    );
                    Assert.True(
                        files.Storage.Exists(realm.All<RealmFile>().First().GetStoragePath())
                    );
                }
            );
        }
        ...
        public void TestDontPurgeReferenced()
        {
            RunTestWithRealm(
                (realmAccess, storage) =>
                {
                    ...
                    var file = realm.Write(() =>
                        files.Add(new MemoryStream(new byte[] { 0, 1, 2, 3 }), realm)
                    );
