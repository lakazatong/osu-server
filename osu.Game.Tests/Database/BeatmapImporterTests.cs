    ...
    public class BeatmapImporterTests : RealmTest
    {
        ...
        public void TestDetachBeatmapSet()
        {
            RunTestWithRealmAsync(
                async (realm, storage) =>
                {
                    ...
                    {
                        var beatmapSet = await importer.Import(
                            new ImportTask(TestResources.GetTestBeatmapStream(), "renatus.osz")
                        );
                        ...
                        beatmapSet.PerformRead(live =>
                        {
                            ...
                            Assert.AreEqual(
                                live.Beatmaps.Select(f => f.Difficulty).Count(),
                                detachedBeatmapSet.Beatmaps.Select(f => f.Difficulty).Count()
                            );
                        ...
                        Assert.AreEqual(
                            detachedBeatmapSet,
                            detachedBeatmapSet.Beatmaps.First().BeatmapSet
                        );
                    }
                }
            );
                ...
                async (realm, storage) =>
                {
                    ...
                    using (new RealmRulesetStore(realm, storage))
                    {
                        ...
                        beatmapSet.PerformRead(s =>
                        {
                            ...
                            Assert.AreEqual(
                                s.Files.Select(f => f.File).Count(),
                                detachedBeatmapSet.Files.Select(f => f.File).Count()
                            );
                            Assert.AreEqual(s.Beatmaps.Count, detachedBeatmapSet.Beatmaps.Count);
                            Assert.AreEqual(
                                s.Beatmaps.Select(f => f.Difficulty).Count(),
                                detachedBeatmapSet.Beatmaps.Select(f => f.Difficulty).Count()
                            );
                        ...
                        Task.Run(async () =>
                            {
                                var beatmapSet = await importer.Import(
                                    new ImportTask(
                                        TestResources.GetTestBeatmapStream(),
                                        "renatus.osz"
                                    )
                                );
        ...
        public void TestImportBeatmapThenCleanup()
        {
            RunTestWithRealmAsync(
                async (realm, storage) =>
                {
                    ...
                    using (new RealmRulesetStore(realm, storage))
                    {
                        ...
                        Assert.AreEqual(
                            1,
                            realm.Realm.All<BeatmapSetInfo>().Count(s => s.DeletePending)
                        );
                    }
                }
            );
            ...
            RunTestWithRealm(
                (realm, _) =>
                {
                    Assert.AreEqual(0, realm.Realm.All<BeatmapSetInfo>().Count());
                }
            );
        }
        ...
        public void TestImportThenDeleteFromStream()
        {
            RunTestWithRealmAsync(
                async (realm, storage) =>
                {
                    ...
                    {
                        importedSet = await importer.Import(
                            new ImportTask(stream, Path.GetFileName(tempPath))
                        );
                        EnsureLoaded(realm.Realm);
                    }
        ...
        public void TestImportThenImport()
        {
            RunTestWithRealmAsync(
                async (realm, storage) =>
                {
                    ...
                    Assert.IsTrue(
                        imported.Beatmaps.First().ID == importedSecondTime.Beatmaps.First().ID
                    );
            ...
            RunTestWithRealmAsync(
                async (realm, storage) =>
                {
                    ...
                    try
                    {
                        ...
                        Assert.IsTrue(imported.ID == importedSecondTime.ID);
                        Assert.IsTrue(
                            imported.Beatmaps.First().ID
                                == importedSecondTime.PerformRead(s => s.Beatmaps.First().ID)
                        );
                    }
                    finally
                    {
            ...
            RunTestWithRealmAsync(
                async (realm, storage) =>
                {
                    ...
                    try
                    {
                        ...
                        using (
                            var sw = new FileInfo(
                                Directory.GetFiles(extractedFolder, "*.osu").First()
                            ).AppendText()
                        )
        ...
        public void TestImport_ThenModifyMapWithScore_ThenImport()
        {
            RunTestWithRealmAsync(
                async (realm, storage) =>
                {
                    ...
                    var importedSecondTimeBeatmap = importedSecondTime.PerformRead(s =>
                        s.Beatmaps.First()
                    );
                    ...
                    Assert.That(
                        importedSecondTimeBeatmap.Scores.Single().BeatmapInfo,
                        Is.EqualTo(importedSecondTimeBeatmap)
                    );
                }
            );
        }
            ...
            RunTestWithRealmAsync(
                async (realm, storage) =>
                {
                    ...
                    try
                    {
                        ...
                        using (
                            var sw = new FileInfo(
                                Directory.GetFiles(extractedFolder, "*.mp3").First()
                            ).AppendText()
                        )
            ...
            RunTestWithRealmAsync(
                async (realm, storage) =>
                {
                    ...
                    try
                    {
                        ...
                        firstFile.MoveTo(
                            Path.Combine(
                                firstFile.DirectoryName.AsNonNull(),
                                $"{firstFile.Name}-changed{firstFile.Extension}"
                            )
                        );
        ...
        public void TestImportCorruptThenImport()
        {
            RunTestWithRealmAsync(
                async (realm, storage) =>
                {
                    ...
                    using (
                        var stream = fileStorage.CreateFileSafely(firstFile.File.GetStoragePath())
                    )
                        stream.WriteByte(0);
                    ...
                    using (var stream = fileStorage.GetStream(firstFile.File.GetStoragePath()))
                        Assert.AreEqual(
                            stream.Length,
                            originalLength,
                            "Corruption was not fixed on second import"
                        );
        ...
        public void TestModelCreationFailureDoesntReturn()
        {
            RunTestWithRealmAsync(
                async (realm, storage) =>
                {
                    ...
                    Assert.AreEqual(
                        ProgressNotificationState.Cancelled,
                        progressNotification.State
                    );
                }
            );
        }
        ...
        public void TestRollbackOnFailure()
        {
            RunTestWithRealmAsync(
                async (realm, storage) =>
                {
                    ...
                    MemoryStream brokenOsz = new MemoryStream(
                        await File.ReadAllBytesAsync(brokenTempFilename)
                    );
        ...
        public void TestImportThenReimportWithNewDifficulty()
        {
            RunTestWithRealmAsync(
                async (realm, storage) =>
                {
                    ...
                    string pathMissingOneBeatmap = pathOriginal.Replace(
                        ".osz",
                        "_missing_difficulty.osz"
                    );
                    ...
                    try
                    {
                        ...
                        using (var zip = ZipArchive.Create())
                        {
                            zip.AddAllFromDirectory(extractedFolder);
                            zip.SaveTo(
                                pathMissingOneBeatmap,
                                new ZipWriterOptions(CompressionType.Deflate)
                            );
                        }

                        var firstImport = await importer.Import(
                            new ImportTask(pathMissingOneBeatmap)
                        );
                        ...
                        Assert.That(
                            realm.Realm.All<BeatmapSetInfo>().First(s => !s.DeletePending).Beatmaps,
        ...
        public void TestImportThenReimportAfterMissingFiles()
        {
            RunTestWithRealmAsync(
                async (realmFactory, storage) =>
                {
                    ...
                    Assert.That(
                        imported.Files.All(f =>
                            !storage.GetStorageForDirectory("files").Exists(f.File.GetStoragePath())
                        )
                    );
                    ...
                    Assert.That(
                        importedSecondTime.Files.All(f =>
                            storage.GetStorageForDirectory("files").Exists(f.File.GetStoragePath())
                        )
                    );
                }
        ...
        public void TestImportWhenFileOpen()
        {
            RunTestWithRealmAsync(
                async (realm, storage) =>
                {
                    ...
                    Assert.IsFalse(
                        File.Exists(temp),
                        "We likely held a read lock on the file when we shouldn't"
                    );
                }
            );
        }
                    ...
                    try
                    {
                        ...
                        Assert.IsFalse(
                            imported.PerformRead(s =>
                                s.Files.Any(f => f.Filename.Contains("subfolder"))
                            ),
                            "Files contain common subfolder"
                        );
                    }
                    finally
                    {
        ...
        public void TestImportWithIgnoredDirectoryInArchive()
        {
            RunTestWithRealmAsync(
                async (realm, storage) =>
                {
                    ...
                    {
                        await resourceForkFile.WriteLineAsync(
                            "adding content so that it's not empty"
                        );
                    }
                    ...
                    try
                    {
                        ...
                        Assert.IsFalse(
                            imported.PerformRead(s =>
                                s.Files.Any(f => f.Filename.Contains("__MACOSX"))
                            ),
                            "Files contain resource fork folder, which should be ignored"
                        );
                        Assert.IsFalse(
                            imported.PerformRead(s =>
                                s.Files.Any(f => f.Filename.Contains("actual_data"))
                            ),
                            "Files contain common subfolder"
                        );
                    }
        ...
        public void TestUpdateBeatmapInfo()
        {
            RunTestWithRealmAsync(
                async (realm, storage) =>
                {
                    ...
                    BeatmapInfo updatedInfo = realm
                        .Realm.All<BeatmapInfo>()
                        .First(b => b.ID == beatmapToUpdate.ID);
                    Assert.That(updatedInfo.DifficultyName, Is.EqualTo("updated"));
                }
            );
        }

        public static async Task<BeatmapSetInfo?> LoadQuickOszIntoOsu(
            BeatmapImporter importer,
            Realm realm
        )
        {
        ...
        public static async Task<BeatmapSetInfo> LoadOszIntoStore(
            BeatmapImporter importer,
            Realm realm,
            string? path = null,
        ...
        )
        {
            ...
            var importedSet = await importer.Import(
                new ImportTask(temp),
                new ImportParameters { Batch = batchImport }
            );
        ...
        private static void checkBeatmapSetCount(
            Realm realm,
            int expected,
            bool includeDeletePending = false
        )
        {
            Assert.AreEqual(
                expected,
        ...
        internal static void EnsureLoaded(Realm realm, int timeout = 60000)
        {
            ...
            waitForOrAssert(
                () =>
                {
                    ...
                    return (
                        resultSets = realm
                            .All<BeatmapSetInfo>()
                            .Where(s => !s.DeletePending && s.OnlineID == 241526)
                    ).Any();
                },
            ...
            Assert.IsTrue(
                resultSets?.Count() == 1,
                $@"Incorrect result count found ({resultSets?.Count()} but should be 1)."
            );

            IEnumerable<BeatmapSetInfo> queryBeatmapSets() =>
                realm.All<BeatmapSetInfo>().Where(s => !s.DeletePending && s.OnlineID == 241526);

            var set = queryBeatmapSets().First();

            // ReSharper disable once PossibleUnintendedReferenceComparison
            IEnumerable<BeatmapInfo> queryBeatmaps() =>
                realm.All<BeatmapInfo>().Where(s => s.BeatmapSet != null && s.BeatmapSet == set);
