    ...
    public partial class TestSceneScoreProcessor : OsuTestScene
    {
        ...
        public void TestNoScoreIncreaseFromMiss()
        {
            ...
            scoreProcessor.ApplyResult(
                new JudgementResult(new HitObject(), new TestJudgement()) { Type = HitResult.Miss }
            );
        ...
        public void TestOnlyBonusScore()
        {
            ...
            // Apply a judgement
            scoreProcessor.ApplyResult(
                new JudgementResult(new HitObject(), new TestJudgement(HitResult.LargeBonus))
                {
                    Type = HitResult.LargeBonus,
                }
            );

            Assert.That(
                scoreProcessor.TotalScore.Value,
                Is.EqualTo(scoreProcessor.GetBaseScoreForResult(HitResult.LargeBonus))
            );
        }
        ...
        public void TestResetFromReplayFrame()
        {
            ...
            scoreProcessor.ApplyResult(
                new JudgementResult(beatmap.HitObjects[0], new TestJudgement(HitResult.Great))
                {
            ...
            scoreProcessor.ResetFromReplayFrame(
                new OsuReplayFrame
                {
                    Header = new FrameHeader(
                        ...
                        0,
                        0,
                        new Dictionary<HitResult, int> { { HitResult.Miss, 1 } },
        ...
        public void TestFailScore()
        {
            ...
            scoreProcessor.ApplyResult(
                ...
                {
                    Type = HitResult.LargeTickHit,
                }
            );
            scoreProcessor.ApplyResult(
                new JudgementResult(beatmap.HitObjects[2], beatmap.HitObjects[2].Judgement)
                {
                    Type = HitResult.SmallTickMiss,
                }
            );
            scoreProcessor.ApplyResult(
                new JudgementResult(beatmap.HitObjects[3], beatmap.HitObjects[3].Judgement)
                {
                    Type = HitResult.SmallBonus,
                }
            );
        ...
        public void TestAccuracyModes()
        {
            var beatmap = new Beatmap<HitObject>
            {
                HitObjects = Enumerable
                    .Range(0, 4)
                    .Select(_ => new TestHitObject(HitResult.Great))
                    .ToList<HitObject>(),
            };
            ...
            Assert.That(
                scoreProcessor.Accuracy.Value,
                Is.EqualTo((double)(100 + 300) / (2 * 300)).Within(Precision.DOUBLE_EPSILON)
            );
            Assert.That(
                scoreProcessor.MinimumAccuracy.Value,
                Is.EqualTo((double)(100 + 300) / (4 * 300)).Within(Precision.DOUBLE_EPSILON)
            );
            Assert.That(
                scoreProcessor.MaximumAccuracy.Value,
                Is.EqualTo((double)(100 + 3 * 300) / (4 * 300)).Within(Precision.DOUBLE_EPSILON)
            );
        }
