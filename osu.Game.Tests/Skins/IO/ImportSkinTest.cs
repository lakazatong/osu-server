...
namespace osu.Game.Tests.Skins.IO
{
    public class ImportSkinTest : ImportTest
    {
        ...
        public Task TestImportModifiedSkinHasResources(string archive) =>
            runSkinTest(async osu =>
            {
        ...
        [Test]
        public Task TestSingleImportDifferentFilename() =>
            runSkinTest(async osu =>
            {
                var import1 = await loadSkinIntoOsu(
                    osu,
                    new ImportTask(createOskWithIni("test skin", "skinner"), "skin.osk")
                );
        ...
        [Test]
        public Task TestSingleImportWeirdIniFileCase() =>
            runSkinTest(async osu =>
            {
                var import1 = await loadSkinIntoOsu(
                    osu,
                    new ImportTask(
                        createOskWithIni("test skin", "skinner", iniFilename: "Skin.InI"),
                        "skin.osk"
                    )
                );
        ...
        [Test]
        public Task TestSingleImportMissingSectionHeader() =>
            runSkinTest(async osu =>
            {
                var import1 = await loadSkinIntoOsu(
                    osu,
                    new ImportTask(
                        createOskWithIni("test skin", "skinner", includeSectionHeader: false),
                        "skin.osk"
                    )
                );
        ...
        [Test]
        public Task TestSingleImportMatchingFilename() =>
            runSkinTest(async osu =>
            {
                var import1 = await loadSkinIntoOsu(
                    osu,
                    new ImportTask(createOskWithIni("test skin", "skinner"), "test skin.osk")
                );
        ...
        [Test]
        public Task TestSingleImportNoIniFile() =>
            runSkinTest(async osu =>
            {
                var import1 = await loadSkinIntoOsu(
                    osu,
                    new ImportTask(createOskWithNonIniFile(), "test skin.osk")
                );

                // When the import filename matches it shouldn't be appended.
                assertCorrectMetadata(
                    import1,
                    "test skin",
                    "Unknown",
        ...
        [Test]
        public Task TestEmptyImportImportsWithFilename() =>
            runSkinTest(async osu =>
            {
                var import1 = await loadSkinIntoOsu(
                    osu,
                    new ImportTask(createEmptyOsk(), "test skin.osk")
                );
        ...
        [Test]
        public Task TestImportTwiceWithSameMetadataAndFilename([Values] bool batchImport) =>
            runSkinTest(async osu =>
            {
                var import1 = await loadSkinIntoOsu(
                    osu,
                ...
                var import2 = await loadSkinIntoOsu(
                    osu,
                    new ImportTask(createOskWithIni("test skin", "skinner"), "skin.osk"),
                    batchImport
                );
        ...
        [Test]
        public Task TestImportTwiceWithNoMetadataSameDownloadFilename([Values] bool batchImport) =>
            runSkinTest(async osu =>
            {
                ...
                var import1 = await loadSkinIntoOsu(
                    osu,
                    new ImportTask(createOskWithIni(string.Empty, string.Empty), "download.osk"),
                    batchImport
                );
                var import2 = await loadSkinIntoOsu(
                    osu,
        ...
        [Test]
        public Task TestImportUpperCasedOskArchive() =>
            runSkinTest(async osu =>
            {
                var import1 = await loadSkinIntoOsu(
                    osu,
                    new ImportTask(createOskWithIni("name 1", "author 1"), "name 1.OsK")
                );
        ...
        [Test]
        public Task TestImportExportedSkinFilename() =>
            runSkinTest(async osu =>
            {
                ...
                var import1 = await loadSkinIntoOsu(
                    osu,
                    new ImportTask(createOskWithIni("name 1", "author 1"), "custom.osk")
                );
                assertCorrectMetadata(import1, "name 1 [custom]", "author 1", 1.0m, osu);

                await new LegacySkinExporter(osu.Dependencies.Get<Storage>()).ExportToStreamAsync(
                    import1,
                    exportStream
                );
                ...
                var import2 = await loadSkinIntoOsu(
                    osu,
                    new ImportTask(exportStream, $"{exportFilename}.osk")
                );
        ...
        [Test]
        public Task TestImportExportedNonAsciiSkinFilename() =>
            runSkinTest(async osu =>
            {
                MemoryStream exportStream = new MemoryStream();
        ...
        [Test]
        public Task TestSameMetadataNameSameFolderName([Values] bool batchImport) =>
            runSkinTest(async osu =>
            {
                var import1 = await loadSkinIntoOsu(
                    osu,
                    new ImportTask(createOskWithIni("name 1", "author 1"), "my custom skin 1"),
                    batchImport
                );
                var import2 = await loadSkinIntoOsu(
                    osu,
        ...
        [Test]
        public Task TestImportWithSubfolder() =>
            runSkinTest(async osu =>
            {
                const string filename = "Archives/skin-with-subfolder-zip-entries.osk";
                var import = await loadSkinIntoOsu(
                    osu,
                    new ImportTask(TestResources.OpenResource(filename), filename)
                );

                assertCorrectMetadata(
                    import,
                    $"Totally fully features skin [Real Skin with Real Features] [{filename[..^4]}]",
                    "Unknown",
                    2.7m,
                    osu
                );
        ...
        [Test]
        public Task TestImportTwiceWithSameMetadataButDifferentFilename() =>
            runSkinTest(async osu =>
            {
                var import1 = await loadSkinIntoOsu(
                    osu,
                ...
                var import2 = await loadSkinIntoOsu(
                    osu,
                    new ImportTask(createOskWithIni("test skin", "skinner"), "skin2.osk")
                );
        ...
        [Test]
        public Task TestImportTwiceWithNoMetadataDifferentDownloadFilename() =>
            runSkinTest(async osu =>
            {
                // if a user downloads two skins that do have skin.ini files but don't have any creator metadata in the skin.ini, they should both import separately just for safety.
                var import1 = await loadSkinIntoOsu(
                ...
                var import2 = await loadSkinIntoOsu(
                    osu,
                    new ImportTask(createOskWithIni(string.Empty, string.Empty), "download2.osk")
                );
        ...
        [Test]
        public Task TestImportTwiceWithSameFilenameDifferentMetadata() =>
            runSkinTest(async osu =>
            {
                var import1 = await loadSkinIntoOsu(
                    osu,
                    new ImportTask(createOskWithIni("test skin v2", "skinner"), "skin.osk")
                );
                var import2 = await loadSkinIntoOsu(
                    osu,
                    new ImportTask(createOskWithIni("test skin v2.1", "skinner"), "skin.osk")
                );
        ...
        [Test]
        public Task TestSameMetadataNameDifferentFolderName() =>
            runSkinTest(async osu =>
            {
                var import1 = await loadSkinIntoOsu(
                    osu,
                ...
                var import2 = await loadSkinIntoOsu(
                    osu,
                    new ImportTask(createOskWithIni("name 1", "author 1"), "my custom skin 2")
                );
        ...
        public Task TestExportThenImportDefaultSkin() =>
            runSkinTest(async osu =>
            {
                ...
                await skinManager.CurrentSkinInfo.Value.PerformRead(async s =>
                {
                    ...
                    await new LegacySkinExporter(
                        osu.Dependencies.Get<Storage>()
                    ).ExportToStreamAsync(skinManager.CurrentSkinInfo.Value, exportStream);

                    Assert.Greater(exportStream.Length, 0);
                });

                var imported = await skinManager.Import(
                    new ImportTask(exportStream, "exported.osk")
                );
        ...
        [Test]
        public Task TestExportThenImportClassicSkin() =>
            runSkinTest(async osu =>
            {
                var skinManager = osu.Dependencies.Get<SkinManager>();
                ...
                await skinManager.CurrentSkinInfo.Value.PerformRead(async s =>
                {
                    ...
                    Assert.AreEqual(
                        ...
                        s.CreateInstance(skinManager).GetType()
                    );
        ...
        private void assertCorrectMetadata(
            Live<SkinInfo> import1,
            string name,
            string creator,
            decimal version,
            OsuGameBase osu
        )
        {
            import1.PerformRead(i =>
            {
                ...
                var instance = i.CreateInstance(
                    (IStorageResourceProvider)osu.Dependencies.Get(typeof(SkinManager))
                );
        ...
        {
            import1.PerformRead(i1 =>
                import2.PerformRead(i2 =>
                {
        ...
        private MemoryStream createOskWithIni(
            string name,
            string author,
            bool makeUnique = false,
            string iniFilename = @"skin.ini",
            bool includeSectionHeader = true
        )
        {
            var zipStream = new MemoryStream();
            using var zip = ZipArchive.Create();
            zip.AddEntry(
                iniFilename,
                generateSkinIni(name, author, makeUnique, includeSectionHeader)
            );
            zip.SaveTo(zipStream);
            return zipStream;
        }

        private MemoryStream generateSkinIni(
            string name,
            string author,
        ...
        private async Task runSkinTest(
            Func<OsuGameBase, Task> action,
            [CallerMemberName] string callingMethodName = @""
        )
        {
            using (
                HeadlessGameHost host = new CleanRunHeadlessGameHost(
        ...
        private async Task<Live<SkinInfo>> loadSkinIntoOsu(
            OsuGameBase osu,
            ImportTask import,
