    ...
    public partial class TestSceneSoloResultsScreen : ScreenTestScene
    {
        ...
        public override void SetUpSteps()
        {
            ...
            AddStep(
                ...
                () =>
                {
                    ...
                    importedBeatmap = beatmapManager
        ...
        public void TestLocalLeaderboardWithOfflineScore()
        {
            ...
            AddUntilStep(
                "local score is #16",
                () =>
                    this.ChildrenOfType<ScorePanelList>()
                        .Single()
                        .GetPanelForScore(localScore)
                        .ScorePosition.Value,
                () => Is.EqualTo(16)
            );
        }
                    ...
                    dummyAPI.HandleRequest = req =>
                    {
                        switch (req)
                        {
                            case GetScoresRequest getScoresRequest:
                                ...
                                getScoresRequest.TriggerSuccess(
                                    new APIScoresCollection { Scores = scores }
                                );
                                return true;
                        }
        ...
        public void TestOnlineLeaderboardWithLessThan50Scores_UserWasInTop50()
        {
            ...
            AddAssert(
                "previous user best not shown",
                () => this.ChildrenOfType<ScorePanel>().All(p => p.Score.OnlineID != 123456)
            );
        }
        ...
        public void TestOnlineLeaderboardWithLessThan50Scores_ShowingAnotherUserScore()
        {
            ...
            AddUntilStep("wait for loaded", () => ((Drawable)Stack.CurrentScreen).IsLoaded);
            AddAssert(
                "local user best shown",
                () =>
                    this.ChildrenOfType<ScorePanel>()
                        .Any(p => p.Score.UserID == API.LocalUser.Value.Id)
            );
        }
        ...
        public void TestOnlineLeaderboardWithMoreThan50Scores_UserOutsideOfTop50_DidNotBeatOwnBest()
        {
                    ...
                    dummyAPI.HandleRequest = req =>
                    {
                        switch (req)
                        {
                            case GetScoresRequest getScoresRequest:
                                ...
                                var userBest = SoloScoreInfo.ForSubmission(
                                    TestResources.CreateTestScoreInfo(importedBeatmap)
                                );
            ...
            AddUntilStep(
                "previous user best shown at same position",
                () =>
                    this.ChildrenOfType<ScorePanel>()
                        .Any(p => p.Score.OnlineID == 123456 && p.ScorePosition.Value == 133_337)
            );
        }
        ...
        public void TestOnlineLeaderboardDeduplication()
        {
            ...
            AddUntilStep("wait for loaded", () => ((Drawable)Stack.CurrentScreen).IsLoaded);
            AddAssert(
                "only one score with ID 12345",
                () => this.ChildrenOfType<ScorePanel>().Count(s => s.Score.OnlineID == 12345),
                () => Is.EqualTo(1)
            );
            AddUntilStep(
                "user best position preserved",
                () => this.ChildrenOfType<ScorePanel>().Any(p => p.ScorePosition.Value == 133_337)
            );
        }
    }
}
