...
namespace osu.Game.Tests.Visual.Ranking
{
    public partial class TestSceneStatisticsPanel : OsuTestScene
    {
        ...
        public void TestScoreInRulesetWhereAllStatsRequireHitEvents()
        {
            loadPanel(
                TestResources.CreateTestScoreInfo(
                    new TestRulesetAllStatsRequireHitEvents().RulesetInfo
                )
            );
        }
        ...
        public void TestScoreInRulesetWhereNoStatsRequireHitEvents()
        {
            loadPanel(
                TestResources.CreateTestScoreInfo(
                    new TestRulesetNoStatsRequireHitEvents().RulesetInfo
                )
            );
        }
        ...
        public void TestStatisticsShownCorrectlyIfUpdateDeliveredBeforeLoad()
        {
            ...
            AddStep(
                "create user statistics watcher",
                () =>
                    Add(
                        userStatisticsWatcher = new UserStatisticsWatcher(
                            new LocalUserStatisticsProvider()
                        )
                    )
            );
            AddStep(
                "set user statistics update",
                () =>
                {
                    score = TestResources.CreateTestScoreInfo();
                    score.OnlineID = 1234;
                    (
                        (Bindable<ScoreBasedUserStatisticsUpdate>)userStatisticsWatcher.LatestUpdate
                    ).Value = new ScoreBasedUserStatisticsUpdate(
                        score,
                        new UserStatistics
                        {
                            Level = new UserStatistics.LevelInfo { Current = 5, Progress = 20 },
                            GlobalRank = 38000,
                            CountryRank = 12006,
                            PP = 2134,
                        ...
                        new UserStatistics
                        {
                            Level = new UserStatistics.LevelInfo { Current = 5, Progress = 30 },
                            GlobalRank = 36000,
                            CountryRank = 12000,
                            PP = (decimal)2134.5,
            ...
            AddStep(
                "load user statistics panel",
                () =>
                    Child = new DependencyProvidingContainer
                    {
                        CachedDependencies =
                        [
                            (typeof(UserStatisticsWatcher), userStatisticsWatcher),
                        ],
            ...
            AddUntilStep(
                "overall ranking present",
                () => this.ChildrenOfType<OverallRanking>().Any()
            );
            AddUntilStep(
                "loading spinner not visible",
                () =>
                    this.ChildrenOfType<OverallRanking>()
                        .Single()
                        .ChildrenOfType<LoadingLayer>()
                        .All(l => l.State.Value == Visibility.Hidden)
            );
        }
        ...
        private class TestRuleset : Ruleset
        {
            ...
            public override IBeatmapConverter CreateBeatmapConverter(IBeatmap beatmap) =>
                new TestBeatmapConverter(beatmap);
            ...
            protected static Drawable CreatePlaceholderStatistic(string message) =>
                new Container
                {
...
#pragma warning restore CS0067
        ...
        private class TestRulesetAllStatsRequireHitEvents : TestRuleset
        {
            public override StatisticItem[] CreateStatisticsForScore(
                ...
                {
                    new StatisticItem(
                        "Statistic Requiring Hit Events 1",
                        () =>
                            CreatePlaceholderStatistic(
                                "Placeholder statistic. Requires hit events"
                            ),
                        true
                    ),
                    new StatisticItem(
                        "Statistic Requiring Hit Events 2",
                        () =>
                            CreatePlaceholderStatistic(
                                "Placeholder statistic. Requires hit events"
                            ),
        ...
        private class TestRulesetNoStatsRequireHitEvents : TestRuleset
        {
            ...
            )
            {
                ...
                {
                    new StatisticItem(
                        "Statistic Not Requiring Hit Events 1",
                        () =>
                            CreatePlaceholderStatistic(
                                "Placeholder statistic. Does not require hit events"
                            )
                    ),
                    new StatisticItem(
                        "Statistic Not Requiring Hit Events 2",
                        () =>
                            CreatePlaceholderStatistic(
                                "Placeholder statistic. Does not require hit events"
                            )
        ...
        private class TestRulesetMixed : TestRuleset
        {
            ...
            )
            {
                ...
                {
                    new StatisticItem(
                        "Statistic Requiring Hit Events",
                        () =>
                            CreatePlaceholderStatistic(
                                "Placeholder statistic. Requires hit events"
                            ),
                    ...
                    new StatisticItem(
                        "Statistic Not Requiring Hit Events",
                        () =>
                            CreatePlaceholderStatistic(
                                "Placeholder statistic. Does not require hit events"
                            )
