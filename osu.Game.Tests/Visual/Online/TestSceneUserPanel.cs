...
namespace osu.Game.Tests.Visual.Online
{
    ...
    public partial class TestSceneUserPanel : OsuTestScene
    {
        ...
        public void SetUp() =>
            Schedule(() =>
            {
                Child = new DependencyProvidingContainer
                {
                    ...
                    [
                        (
                            typeof(LocalUserStatisticsProvider),
                            statisticsProvider = new TestUserStatisticsProvider()
                        ),
        ...
        public void TestUserActivity()
        {
            ...
            AddStep(
                ...
                () =>
                    setPresence(
                        UserStatus.Online,
                        new UserActivity.WatchingReplay(createScore(@"nats"))
                    )
            );
            AddStep(
                "spectating user",
                () =>
                    setPresence(
                        UserStatus.Online,
                        new UserActivity.SpectatingUser(createScore(@"mrekk"))
                    )
            );
            AddStep(
                "solo (osu!)",
                () => setPresence(UserStatus.Online, soloGameStatusForRuleset(0))
            );
            AddStep(
                "solo (osu!taiko)",
                () => setPresence(UserStatus.Online, soloGameStatusForRuleset(1))
            );
            AddStep(
                "solo (osu!catch)",
                () => setPresence(UserStatus.Online, soloGameStatusForRuleset(2))
            );
            AddStep(
                "solo (osu!mania)",
                () => setPresence(UserStatus.Online, soloGameStatusForRuleset(3))
            );
            AddStep(
                "choosing",
                () => setPresence(UserStatus.Online, new UserActivity.ChoosingBeatmap())
            );
            AddStep(
                "editing beatmap",
                () =>
            ...
            AddStep(
                ...
                () =>
                    setPresence(
                        UserStatus.Online,
                        new UserActivity.ModdingBeatmap(new BeatmapInfo())
                    )
            );
            AddStep(
                "testing beatmap",
                () =>
                    setPresence(
                        UserStatus.Online,
                        new UserActivity.TestingBeatmap(new BeatmapInfo())
                    )
            );
        }
        ...
        public void TestUserStatisticsChange()
        {
            ...
            AddStep(
                "set statistics to something big",
                () =>
                {
                    statisticsProvider.UpdateStatistics(
                        new UserStatistics
            ...
            AddStep(
                "set statistics to empty",
                () => statisticsProvider.UpdateStatistics(new UserStatistics(), Ruleset.Value)
            );
        }
        ...
        public void TestLocalUserActivity()
        {
            AddStep(
                "idle",
                () => setPresence(UserStatus.Online, null, API.LocalUser.Value.OnlineID)
            );
            AddStep(
                "watching replay",
                () =>
            ...
            AddStep(
                ...
                () =>
                    setPresence(
                        UserStatus.Online,
                        new UserActivity.ChoosingBeatmap(),
                        API.LocalUser.Value.OnlineID
                    )
            );
            AddStep(
            ...
            AddStep(
                "set offline status",
                () => setPresence(UserStatus.Offline, null, API.LocalUser.Value.OnlineID)
            );
        }
        ...
        private void setPresence(UserStatus status, UserActivity? activity, int? userId = null)
        {
            if (status == UserStatus.Offline)
                ...
                metadataClient.UserPresenceUpdated(
                    userId ?? panel.User.OnlineID,
                    new UserPresence { Status = status, Activity = activity }
                );
        }

        private UserActivity soloGameStatusForRuleset(int rulesetId) =>
            new UserActivity.InSoloGame(new BeatmapInfo(), rulesetStore.GetRuleset(rulesetId)!);

        private ScoreInfo createScore(string name) =>
            new ScoreInfo(new TestBeatmap(Ruleset.Value).BeatmapInfo)
            {
                User = new APIUser { Username = name },
            };
        ...
        {
            public new void UpdateStatistics(
