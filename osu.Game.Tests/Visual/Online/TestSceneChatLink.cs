...
namespace osu.Game.Tests.Visual.Online
{
    ...
    public partial class TestSceneChatLink : OsuTestScene
    {
        ...
        private void load(OsuColour colours)
        {
            ...
            BindableList<Channel> availableChannels =
                (BindableList<Channel>)chatManager.AvailableChannels;
        ...
        [TestCase(
            $"{OsuGameBase.OSU_PROTOCOL}edit/00:12:345 - Test?",
            LinkAction.OpenEditorTimestamp
        )]
        [TestCase(
            $"{OsuGameBase.OSU_PROTOCOL}edit/00:12:345 (1,2) - Test?",
            LinkAction.OpenEditorTimestamp
        )]
        [TestCase(
            $"{OsuGameBase.OSU_PROTOCOL}00:12:345 - not an editor timestamp",
            LinkAction.External
        )]
        ...
        [TestCase(
            "[osu forums](https://dev.ppy.sh/forum) (new link format 2)",
            LinkAction.External
        )]
        [TestCase(
            "[https://dev.ppy.sh/home This is only a link to the new osu webpage but this is supposed to test word wrap.]",
            LinkAction.External
        )]
        [TestCase(
            "Let's (try)[https://dev.ppy.sh/home] [https://dev.ppy.sh/b/252238 multiple links] https://dev.ppy.sh/home",
            LinkAction.External,
            LinkAction.OpenBeatmap,
            LinkAction.External
        )]
        [TestCase(
            "[https://dev.ppy.sh/home New link format with escaped [and \\[ paired] braces]",
            LinkAction.External
        )]
        [TestCase(
            "[Markdown link format with escaped [and \\[ paired] braces](https://dev.ppy.sh/home)",
            LinkAction.External
        )]
        [TestCase(
            "(Old link format with escaped (and \\( paired) parentheses)[https://dev.ppy.sh/home] and [[also a rogue wiki link]]",
            LinkAction.External,
            LinkAction.OpenWiki
        )]
        [TestCase(
            "#lobby or #osu would be blue (and work) in the ChatDisplay test (when a proper ChatOverlay is present)."
        ...
        [TestCase("Join my [multiplayer game](osu://room/12346).", LinkAction.JoinRoom)]
        [TestCase(
            "Join my multiplayer game http://dev.ppy.sh/multiplayer/rooms/12346",
            LinkAction.JoinRoom
        )]
        [TestCase(
            "Join my [multiplayer game](http://dev.ppy.sh/multiplayer/rooms/12346).",
        ...
        [TestCase(
            "is now listening to [https://dev.ppy.sh/s/93523 IMAGE -MATERIAL- <Version 0>]",
            true,
            false,
            LinkAction.OpenBeatmapSet
        )]
        [TestCase(
            "is now playing [https://dev.ppy.sh/b/252238 IMAGE -MATERIAL- <Version 0>]",
            true,
            false,
            LinkAction.OpenBeatmap
        )]
        ...
        [TestCase(
            "likes to post this [https://dev.ppy.sh/home link].",
            true,
            true,
            LinkAction.External
        )]
        public void TestActionAndImportantLinks(
            string text,
            bool isAction,
            bool isImportant,
            params LinkAction[] expectedActions
        )
        {
        ...
        )
        {
            ...
            AddAssert(
                "msg has the right action",
                () => newLine.Message.Links.Select(l => l.Action),
                () => Is.EqualTo(expectedActions)
            );
            ...
            bool isShowingLinks()
            {
                ...
                Color4 textColour =
                    isAction && hasBackground
                        ? Color4Extensions.FromHex(newLine.Message.Sender.Colour)
                        : Color4.White;

                var linkCompilers = newLine
                    .DrawableContentFlow.Where(d => d is DrawableLinkCompiler)
                    .ToList();
                var linkSprites = linkCompilers.SelectMany(comp =>
                    ((DrawableLinkCompiler)comp).Parts
                );

                return linkSprites.All(d => d.Colour == linkColour)
                    && newLine
                        .DrawableContentFlow.Except(linkSprites.Concat(linkCompilers))
                        .All(d => d.Colour == textColour)
        ...
        public void TestEcho()
        {
            ...
            addEchoWithWait(
                "(long loading times! clickable while loading?)[https://dev.ppy.sh/home]",
                null,
                5000
            );
            ...
            void addEchoWithWait(string text, string? completeText = null, double delay = 250)
            {
                ...
                AddStep(
                    $"send msg #{index} after {delay}ms",
                    () =>
                    {
                        ChatLine newLine = new ChatLine(new DummyEchoMessage(text));
                        textContainer.Add(newLine);
                        Scheduler.AddDelayed(
                            () => newLine.Message = new DummyMessage(completeText ?? text),
                            delay
                        );
                    }
                );
                ...
                AddUntilStep(
                    $"wait for msg #{index}",
                    () => textContainer.All(line => line.Message is DummyMessage)
                );
            }
        }
