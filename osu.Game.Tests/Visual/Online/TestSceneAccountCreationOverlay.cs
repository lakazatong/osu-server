    ...
    public partial class TestSceneAccountCreationOverlay : OsuTestScene
    {
        ...
        private void load()
        {
            ...
            localUser.BindValueChanged(
                user =>
                {
                    userPanelArea.Child = new UserGridPanel(user.NewValue) { Width = 200 };
                },
                true
            );
        }
        ...
        public void TestOverlayVisibility()
        {
            ...
            AddUntilStep(
                "overlay is visible",
                () => accountCreation.State.Value == Visibility.Visible
            );

            AddStep(
                "click button",
                () => accountCreation.ChildrenOfType<SettingsButton>().Single().TriggerClick()
            );
            AddUntilStep(
                "warning screen is present",
                () =>
                    accountCreation.ChildrenOfType<ScreenWarning>().SingleOrDefault()?.IsPresent
                    == true
            );
            ...
            AddUntilStep(
                "overlay is hidden",
                () => accountCreation.State.Value == Visibility.Hidden
            );
        }
        ...
        public void TestFullFlow()
        {
            ...
            AddStep(
                ...
                () =>
                    accountCreation
                        .ChildrenOfType<DangerousSettingsButton>()
                        .Single()
                        .TriggerClick()
            );
            AddUntilStep(
                "entry screen is present",
                () =>
                    accountCreation.ChildrenOfType<ScreenEntry>().SingleOrDefault()?.IsPresent
                    == true
            );
            ...
            AddStep(
                ...
                () =>
                {
                    ...
                    entryScreen.ChildrenOfType<OsuTextBox>().ElementAt(1).Text =
                        "new.user@fake.mail";
            ...
            AddStep(
                ...
                () =>
                    accountCreation
                        .ChildrenOfType<ScreenEntry>()
                        .Single()
                        .ChildrenOfType<SettingsButton>()
                        .Single()
                        .TriggerClick()
            );
            AddUntilStep(
                "verification screen is present",
                () =>
                    accountCreation
                        .ChildrenOfType<ScreenEmailVerification>()
                        .SingleOrDefault()
                        ?.IsPresent == true
            );
