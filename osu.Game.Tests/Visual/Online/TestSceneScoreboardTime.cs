...
namespace osu.Game.Tests.Visual.Online
{
    public partial class TestSceneScoreboardTime : OsuTestScene
    {
        ...
        {
            AddStep(
                "create various scoreboard times",
                () =>
                    Child = new FillFlowContainer
                    {
                        ...
                        ChildrenEnumerable = testCases.Select(dateTime =>
                            new ScoreboardTime(dateTime, 24).With(time =>
                                time.Anchor = time.Origin = Anchor.TopCentre
                            )
                        ),
                    }
            );
        ...
        private static IEnumerable<DateTimeOffset> testCases =>
            new[]
            {
                DateTimeOffset.Now,
