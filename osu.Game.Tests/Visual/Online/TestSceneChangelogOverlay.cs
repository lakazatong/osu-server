...
namespace osu.Game.Tests.Visual.Online
{
    ...
    public partial class TestSceneChangelogOverlay : OsuTestScene
    {
        ...
        {
            streams = APIUpdateStream
                .KNOWN_STREAMS.Keys.Select(
                    (stream, id) =>
                        new APIUpdateStream
                        {
                            Id = id + 1,
            ...
            builds = APIUpdateStream
                .KNOWN_STREAMS.Keys.Select(stream => new APIChangelogBuild
                {
                    Version = version,
        ...
        public void ShowWithBuild(bool isSupporter)
        {
                ...
                new APIChangelogBuild
                {
                    ...
                    ChangelogEntries = new List<APIChangelogEntry>
                    {
                        ...
                        new APIChangelogEntry
                        {
                            ...
                            GithubUser = new APIChangelogUser { DisplayName = "some dude" },
                        },
                        new APIChangelogEntry
                        {
                            Type = ChangelogEntryType.Misc,
            ...
            AddUntilStep(@"wait for streams", () => changelog.Streams?.Count > 0);
            AddAssert(
                @"correct build displayed",
                () => changelog.Current.Value.Version == "2018.712.0"
            );
            AddAssert(
                @"correct stream selected",
                () => changelog.Header.Streams.Current.Value.Id == 5
            );
            AddUntilStep(
                @"wait for content load",
                () => changelog.ChildrenOfType<ChangelogSupporterPromo>().Any()
            );
            AddAssert(
                @"supporter promo showed",
                () =>
                    changelog.ChildrenOfType<ChangelogSupporterPromo>().First().Alpha
                    == (isSupporter ? 0 : 1)
            );
        }
        ...
        public void TestHTMLUnescaping()
        {
            showBuild(() =>
                new APIChangelogBuild
                {
                    ...
                    UpdateStream = new APIUpdateStream { Name = "Test", DisplayName = "Test" },
                    ChangelogEntries = new List<APIChangelogEntry>
                    {
                        new APIChangelogEntry
                        {
                            ...
                            MessageHtml =
                                "&quot;&quot;&quot;This text should appear triple-quoted&quot;&quot;&quot;    &gt;_&lt;",
                            GithubUser = new APIChangelogUser
                            {
                                DisplayName = "Dummy",
                                OsuUsername = "Dummy",
