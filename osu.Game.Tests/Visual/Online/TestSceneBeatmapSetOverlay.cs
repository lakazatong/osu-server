...
namespace osu.Game.Tests.Visual.Online
{
    public partial class TestSceneBeatmapSetOverlay : OsuManualInputManagerTestScene
    {
        ...
        public void TestLocalBeatmaps()
        {
            ...
            AddStep(
                "show many difficulties",
                () => overlay.ShowBeatmapSet(createManyDifficultiesBeatmapSet())
            );
            downloadAssert(true);
            ...
            AddAssert(
                "status is loved",
                () =>
                    overlay.ChildrenOfType<BeatmapSetOnlineStatusPill>().Single().Status
                    == BeatmapOnlineStatus.Loved
            );
            AddAssert(
                "scores container is visible",
                () => overlay.ChildrenOfType<ScoresContainer>().Single().Alpha == 1
            );
            AddAssert(
                "mod selector is visible",
                () => overlay.ChildrenOfType<LeaderboardModSelector>().Single().Alpha == 1
            );

            AddStep(
                "go to second beatmap",
                () =>
                    overlay
                        .ChildrenOfType<BeatmapPicker.DifficultySelectorButton>()
                        .ElementAt(1)
                        .TriggerClick()
            );
            ...
            AddAssert(
                "scores container is hidden",
                () => overlay.ChildrenOfType<ScoresContainer>().Single().Alpha == 0
            );
        }
        ...
        {
            AddStep(
                "show multiple rulesets beatmap",
                () =>
                {
            ...
            AddAssert(
                "shown beatmaps of current ruleset",
                () =>
                    overlay.Header.HeaderContent.Picker.Difficulties.All(b =>
                        b.Beatmap.Ruleset.OnlineID
                        == overlay.Header.RulesetSelector.Current.Value.OnlineID
                    )
            );
            AddAssert(
                "left-most beatmap selected",
                () =>
                    overlay.Header.HeaderContent.Picker.Difficulties.First().State
                    == BeatmapPicker.DifficultySelectorState.Selected
            );
        }
        ...
        public void TestBeatmapSetHasVideoOrStoryboard()
        {
            ...
            AddStep(
                "show beatmapset with storyboard",
                () =>
                {
                    var beatmapSet = getBeatmapSet();
            ...
            AddStep(
                "show beatmapset with video and storyboard",
                () =>
                {
                    var beatmapSet = getBeatmapSet();
                    beatmapSet.HasVideo = true;
        ...
        public void TestSelectedModsDontAffectStatistics()
        {
            ...
            AddAssert(
                ...
                () =>
                    overlay
                        .ChildrenOfType<AdvancedStats.StatisticRow>()
                        .Single(s => s.Title == BeatmapsetsStrings.ShowStatsAr)
                        .Value,
                () => Is.EqualTo((0, 0))
            );
            AddStep(
                "set AR10 diff adjust",
                () =>
                    SelectedMods.Value = new[]
                    {
                        new OsuModDifficultyAdjust { ApproachRate = { Value = 10 } },
                    }
            );
            AddAssert(
                "AR still displayed as 0",
                () =>
                    overlay
                        .ChildrenOfType<AdvancedStats.StatisticRow>()
                        .Single(s => s.Title == BeatmapsetsStrings.ShowStatsAr)
        ...
        public void TestBeatmapSetWithGuestDifficulty()
        {
            AddStep(
                "show map",
                () => overlay.ShowBeatmapSet(createBeatmapSetWithGuestDifficulty())
            );
            AddStep(
                "move mouse to host difficulty",
                () =>
                {
            ...
            AddAssert(
                "guest mapper information not shown",
                () =>
                    overlay
                        .ChildrenOfType<BeatmapPicker>()
                        .Single()
                        .ChildrenOfType<OsuSpriteText>()
                        .All(s => s.Text != "BanchoBot0")
            );
            AddStep(
                "move mouse to guest difficulty",
                () =>
                {
            ...
            AddAssert(
                "guest mapper information shown",
                () =>
                    overlay
                        .ChildrenOfType<BeatmapPicker>()
                        .Single()
                        .ChildrenOfType<OsuSpriteText>()
                        .Any(s => s.Text == "BanchoBot0")
            );
        }
        ...
        {
            AddStep(
                "show map with 2 mapper",
                () => overlay.ShowBeatmapSet(createBeatmapSetWithGuestDifficulty(2))
            );
            AddStep(
                "move mouse to guest difficulty",
                () =>
            ...
            AddStep(
                "show map with 3 mapper",
                () => overlay.ShowBeatmapSet(createBeatmapSetWithGuestDifficulty(3))
            );
            AddStep(
                "move mouse to guest difficulty",
                () =>
            ...
            AddStep(
                "show map with 10 mapper",
                () => overlay.ShowBeatmapSet(createBeatmapSetWithGuestDifficulty(10))
            );
            AddStep(
                "move mouse to guest difficulty",
                () =>
            ...
            AddStep(
                "show map with 20 mapper",
                () => overlay.ShowBeatmapSet(createBeatmapSetWithGuestDifficulty(20))
            );
            AddStep(
                "move mouse to guest difficulty",
                () =>
            ...
            for (int i = 1; i < 41; i++)
            {
                beatmaps.Add(
                    new APIBeatmap
                    {
                        ...
                        Status =
                            i % 2 == 0 ? BeatmapOnlineStatus.Graveyard : BeatmapOnlineStatus.Loved,
                    }
                );
            }
        ...
        private APIBeatmapSet createBeatmapSetWithGuestDifficulty(int guestCount = 1)
        {
            ...
            {
                var guestUser = new APIUser { Username = @$"BanchoBot{i}", Id = i + 3 };

                beatmapOwners.Add(
                    new APIBeatmap.BeatmapOwner { Username = @$"BanchoBot{i}", Id = i + 3 }
                );
        ...
        {
            AddAssert(
                $"is download button {(shown ? "shown" : "hidden")}",
                () => overlay.Header.HeaderContent.DownloadButtonsVisible == shown
            );
        }
