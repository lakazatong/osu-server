...
namespace osu.Game.Tests.Visual.Online
{
    public partial class TestSceneOfflineCommentsContainer : OsuTestScene
    {
        ...
        public void TestAppendDuplicatedComment()
        {
            ...
            AddStep(
                "Append existing comment",
                () => comments.AppendComments(getCommentSubBundle())
            );
        ...
        public void TestLocalCommentBundle()
        {
            AddStep("Add comment bundle", () => comments.ShowComments(getCommentBundle()));
            AddStep(
                "Add empty comment bundle",
                () => comments.ShowComments(getEmptyCommentBundle())
            );
        }

        private CommentBundle getEmptyCommentBundle() =>
            new CommentBundle { Comments = new List<Comment>() };

        private CommentBundle getCommentBundle() =>
            new CommentBundle
            {
                Comments = new List<Comment>
                {
                    ...
                    new Comment
                    {
                        Id = 100,
                        Message =
                            "This comment has \"load replies\" button because it has unloaded replies",
                    ...
                    new Comment
                    {
                        Id = 111,
                        Message =
                            "This comment has \"Show More\" button because it has unloaded replies, but some of them are loaded",
                ...
                UserVotes = new List<long> { 5 },
                Users = new List<APIUser>
                {
                    new APIUser { Id = 1, Username = "Good_Admin" },
                },
                Total = 10,
            };

        private CommentBundle getCommentSubBundle() =>
            new CommentBundle
            {
                Comments = new List<Comment>
                {
        ...
        {
            public new void AppendComments([NotNull] CommentBundle bundle) =>
                base.AppendComments(bundle);
