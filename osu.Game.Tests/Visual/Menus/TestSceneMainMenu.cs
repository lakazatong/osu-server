...
namespace osu.Game.Tests.Visual.Menus
{
    public partial class TestSceneMainMenu : OsuGameTestScene
    {
        private OnlineMenuBanner onlineMenuBanner =>
            Game.ChildrenOfType<OnlineMenuBanner>().Single();

        public override void SetUpSteps()
        {
            base.SetUpSteps();
            AddStep(
                "don't fetch online content",
                () => onlineMenuBanner.FetchOnlineContent = false
            );
            AddStep(
                "disable return to top on idle",
                () => Game.ChildrenOfType<ButtonSystem>().Single().ReturnToTopOnIdle = false
            );
        }
        ...
        public void TestDailyChallenge()
        {
                            ...
                            case GetRoomRequest getRoomRequest:
                                ...
                                getRoomRequest.TriggerSuccess(
                                    new Room
                                    {
                                        RoomID = 1234,
                                        Name = "Aug 8, 2024",
                                        Playlist = [new PlaylistItem(beatmap)],
            ...
            AddStep(
                "beatmap of the day active",
                () =>
                    Game.ChildrenOfType<IMetadataClient>()
                        .Single()
                        .DailyChallengeUpdated(new DailyChallengeInfo { RoomID = 1234 })
            );
            ...
            AddUntilStep(
                "wait for daily challenge screen",
                () => Game.ScreenStack.CurrentScreen,
                Is.TypeOf<Screens.OnlinePlay.DailyChallenge.DailyChallenge>
            );
        }
        ...
        public void TestOnlineMenuBannerTrusted()
        {
            AddStep(
                "set online content",
                () =>
                    onlineMenuBanner.Current.Value = new APIMenuContent
                    {
                        Images = new[]
                        {
                            new APIMenuImage
                            {
                                ...
                                Url =
                                    $@"{API.Endpoints.WebsiteUrl}/home/news/2023-12-21-project-loved-december-2023",
                            },
                        },
                    }
            );
            AddAssert(
                "system title not visible",
                () => onlineMenuBanner.State.Value,
                () => Is.EqualTo(Visibility.Hidden)
            );
            AddStep("enter menu", () => InputManager.Key(Key.Enter));
            AddUntilStep(
                "system title visible",
                () => onlineMenuBanner.State.Value,
            ...
            AddAssert(
                "no dialog",
                () => Game.ChildrenOfType<DialogOverlay>().SingleOrDefault()?.CurrentDialog == null
            );
        }
