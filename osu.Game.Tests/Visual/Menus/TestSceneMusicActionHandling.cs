...
namespace osu.Game.Tests.Visual.Menus
{
    public partial class TestSceneMusicActionHandling : OsuGameTestScene
    {
        ...
        {
            AddStep(
                "ensure playing something",
                () => Game.MusicController.EnsurePlayingSomething()
            );
            ...
            AddUntilStep(
                "music paused",
                () => !Game.MusicController.IsPlaying && Game.MusicController.UserPauseRequested
            );
            AddStep(
                "toggle playback",
                () => globalActionContainer.TriggerPressed(GlobalAction.MusicPlay)
        ...
        public void TestMusicNavigationActions()
        {
            ...
            AddStep(
                "import beatmap with track",
                () =>
                {
                    var setWithTrack = Game
                        .BeatmapManager.Import(
                            new ImportTask(TestResources.GetTestBeatmapForImport())
                        )
                        .GetResultSafely();
            ...
            AddStep(
                "bind to track change",
                () =>
                {
                    trackChangeQueue = new Queue<(IWorkingBeatmap, TrackChangeDirection)>();
                    Game.MusicController.TrackChanged += (working, changeDirection) =>
                        trackChangeQueue.Enqueue((working, changeDirection));
                }
            );

            AddStep("seek track to 6 second", () => Game.MusicController.SeekTo(6000));
            AddUntilStep(
                "wait for current time to update",
                () => Game.MusicController.CurrentTrack.CurrentTime > 5000
            );

            AddStep(
                "press previous",
                () => globalActionContainer.TriggerPressed(GlobalAction.MusicPrev)
            );
            ...
            AddStep(
                "press next",
                () => globalActionContainer.TriggerPressed(GlobalAction.MusicNext)
            );
            AddUntilStep(
                "track changed to next",
                () =>
                    trackChangeQueue.Count == 1
            ...
            AddAssert(
                ...
                () =>
                    !trackChangeQueue
                        .First()
                        .working.BeatmapInfo.Equals(trackChangeQueue.Last().working.BeatmapInfo)
            );
        }
