...
namespace osu.Game.Tests.Visual.SongSelectV2
{
    public partial class TestSceneCollectionDropdown : OsuManualInputManagerTestScene
    {
        ...
        public void TestCollectionAddedToDropdown()
        {
            ...
            AddStep(
                "add collection",
                () => writeAndRefresh(r => r.Add(new BeatmapCollection(name: "2")))
            );
        ...
        public void TestCollectionsCleared()
        {
            ...
            AddStep(
                "add collection",
                () => writeAndRefresh(r => r.Add(new BeatmapCollection(name: "3")))
            );
        ...
        public void TestCollectionRemovedFromDropdown()
        {
            ...
            AddStep(
                "add collection",
                () => writeAndRefresh(r => r.Add(first = new BeatmapCollection(name: "1")))
            );
            AddStep(
                "add collection",
                () => writeAndRefresh(r => r.Add(new BeatmapCollection(name: "2")))
        ...
        public void TestAllBeatmapFilterDoesNotHaveAddButton()
        {
            ...
            AddAssert(
                "'All beatmaps' filter does not have add button",
                () => !getAddOrRemoveButton(0).IsPresent
            );
        }
        ...
        public void TestButtonDisabledAndEnabledWithBeatmapChanges()
        {
            ...
            AddStep(
                "select available beatmap",
                () =>
                    Beatmap.Value = beatmapManager.GetWorkingBeatmap(
                        beatmapManager.GetAllUsableBeatmapSets().First().Beatmaps[0]
                    )
            );
        ...
        public void TestButtonChangesWhenAddedAndRemovedFromCollection()
        {
            ...
            AddStep(
                ...
                () =>
                    writeAndRefresh(r =>
                        getFirstCollection().BeatmapMD5Hashes.Add(Beatmap.Value.BeatmapInfo.MD5Hash)
                    )
            );
            assertFirstButtonIs(FontAwesome.Solid.MinusSquare);

            AddStep(
                "remove beatmap from collection",
                () => writeAndRefresh(r => getFirstCollection().BeatmapMD5Hashes.Clear())
            );
            assertFirstButtonIs(FontAwesome.Solid.PlusSquare);
        }
        ...
        public void TestButtonAddsAndRemovesBeatmap()
        {
            ...
            AddAssert(
                "collection contains beatmap",
                () =>
                    getFirstCollection()
                        .BeatmapMD5Hashes.Contains(Beatmap.Value.BeatmapInfo.MD5Hash)
            );
            assertFirstButtonIs(FontAwesome.Solid.MinusSquare);

            addClickAddOrRemoveButtonStep(1);
            AddAssert(
                "collection does not contain beatmap",
                () =>
                    !getFirstCollection()
                        .BeatmapMD5Hashes.Contains(Beatmap.Value.BeatmapInfo.MD5Hash)
            );
        ...
        public void TestManageCollectionsFilterIsNotSelected()
        {
            ...
            AddStep(
                ...
                () =>
                    writeAndRefresh(r =>
                        r.Add(new BeatmapCollection(name: "1", new List<string> { "abc" }))
                    )
            );
            assertCollectionDropdownContains("1");
            ...
            AddStep(
                "watch for filter requests",
                () =>
                {
                    received = false;
                    dropdown.ChildrenOfType<CollectionDropdown>().First().RequestFilter = () =>
                        received = true;
                }
            );

            AddStep(
                "click manage collections filter",
                () =>
                {
                    int lastItemIndex =
                        dropdown.ChildrenOfType<CollectionDropdown>().Single().Items.Count() - 1;
            ...
            AddAssert(
                "collection filter still selected",
                () => dropdown.Current.Value.CollectionName == "1"
            );

            AddAssert("filter request not fired", () => !received);
        }

        private void writeAndRefresh(Action<Realm> action) =>
            Realm.Write(r =>
            {
                action(r);
                r.Refresh();
        ...
        private BeatmapCollection getFirstCollection() =>
            Realm.Run(r => r.All<BeatmapCollection>().First());

        private void assertCollectionHeaderDisplays(
            string collectionName,
            bool shouldDisplay = true
        ) =>
            AddUntilStep(
                $"collection dropdown header displays '{collectionName}'",
                () =>
                    shouldDisplay
                    == dropdown
                        .ChildrenOfType<CollectionDropdown.ShearedDropdownHeader>()
                        .Any(h => h.ChildrenOfType<SpriteText>().Any(t => t.Text == collectionName))
            );

        private void assertFirstButtonIs(IconUsage icon) =>
            AddUntilStep(
                $"button is {icon.Icon.ToString()}",
                () => getAddOrRemoveButton(1).Icon.Equals(icon)
            );

        private void assertCollectionDropdownContains(
            string collectionName,
            bool shouldContain = true
        ) =>
            AddUntilStep(
                $"collection dropdown {(shouldContain ? "contains" : "does not contain")} '{collectionName}'",
                ...
                () =>
                    ...
                    == dropdown
                        .ChildrenOfType<Menu.DrawableMenuItem>()
                        .Any(i =>
                            i.ChildrenOfType<CompositeDrawable>().OfType<IHasText>().First().Text
                            == collectionName
                        )
            );
        ...
        private void addExpandHeaderStep() =>
            AddStep(
                "expand header",
                () =>
                {
                    InputManager.MoveMouseTo(
                        dropdown.ChildrenOfType<CollectionDropdown.ShearedDropdownHeader>().Single()
                    );
                    InputManager.Click(MouseButton.Left);
                }
            );
        ...
        private Menu.DrawableMenuItem getCollectionDropdownItemAt(int index)
        {
            ...
            CollectionFilterMenuItem item = dropdown
                .ChildrenOfType<CollectionDropdown>()
                .Single()
                .ItemSource.ElementAt(index);
            return dropdown
                .ChildrenOfType<Menu.DrawableMenuItem>()
                .Single(i => i.Item.Text.Value == item.CollectionName);
        }
    }
}
