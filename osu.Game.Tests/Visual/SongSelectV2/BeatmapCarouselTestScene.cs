...
namespace osu.Game.Tests.Visual.SongSelectV2
{
    ...
    {
        protected readonly Stack<BeatmapSetInfo> BeatmapSetRequestedSelections =
            new Stack<BeatmapSetInfo>();
        protected readonly Stack<BeatmapInfo> BeatmapRequestedSelections = new Stack<BeatmapInfo>();

        protected readonly BindableList<BeatmapSetInfo> BeatmapSets =
            new BindableList<BeatmapSetInfo>();

        protected TestBeatmapCarousel Carousel = null!;

        protected OsuScrollContainer<Drawable> Scroll =>
            Carousel.ChildrenOfType<OsuScrollContainer<Drawable>>().Single();
        ...
        public Func<
            IEnumerable<BeatmapInfo>,
            BeatmapInfo
        >? BeatmapRecommendationFunction { get; set; }
        ...
        {
            store = new TestBeatmapStore { BeatmapSets = { BindTarget = BeatmapSets } };

            BeatmapSets.BindCollectionChanged(
                (_, _) => beatmapCount = BeatmapSets.Sum(s => s.Beatmaps.Count)
            );
                ...
                () =>
                {
                    ...
                    Children = new Drawable[]
                    {
                        new GridContainer
                        {
                            ...
                            ColumnDimensions = new[] { new Dimension(GridSizeMode.Relative, 1) },
                            RowDimensions = new[]
                            {
                            ...
                            Content = new[]
                            {
                                ...
                                new Drawable[]
                                {
                                    Carousel = new TestBeatmapCarousel
                                    {
                                        ...
                                        {
                                            BeatmapSetRequestedSelections.Push(
                                                beatmaps.First().BeatmapSet!
                                            );
                                            Carousel.CurrentSelection =
                                                BeatmapRecommendationFunction?.Invoke(beatmaps)
                                                ?? beatmaps.First();
                                        },
                                        BleedTop = 50,
                                        BleedBottom = 50,
        ...
        protected void SortBy(SortMode mode) =>
            ApplyToFilterAndWaitForFilter(
                $"sort by {mode.GetDescription().ToLowerInvariant()}",
                c => c.Sort = mode
            );

        protected void GroupBy(GroupMode mode) =>
            ApplyToFilterAndWaitForFilter(
                $"group by {mode.GetDescription().ToLowerInvariant()}",
                c => c.Group = mode
            );
        ...
        {
            ApplyToFilterAndWaitForFilter(
                $"sort by {sort.GetDescription().ToLowerInvariant()} & group by {group.GetDescription().ToLowerInvariant()}",
                c =>
                {
                    c.Sort = sort;
                    c.Group = group;
        ...
        protected void ApplyToFilterAndWaitForFilter(
            string description,
            Action<FilterCriteria>? apply
        )
        {
            AddStep(
                description,
        ...
        protected void WaitForDrawablePanels() =>
            AddUntilStep(
                "drawable panels loaded",
                () => Carousel.ChildrenOfType<ICarouselPanel>().Count(),
                () => Is.GreaterThan(0)
            );
        ...
        protected void WaitForScrolling() =>
            AddUntilStep("scroll finished", () => Scroll.Current, () => Is.EqualTo(Scroll.Target));

        protected void ToggleGroupCollapse() =>
            AddStep(
        ...
        protected void SelectPrevGroup() =>
            AddStep(
                "select prev group",
                () =>
                {
        ...
        protected void SelectNextPanel() =>
            AddStep("select next panel", () => InputManager.Key(Key.Down));

        protected void SelectPrevPanel() =>
            AddStep("select prev panel", () => InputManager.Key(Key.Up));

        protected void SelectNextSet() =>
            AddStep("select next set", () => InputManager.Key(Key.Right));

        protected void SelectPrevSet() =>
            AddStep("select prev set", () => InputManager.Key(Key.Left));

        protected void Select() => AddStep("select", () => InputManager.Key(Key.Enter));

        protected void CheckNoSelection() =>
            AddAssert("has no selection", () => Carousel.CurrentSelection, () => Is.Null);

        protected void CheckHasSelection() =>
            AddAssert("has selection", () => Carousel.CurrentSelection, () => Is.Not.Null);

        protected void CheckRequestPresentCount(int expected) =>
            AddAssert(
                $"check present count is {expected}",
                () => Carousel.RequestPresentBeatmapCount,
                () => Is.EqualTo(expected)
            );

        protected void CheckActivationCount(int expected) =>
            AddAssert(
                $"check activation count is {expected}",
                () => Carousel.ActivationCount,
                () => Is.EqualTo(expected)
            );
        ...
        {
            AddAssert(
                $"{expected} sets displayed",
                () =>
                {
                    var groupingFilter = Carousel
                        .Filters.OfType<BeatmapCarouselFilterGrouping>()
                        .Single();
                    ...
                    return groupingFilter.SetItems.Sum(s =>
                        s.Value.Count(i => i.Model is BeatmapSetInfo)
                    );
                },
                () => Is.EqualTo(expected)
            );
        ...
        protected ICarouselPanel? GetSelectedPanel() =>
            Carousel.ChildrenOfType<ICarouselPanel>().SingleOrDefault(p => p.Selected.Value);

        protected ICarouselPanel? GetKeyboardSelectedPanel() =>
            Carousel
                .ChildrenOfType<ICarouselPanel>()
                .SingleOrDefault(p => p.KeyboardSelected.Value);

        protected void WaitForExpandedGroup(int group)
        {
            AddUntilStep(
                $"group {group} is expanded",
                () =>
                {
                    var groupingFilter = Carousel
                        .Filters.OfType<BeatmapCarouselFilterGrouping>()
        ...
        {
            AddUntilStep(
                $"selected is group{group} panel{panel}",
                () =>
                {
                    var groupingFilter = Carousel
                        .Filters.OfType<BeatmapCarouselFilterGrouping>()
                    ...
                    GroupDefinition? groupDefinition =
                        groupingFilter.GroupItems.Keys.ElementAtOrDefault(group);

                    if (groupDefinition == null)
                        return false;

                    // offset by one because the group itself is included in the items list.
                    CarouselItem item = groupingFilter
                        .GroupItems[groupDefinition]
                        .ElementAt(panel + 1);

                    return (Carousel.CurrentSelection as BeatmapInfo)?.Equals(
                            item.Model as BeatmapInfo
                        ) == true;
                }
            );
        }
        ...
        protected void WaitForSetSelection(int set, int? diff = null)
        {
            ...
            {
                AddUntilStep(
                    $"selected is set{set} diff{diff.Value}",
            ...
            else
            {
                AddUntilStep(
                    $"selected is set{set}",
                    () => BeatmapSets[set].Beatmaps.Contains(Carousel.CurrentSelection)
                );
            }
        }
        ...
        {
            AddStep(
                $"move mouse to panel {index} with offset {positionOffsetFromCentre}",
                () =>
                {
                    var panel = Carousel
                        .ChildrenOfType<UserTrackingScrollContainer>()
                    ...
                    InputManager.MoveMouseTo(
                        panel.ScreenSpaceDrawQuad.Centre
                            + panel.ToScreenSpace(positionOffsetFromCentre)
                            - panel.ToScreenSpace(Vector2.Zero)
                    );
                }
            );
        ...
        protected void AddBeatmaps(
            int count,
            int? fixedDifficultiesPerSet = null,
            bool randomMetadata = false
        ) =>
            AddStep(
                $"add {count} beatmaps{(randomMetadata ? " with random data" : "")}",
                () =>
                {
                    ...
                    for (int i = 0; i < count; i++)
                        beatmaps.Add(
                            CreateTestBeatmapSetInfo(fixedDifficultiesPerSet, randomMetadata)
                        );
        ...
        protected static BeatmapSetInfo CreateTestBeatmapSetInfo(
            int? fixedDifficultiesPerSet,
            bool randomMetadata
        )
        {
            var beatmapSetInfo = TestResources.CreateTestBeatmapSetInfo(
                fixedDifficultiesPerSet ?? RNG.Next(1, 4)
            );
            ...
            if (randomMetadata)
            {
                ...
                var metadata = new BeatmapMetadata
                {
                    ...
                    Title =
                        $"{randomCharacter}ome Song (set id {beatmapSetInfo.OnlineID:000}) {Guid.NewGuid()}",
        ...
        protected void RemoveAllBeatmaps() =>
            AddStep("clear all beatmaps", () => BeatmapSets.Clear());

        protected void RemoveFirstBeatmap() =>
            AddStep(
                "remove first beatmap",
                () =>
                {
        ...
        public partial class TestBeatmapCarousel : BeatmapCarousel
        {
            ...
            protected override async Task<IEnumerable<CarouselItem>> FilterAsync(
                bool clearExistingPanels = false
            )
            {
