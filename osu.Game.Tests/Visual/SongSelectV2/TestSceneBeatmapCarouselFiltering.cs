...
namespace osu.Game.Tests.Visual.SongSelectV2
{
    ...
    public partial class TestSceneBeatmapCarouselFiltering : BeatmapCarouselTestScene
    {
        ...
        public void TestCarouselRemembersSelection()
        {
            ...
            AddStep(
                "record selection",
                () => selectedID = ((BeatmapInfo)Carousel.CurrentSelection!).ID
            );
            ...
            for (int i = 0; i < 5; i++)
            {
                ApplyToFilterAndWaitForFilter(
                    "filter all",
                    c => c.SearchText = Guid.NewGuid().ToString()
                );
                AddAssert(
                    "selection not changed",
                    () => ((BeatmapInfo)Carousel.CurrentSelection!).ID == selectedID
                );
                ApplyToFilterAndWaitForFilter("remove filter", c => c.SearchText = string.Empty);
                AddAssert(
                    "selection not changed",
        ...
        public void TestCarouselChangesSelectionOnSingleMatch_FromSelection()
        {
            ...
            ApplyToFilterAndWaitForFilter(
                "filter all but one",
                c => c.SearchText = BeatmapSets.First().Metadata.Title
            );
        ...
        public void TestCarouselRetainsSelectionFromDifficultySort()
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                AddStep(
                    $"select diff {diff}",
                    () => Carousel.CurrentSelection = chosenBeatmap = BeatmapSets[20].Beatmaps[diff]
                );
                AddUntilStep(
                    "selection changed",
        ...
        {
            ApplyToFilterAndWaitForFilter(
                "allow converted beatmaps, filter to osu",
                c =>
                {
                ...
                () =>
                {
                    ...
                    return visibleBeatmapPanels.Count() == 1
                        && visibleBeatmapPanels.Count(p =>
                            ((BeatmapInfo)p.Item!.Model).Ruleset.OnlineID == 0
                        ) == 1;
                }
            );
            ...
            ApplyToFilterAndWaitForFilter(
                "filter to taiko",
                c => c.Ruleset = rulesets.AvailableRulesets.ElementAt(1)
            );
            ...
            AddUntilStep(
                ...
                () =>
                {
                    ...
                    return visibleBeatmapPanels.Count() == 2
                        ...
                        && visibleBeatmapPanels.Count(p =>
                            ((BeatmapInfo)p.Item!.Model).Ruleset.OnlineID == 1
                        ) == 1;
                }
            );
            ...
            ApplyToFilterAndWaitForFilter(
                "filter to catch",
                c => c.Ruleset = rulesets.AvailableRulesets.ElementAt(2)
            );
            ...
            AddUntilStep(
                ...
                () =>
                {
                    ...
                    return visibleBeatmapPanels.Count() == 2
                        ...
                        && visibleBeatmapPanels.Count(p =>
                            ((BeatmapInfo)p.Item!.Model).Ruleset.OnlineID == 2
                        ) == 1;
                }
            );
        }
        ...
        public void TestNavigateFromFilteredItem_SelectNextGroup_OnlyOnePanelAvailable()
        {
            ...
            ApplyToFilterAndWaitForFilter(
                "filter first set away",
                c => c.SearchText = BeatmapSets.Last().Metadata.Title
            );
        ...
        public void TestNavigateFromFilteredItem_SelectNextPanel()
        {
            ...
            SelectNextPanel();
            AddAssert(
                "keyboard selected is first set",
        ...
        public void TestNavigateFromFilteredItem_SelectNextPanel_OnlyOnePanelAvailable()
        {
            ...
            SelectPrevPanel();
            AddAssert(
                "keyboard selected is second set",
                () => GetKeyboardSelectedPanel()?.Item?.Model,
                () => Is.EqualTo(BeatmapSets.Last())
            );
        }
