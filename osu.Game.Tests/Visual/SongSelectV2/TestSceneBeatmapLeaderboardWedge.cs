...
namespace osu.Game.Tests.Visual.SongSelectV2
{
    public partial class TestSceneBeatmapLeaderboardWedge : SongSelectComponentsTestScene
    {
        ...
        private void load()
        {
            LoadComponent(dialogOverlay = new DialogOverlay { Depth = -1 });
            ...
            Child = contentContainer = new OsuContextMenuContainer
            {
                ...
                Height = 500,
                Children = new Drawable[] { dialogOverlay },
            };
            ...
            AddStep(
                ...
                () =>
                    leaderboard.SetScores(
                        TestSceneBeatmapLeaderboard.GenerateSampleScores(new BeatmapInfo())
                    )
            );
            AddStep(
        ...
        {
            AddStep(
                "ensure no scores displayed",
                () => leaderboard.SetScores(Array.Empty<ScoreInfo>())
            );

            AddStep(@"Retrieving", () => leaderboard.SetState(LeaderboardState.Retrieving));
            AddStep(
                @"Network failure",
                () => leaderboard.SetState(LeaderboardState.NetworkFailure)
            );
            ...
            AddStep(
                @"Ruleset unavailable",
                () => leaderboard.SetState(LeaderboardState.RulesetUnavailable)
            );
            AddStep(
                @"Beatmap unavailable",
                () => leaderboard.SetState(LeaderboardState.BeatmapUnavailable)
            );
        ...
        public void TestUseTheseModsDoesNotCopySystemMods()
        {
            ...
            AddUntilStep(
                "wait for scores",
                () => this.ChildrenOfType<BeatmapLeaderboardScore>().Count(),
                () => Is.GreaterThan(0)
            );
            AddStep(
                "right click panel",
            ...
            AddAssert(
                "received HD",
                () =>
                    this.ChildrenOfType<BeatmapLeaderboardScore>()
                        .Last()
                        .SelectedMods.Value.Any(m => m is OsuModHidden)
            );
            AddAssert(
                "did not receive SV2",
                () =>
                    !this.ChildrenOfType<BeatmapLeaderboardScore>()
                        .Last()
                        .SelectedMods.Value.Any(m => m is ModScoreV2)
            );
        }
        ...
        private void setScope(BeatmapLeaderboardScope scope)
        {
            AddStep(
                @"Set scope",
                () => ((Bindable<BeatmapLeaderboardScope>)leaderboard.Scope).Value = scope
            );
        }
        ...
        private void importMoreScores(Func<BeatmapInfo> beatmapInfo)
        {
            AddStep(
                ...
                {
                    foreach (
                        var score in TestSceneBeatmapLeaderboard.GenerateSampleScores(beatmapInfo())
                    )
        ...
        private void checkDisplayedCount(int expected) =>
            AddUntilStep(
                ...
                () => leaderboard.ChildrenOfType<BeatmapLeaderboardScore>().Count(),
                () => Is.EqualTo(expected)
            );
        ...
        private partial class TestBeatmapLeaderboardWedge : BeatmapLeaderboardWedge
        {
            ...
            public new void SetScores(
                ...
                int? totalCount = null
            ) => base.SetScores(scores, userScore, totalCount);
        }
    }
}
