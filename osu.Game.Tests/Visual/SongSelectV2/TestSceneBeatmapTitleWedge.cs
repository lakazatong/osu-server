...
namespace osu.Game.Tests.Visual.SongSelectV2
{
    public partial class TestSceneBeatmapTitleWedge : SongSelectComponentsTestScene
    {
        ...
        private BeatmapTitleWedge.DifficultyDisplay difficultyDisplay =>
            titleWedge.ChildrenOfType<BeatmapTitleWedge.DifficultyDisplay>().Single();
        ...
        protected override void LoadComplete()
        {
            ...
            AddSliderStep(
                ...
                v =>
                {
                    difficultyDisplay.ChildrenOfType<StarRatingDisplay>().Single().Current.Value =
                        new StarDifficulty(v, 0);
                }
            );
        }
        ...
        public void TestNullBeatmap()
        {
            ...
            AddAssert(
                "check default title",
                () => titleWedge.DisplayedTitle == Beatmap.Default.BeatmapInfo.Metadata.Title
            );
            AddAssert(
                "check default artist",
                () => titleWedge.DisplayedArtist == Beatmap.Default.BeatmapInfo.Metadata.Artist
            );
            AddAssert(
                "check no statistics",
                () =>
                    difficultyDisplay
                        .ChildrenOfType<BeatmapTitleWedge.DifficultyStatisticsDisplay>()
                        .All(d => !d.Statistics.Any())
            );
        }
        ...
        public void TestBPMUpdates()
        {
            ...
            IBeatmap beatmap = TestSceneBeatmapInfoWedge.CreateTestBeatmap(
        ...
        public void TestOnlineAvailability()
        {
            ...
            AddAssert(
                "play count = 10000",
                () =>
                    this.ChildrenOfType<BeatmapTitleWedge.Statistic>().ElementAt(0).Text.ToString()
                    == "10,000"
            );
            AddAssert(
                "favourites count = 2345",
                () =>
                    this.ChildrenOfType<BeatmapTitleWedge.Statistic>().ElementAt(1).Text.ToString()
                    == "2,345"
            );
            AddStep(
                "online beatmapset with local diff",
                () =>
                {
                    var (working, onlineSet) = createTestBeatmap();
        ...
        private void checkDisplayedBPM(string target)
        {
            AddUntilStep(
                ...
                () =>
                {
                    var label = titleWedge
                        .ChildrenOfType<BeatmapTitleWedge.Statistic>()
                        .Single(l => l.TooltipText == BeatmapsetsStrings.ShowStatsBpm);
        ...
        private class HeavyWorkingBeatmap : WorkingBeatmap
        {
            ...
            protected override IBeatmap GetBeatmap()
            {
                ...
                return new Beatmap { BeatmapInfo = beatmap_info, HitObjects = objects };
            }
