...
namespace osu.Game.Tests.Visual.Editing
{
    public partial class TestSceneHitObjectSampleAdjustments : EditorTestScene
    {
                        ...
                        new Slider
                        {
                            ...
                            Path = new SliderPath(
                                new[]
                                {
                                    ...
                                    new PathControlPoint(new Vector2(250, 0)),
                                }
                            ),
        ...
        public void TestHotkeysMultipleSelectionWithSameSampleBank()
        {
            ...
            AddStep(
                "Press addition normal bank shortcut",
                () =>
                {
            ...
            AddStep(
                "Press addition drum bank shortcut",
                () =>
                {
                    InputManager.PressKey(Key.AltLeft);
                    InputManager.Key(Key.R);
        ...
        public void TestHotkeysDuringPlacement()
        {
            ...
            AddStep(
                "Press soft addition bank shortcut",
                () =>
                {
                    InputManager.PressKey(Key.AltLeft);
            ...
            AddStep(
                "Press drum addition bank shortcut",
                () =>
                {
                    InputManager.PressKey(Key.AltLeft);
                    InputManager.Key(Key.R);
            ...
            AddStep(
                "Press auto addition bank shortcut",
                () =>
                {
                    InputManager.PressKey(Key.AltLeft);
                    InputManager.Key(Key.Q);
            ...
            void checkPlacementSampleBank(string expected) =>
                AddAssert(
                    $"Placement sample is {expected}",
                    () =>
                        EditorBeatmap
                            .PlacementObject.Value.Samples.First(s =>
            ...
            void checkPlacementSampleAdditionBank(string expected) =>
                AddAssert(
                    $"Placement sample addition is {expected}",
                    () =>
                        EditorBeatmap
                            .PlacementObject.Value.Samples.First(s =>
                                s.Name != HitSampleInfo.HIT_NORMAL
        ...
        private void clickSamplePiece(int objectIndex) =>
            AddStep(
                $"click {objectIndex.ToOrdinalWords()} sample piece",
                () =>
                {
                    var samplePiece = this.ChildrenOfType<SamplePointPiece>()
                        .Single(piece =>
                            piece is not NodeSamplePointPiece
        ...
        private void clickNodeSamplePiece(int objectIndex, int nodeIndex) =>
            AddStep(
                $"click {objectIndex.ToOrdinalWords()} object {nodeIndex.ToOrdinalWords()} node sample piece",
                () =>
                {
                    var samplePiece = this.ChildrenOfType<NodeSamplePointPiece>()
                        .Where(piece =>
                            piece.HitObject == EditorBeatmap.HitObjects.ElementAt(objectIndex)
                        )
                        .ToArray()[nodeIndex];
        ...
        private void seekSamplePiece(int direction) =>
            AddStep(
                $"seek sample piece {direction}",
                () =>
                {
        ...
        private void samplePopoverIsOpen() =>
            AddUntilStep(
                "sample popover is open",
                () =>
                {
                    var popover = this.ChildrenOfType<SamplePointPiece.SampleEditPopover>()
                        .SingleOrDefault(o => o.IsPresent);
                    return popover != null;
                }
            );

        private void samplePopoverHasNoFocus() =>
            AddUntilStep(
                "sample popover textbox not focused",
                () =>
                {
                    var popover = this.ChildrenOfType<SamplePointPiece.SampleEditPopover>()
                        .SingleOrDefault();
                    var slider = popover
                        ?.ChildrenOfType<IndeterminateSliderWithTextBoxInput<int>>()
                        .Single();
        ...
        private void samplePopoverHasSingleVolume(int volume) =>
            AddUntilStep(
                $"sample popover has volume {volume}",
                () =>
                {
                    var popover = this.ChildrenOfType<SamplePointPiece.SampleEditPopover>()
                        .SingleOrDefault();
        ...
        private void samplePopoverHasIndeterminateVolume() =>
            AddUntilStep(
                "sample popover has indeterminate volume",
                () =>
                {
                    var popover = this.ChildrenOfType<SamplePointPiece.SampleEditPopover>()
                        .SingleOrDefault();
        ...
        private void samplePopoverHasSingleBank(string bank) =>
            AddUntilStep(
                $"sample popover has bank {bank}",
                () =>
                {
                    var popover = this.ChildrenOfType<SamplePointPiece.SampleEditPopover>()
                        .SingleOrDefault();
                    ...
                    return textBox?.Current.Value == bank
                        && string.IsNullOrEmpty(textBox.PlaceholderText.ToString());
                }
            );

        private void samplePopoverHasIndeterminateBank() =>
            AddUntilStep(
                "sample popover has indeterminate bank",
                () =>
                {
                    var popover = this.ChildrenOfType<SamplePointPiece.SampleEditPopover>()
                        .SingleOrDefault();
                    ...
                    return textBox != null
                        && string.IsNullOrEmpty(textBox.Current.Value)
                        && !string.IsNullOrEmpty(textBox.PlaceholderText.ToString());
                }
            );
        ...
        private void dismissPopover()
        {
            ...
            AddUntilStep(
                "wait for dismiss",
                () =>
                    !this.ChildrenOfType<SamplePointPiece.SampleEditPopover>()
                        .Any(popover => popover.IsPresent)
            );
        }

        private void setVolumeViaPopover(int volume) =>
            AddStep(
                $"set volume {volume} via popover",
                () =>
                {
                    var popover = this.ChildrenOfType<SamplePointPiece.SampleEditPopover>()
                        .Single();
        ...
        private void hitObjectHasSampleVolume(int objectIndex, int volume) =>
            AddAssert(
                $"{objectIndex.ToOrdinalWords()} has volume {volume}",
                () =>
                {
        ...
        private void hitObjectNodeHasSampleVolume(int objectIndex, int nodeIndex, int volume) =>
            AddAssert(
        ...
        private void setBankViaPopover(string bank) =>
            AddStep(
                $"set bank {bank} via popover",
                () =>
                {
                    var popover = this.ChildrenOfType<SamplePointPiece.SampleEditPopover>()
                        .Single();
        ...
        private void setAdditionBankViaPopover(string bank) =>
            AddStep(
                $"set addition bank {bank} via popover",
                () =>
                {
                    var popover = this.ChildrenOfType<SamplePointPiece.SampleEditPopover>()
                        .Single();
        ...
        private void toggleAdditionViaPopover(int index) =>
            AddStep(
                $"toggle addition {index} via popover",
                () =>
                {
                    var popover = this.ChildrenOfType<SamplePointPiece.SampleEditPopover>().First();
                    var ternaryButton = popover.ChildrenOfType<DrawableTernaryButton>().ToArray()[
                        index
                    ];
        ...
        private void hitObjectHasSamples(int objectIndex, params string[] samples) =>
            AddAssert(
                $"{objectIndex.ToOrdinalWords()} has samples {string.Join(',', samples)}",
                () =>
                {
                    var h = EditorBeatmap.HitObjects.ElementAt(objectIndex);
        ...
        private void hitObjectHasSampleBank(int objectIndex, string bank) =>
            AddAssert(
                $"{objectIndex.ToOrdinalWords()} has bank {bank}",
                () =>
                {
                    var h = EditorBeatmap.HitObjects.ElementAt(objectIndex);
        ...
        private void hitObjectHasSampleNormalBank(int objectIndex, string bank) =>
            AddAssert(
                $"{objectIndex.ToOrdinalWords()} has normal bank {bank}",
                () =>
                {
                    var h = EditorBeatmap.HitObjects.ElementAt(objectIndex);
                    return h
        ...
        private void hitObjectHasSampleAdditionBank(int objectIndex, string bank) =>
            AddAssert(
                $"{objectIndex.ToOrdinalWords()} has addition bank {bank}",
                () =>
                {
                    var h = EditorBeatmap.HitObjects.ElementAt(objectIndex);
                    return h
        ...
        private void hitObjectNodeHasSamples(
            int objectIndex,
            int nodeIndex,
            ...
            AddAssert(
                ...
                () =>
                {
                    ...
                    return h is not null
                        && h.NodeSamples[nodeIndex].Select(s => s.Name).SequenceEqual(samples);
                }
            );

        private void hitObjectNodeHasSampleBank(int objectIndex, int nodeIndex, string bank) =>
            AddAssert(
                $"{objectIndex.ToOrdinalWords()} object {nodeIndex.ToOrdinalWords()} node has bank {bank}",
                () =>
                {
                    var h = EditorBeatmap.HitObjects.ElementAt(objectIndex) as IHasRepeats;
        ...
        private void hitObjectNodeHasSampleNormalBank(
            int objectIndex,
            int nodeIndex,
            string bank
        ) =>
            AddAssert(
                ...
                () =>
                {
                    var h = EditorBeatmap.HitObjects.ElementAt(objectIndex) as IHasRepeats;
                    return h is not null
                        && h.NodeSamples[nodeIndex]
                            .Where(o => o.Name == HitSampleInfo.HIT_NORMAL)
                            .All(o => o.Bank == bank);
                }
            );

        private void hitObjectNodeHasSampleAdditionBank(
            int objectIndex,
            int nodeIndex,
            string bank
        ) =>
        ...
        private void editorTimeIs(double time) =>
            AddAssert(
                $"editor time is {time}",
                () => Precision.AlmostEquals(EditorClock.CurrentTimeAccurate, time, 1)
            );
    }
}
