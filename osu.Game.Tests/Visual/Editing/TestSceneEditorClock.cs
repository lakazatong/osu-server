...
namespace osu.Game.Tests.Visual.Editing
{
    ...
    public partial class TestSceneEditorClock : EditorClockTestScene
    {
        ...
        public void TestStopAtTrackEnd()
        {
            ...
            AddAssert(
                "clock looped to start",
                () => EditorClock.IsRunning && EditorClock.CurrentTime < 500
            );
        }
        ...
        public void TestClampWhenSeekOutsideBeatmapBounds()
        {
            ...
            AddStep(
                "seek beyond track length",
                () => EditorClock.Seek(EditorClock.TrackLength + 1000)
            );
            AddAssert(
                "time is clamped to track length",
                () => EditorClock.CurrentTime,
                () => Is.EqualTo(EditorClock.TrackLength)
            );
            ...
            AddUntilStep(
                "time is clamped to 0",
                () => EditorClock.CurrentTime,
                () => Is.EqualTo(0)
            );

            AddStep(
                "seek smoothly beyond track length",
                () => EditorClock.SeekSmoothlyTo(EditorClock.TrackLength + 1000)
            );
            AddUntilStep(
                "time is clamped to track length",
                () => EditorClock.CurrentTime,
        ...
        {
            AddAssert(
                "seek smoothly twice and current time is accurate",
                () =>
                {
        ...
        public void TestAdjustmentsRemovedOnDisposal()
        {
            ...
            AddStep(
                "set 0.25x speed",
                () => this.ChildrenOfType<OsuTabControl<double>>().First().Current.Value = 0.25
            );
            AddAssert(
                "track has 0.25x tempo",
