...
namespace osu.Game.Tests.Visual.Background
{
    ...
    public partial class TestSceneBackgroundScreenDefault : OsuTestScene
    {
        ...
        private Graphics.Backgrounds.Background getCurrentBackground() =>
            screen.ChildrenOfType<Graphics.Backgrounds.Background>().FirstOrDefault();
        ...
        public void SetUpSteps()
        {
            ...
            AddStep(
                "push default screen",
                () => stack.Push(screen = new TestBackgroundScreenDefault())
            );
        ...
        public void TestBeatmapBackgroundTracksBeatmap()
        {
            ...
            AddStep(
                "change beatmap",
                () => Beatmap.Value = createTestWorkingBeatmapWithUniqueBackground()
            );
            ...
            AddUntilStep(
                "wait for beatmap background to be loaded",
                () => getCurrentBackground()?.GetType() == typeof(BeatmapBackground)
            );
            ...
            AddUntilStep(
                "wait for beatmap background to change",
                () => screen.CheckLastLoadChange() == true
            );

            AddUntilStep(
                "background is new beatmap background",
                () => last != getCurrentBackground()
            );
            AddStep("store background", () => last = getCurrentBackground());
        ...
        public void TestBeatmapBackgroundTracksBeatmapWhenSuspended()
        {
            ...
            AddStep(
                "push new background to stack",
                () => stack.Push(nestedScreen = new BackgroundScreenBeatmap(Beatmap.Value))
            );
            AddUntilStep(
                "wait for screen to load",
                () => nestedScreen.IsLoaded && nestedScreen.IsCurrentScreen()
            );
            AddUntilStep("previous background hidden", () => !screen.IsAlive);

            AddAssert(
                "top level background hasn't changed yet",
                () => screen.CheckLastLoadChange() == null
            );
        ...
        public void TestBeatmapBackgroundIgnoresNoChangeWhenSuspended()
        {
            ...
            AddStep(
                "change beatmap",
                () =>
                    originalWorking = Beatmap.Value = createTestWorkingBeatmapWithUniqueBackground()
            );
            AddAssert("background changed", () => screen.CheckLastLoadChange() == true);
            AddUntilStep(
                "wait for beatmap background to be loaded",
            ...
            AddAssert(
                "top level background reused existing",
                () => screen.CheckLastLoadChange() == false
            );
        }
        ...
        public void TestBeatmapBackgroundWithStoryboardClockAlwaysUsesCurrentTrack()
        {
            ...
            AddStep(
                "change beatmap",
                () => originalWorking = Beatmap.Value = createTestWorkingBeatmapWithStoryboard()
            );
            AddAssert("background changed", () => screen.CheckLastLoadChange() == true);
            AddUntilStep(
                "wait for beatmap background to be loaded",
                () => getCurrentBackground()?.GetType() == typeof(BeatmapBackgroundWithStoryboard)
            );

            AddStep("start music", () => MusicController.Play());
            AddUntilStep(
                "storyboard clock running",
                () =>
                    screen.ChildrenOfType<DrawableStoryboard>().SingleOrDefault()?.Clock.IsRunning
                    == true
            );
            ...
            AddStep(
                "change beatmap",
                () => Beatmap.Value = createTestWorkingBeatmapWithStoryboard()
            );
            AddStep("change beatmap back", () => Beatmap.Value = originalWorking);
            ...
            AddUntilStep(
                "storyboard clock running",
                () => screen.ChildrenOfType<DrawableStoryboard>().Single().Clock.IsRunning
            );
        ...
        public void TestBeatmapBackgroundWithStoryboardUnloadedOnSuspension()
        {
            ...
            AddUntilStep(
                "storyboard present",
                () =>
                    screen.ChildrenOfType<DrawableStoryboard>().SingleOrDefault()?.IsLoaded == true
            );
            ...
            AddUntilStep(
                "storyboard unloaded",
                () => !screen.ChildrenOfType<DrawableStoryboard>().Any()
            );
        ...
        public void TestBackgroundTypeSwitch()
        {
            ...
            setSourceMode(BackgroundSource.Beatmap);
            AddUntilStep(
                "is beatmap background",
                () => getCurrentBackground() is BeatmapBackground
            );

            setSourceMode(BackgroundSource.BeatmapWithStoryboard);
            AddUntilStep(
                "is storyboard background",
                () => getCurrentBackground() is BeatmapBackgroundWithStoryboard
            );

            setSourceMode(BackgroundSource.Skin);
            AddUntilStep(
                "is default background",
                () => getCurrentBackground().GetType() == typeof(Graphics.Backgrounds.Background)
            );
        ...
        public void TestBackgroundDoesntReloadOnNoChange(
        ...
        )
        {
            ...
            AddUntilStep(
                "wait for beatmap background to be loaded",
                () => (getCurrentBackground())?.GetType() == backgroundType
            );
        ...
        private void setSourceMode(BackgroundSource source) =>
            AddStep(
                $"set background mode to {source}",
                () => config.SetValue(OsuSetting.MenuBackgroundSource, source)
            );

        private void setSupporter(bool isSupporter) =>
            AddStep(
                $"set supporter {isSupporter}",
                () =>
        ...
        private WorkingBeatmap createTestWorkingBeatmapWithUniqueBackground() =>
            new UniqueBackgroundTestWorkingBeatmap(renderer, Audio);

        private WorkingBeatmap createTestWorkingBeatmapWithStoryboard() =>
            new TestWorkingBeatmapWithStoryboard(Audio);
        ...
        private void setCustomSkin()
        {
            ...
            AddStep(
                "set custom skin",
                () => skins.CurrentSkinInfo.Value = new SkinInfo().ToLiveUnmanaged()
            );
        }

        private void setDefaultSkin() =>
            AddStep("set default skin", () => skins.CurrentSkinInfo.SetDefault());
