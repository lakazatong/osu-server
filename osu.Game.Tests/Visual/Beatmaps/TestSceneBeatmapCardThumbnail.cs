...
namespace osu.Game.Tests.Visual.Beatmaps
{
    public partial class TestSceneBeatmapCardThumbnail : OsuManualInputManagerTestScene
    {
        ...
        public void TestThumbnailPreview()
        {
            ...
            AddUntilStep(
                "wait for start",
                () => playButton.Playing.Value && playButton.Enabled.Value
            );
            ...
            AddUntilStep(
                "progress > 0.25",
                () => thumbnail.ChildrenOfType<PlayButton>().Single().Progress.Value > 0.25
            );
            AddUntilStep(
                "progress > 0.5",
                () => thumbnail.ChildrenOfType<PlayButton>().Single().Progress.Value > 0.5
            );
            AddUntilStep(
                "progress > 0.75",
                () => thumbnail.ChildrenOfType<PlayButton>().Single().Progress.Value > 0.75
            );
        ...
        private void iconIs(IconUsage usage) =>
            AddUntilStep(
                "icon is correct",
                () => playButton.ChildrenOfType<SpriteIcon>().Any(icon => icon.Icon.Equals(usage))
            );
    }
}
