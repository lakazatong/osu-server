...
namespace osu.Game.Tests.Visual.Settings
{
    public partial class TestSceneLatencyCertifierScreen : ScreenTestScene
    {
        ...
        public override void SetUpSteps()
        {
            ...
            AddStep(
                "Load screen",
                () => LoadScreen(latencyCertifier = new LatencyCertifierScreen())
            );
        ...
        {
            AddStep(
                "set visual mode to simple",
                () => latencyCertifier.VisualMode.Value = LatencyVisualMode.Simple
            );
        }
        ...
        {
            AddStep(
                "set visual mode to circles",
                () => latencyCertifier.VisualMode.Value = LatencyVisualMode.CircleGameplay
            );
        }
        ...
        {
            AddStep(
                "set visual mode to scrolling",
                () => latencyCertifier.VisualMode.Value = LatencyVisualMode.ScrollingGameplay
            );
        }
        ...
        public void TestCertification()
        {
            ...
            AddAssert(
                "check at results",
                () => !latencyCertifier.ChildrenOfType<LatencyArea>().Any()
            );
            checkDifficulty(1);
        }
        ...
        private void checkDifficulty(int difficulty)
        {
            AddAssert(
                ...
                () => latencyCertifier.DifficultyLevel == difficulty
            );
        }
        ...
        {
            AddUntilStep(
                "click correct button until results",
                () =>
                {
                    var latencyArea = latencyCertifier
                        .ChildrenOfType<LatencyArea>()
                        .SingleOrDefault(a =>
                            clickCorrect ? a.TargetFrameRate == null : a.TargetFrameRate != null
                        );
