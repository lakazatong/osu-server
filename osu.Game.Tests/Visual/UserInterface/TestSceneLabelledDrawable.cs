...
namespace osu.Game.Tests.Visual.UserInterface
{
    public partial class TestSceneLabelledDrawable : OsuTestScene
    {
        ...
        [TestCase(false)]
        [TestCase(true)]
        public void TestNonPadded(bool hasDescription) =>
            createPaddedComponent(hasDescription, false);
        ...
        public void TestFixedWidth()
        {
            ...
            AddStep(
                "unset label width",
                () =>
                    this.ChildrenOfType<LabelledDrawable<Drawable>>()
                        .ForEach(d => d.FixedLabelWidth = null)
            );
            AddStep(
                "reset label width",
                () =>
                    this.ChildrenOfType<LabelledDrawable<Drawable>>()
                        .ForEach(d => d.FixedLabelWidth = label_width)
            );
        }
        ...
        private void createPaddedComponent(bool hasDescription = false, bool padded = true)
        {
            ...
            AddStep(
                ...
                () =>
                {
                    Child = new Container
                    {
                        ...
                        AutoSizeAxes = Axes.Y,
                        Child = component =
                            padded ? new PaddedLabelledDrawable() : new NonPaddedLabelledDrawable(),
                    };
            ...
            AddAssert(
                $"description {(hasDescription ? "visible" : "hidden")}",
                () =>
                    component.ChildrenOfType<TextFlowContainer>().ElementAt(1).IsPresent
                    == hasDescription
            );
        }
