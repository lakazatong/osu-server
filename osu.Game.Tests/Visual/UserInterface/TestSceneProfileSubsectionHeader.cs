    ...
    public partial class TestSceneProfileSubsectionHeader : OsuTestScene
    {
        ...
        public void TestHiddenCounter()
        {
            AddStep(
                "Create header",
                () =>
                    createHeader("Header with hidden counter", CounterVisibilityState.AlwaysHidden)
            );
            AddAssert("Value is 0", () => header.Current.Value == 0);
            AddAssert(
                "Counter is hidden",
                () => header.ChildrenOfType<CounterPill>().First().Alpha == 0
            );
        ...
        public void TestVisibleCounter()
        {
            AddStep(
                ...
                () =>
                    createHeader(
                        "Header with visible counter",
                        CounterVisibilityState.AlwaysVisible
                    )
            );
            AddAssert("Value is 0", () => header.Current.Value == 0);
            AddAssert(
                "Counter is visible",
                () => header.ChildrenOfType<CounterPill>().First().Alpha == 1
            );
            AddStep("Set count 10", () => header.Current.Value = 10);
            AddAssert("Value is 10", () => header.Current.Value == 10);
            AddAssert(
        ...
        public void TestVisibleWhenZeroCounter()
        {
            AddStep(
                ...
                () =>
                    createHeader(
                        "Header with visible when zero counter",
                        CounterVisibilityState.VisibleWhenZero
                    )
            );
            AddAssert("Value is 0", () => header.Current.Value == 0);
            AddAssert(
        ...
        private void createHeader(string text, CounterVisibilityState state, int initialValue = 0)
        {
            Clear();
            Add(
                header = new ProfileSubsectionHeader(text, state)
                {
