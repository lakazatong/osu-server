...
namespace osu.Game.Tests.Visual.UserInterface
{
    public partial class TestSceneUprightAspectMaintainingContainer : OsuGridTestScene
    {
        ...
        private readonly ScaleMode[] scaleModeValues =
        {
            ScaleMode.NoScaling,
        ...
        private readonly List<List<UprightAspectMaintainingContainer>> childContainers = new List<
            List<UprightAspectMaintainingContainer>
        >(rows);
            ...
            : base(rows, columns)
        {
            ...
            rotationSteps.AddRange(
                Enumerable
                    .Range(0, rotation_step_count)
                    .Select(x => 360f * ((float)x / rotation_step_count))
            );
        ...
        public void ExplicitlySizedParent()
        {
            ...
            {
                Vector2 parentScale = new Vector2(
                    state.xFlip * state.xScale,
                    state.yFlip * state.yScale
                );
                float parentRotation = state.rotation;

                AddStep(
                    "S: ("
                        + parentScale.X.ToString("0.00")
                        + ", "
                        + parentScale.Y.ToString("0.00")
                        + "), R: "
                        + parentRotation.ToString("0.00"),
                    () =>
                    {
                ...
                AddAssert(
                    ...
                    () =>
                    {
                        foreach (int i in Enumerable.Range(0, parentContainers.Count))
                        {
                            ...
                            {
                                if (
                                    !uprightAspectMaintainingContainerStateIsValid(
                                        parentContainers[i][j],
                                        childContainers[i][j]
                                    )
                                )
                                    return false;
        ...
        private bool uprightAspectMaintainingContainerStateIsValid(
            Container parent,
            UprightAspectMaintainingContainer child
        )
        {
            ...
            if (
                !(
                    isNearlyZero(MathF.Abs(childMatrix.M21))
                    && isNearlyZero(MathF.Abs(childMatrix.M12))
                )
            )
                return false;
            ...
            switch (child.Scaling)
            {
                case ScaleMode.NoScaling:
                    if (
                        !(
                            isNearlyZero(childMatrix.M11 - 1.0f)
                            && isNearlyZero(childMatrix.M22 - 1.0f)
                        )
                    )
                        return false;
