...
namespace osu.Game.Tests.Visual.UserInterface
{
    public partial class TestSceneOsuTextBox : ThemeComparisonTestScene
    {
        ...
        public void TestNumberBox()
        {
            ...
            AddStep(
                "enter numbers",
                () => numberBoxes.ForEach(numberBox => numberBox.Text = "987654321")
            );
            expectedValue(numberBoxes, "987654321");

            clearTextboxes(numberBoxes);
            AddStep(
                "enter text + single number",
                () => numberBoxes.ForEach(numberBox => numberBox.Text = "1 hello 2 world 3")
            );
        ...
        public void TestSelectAllOnFocus()
        {
            ...
            AddAssert(
                "nothing selected",
                () => string.IsNullOrEmpty(numberBoxes.First().SelectedText)
            );
            AddStep(
                "click on a number box",
                () =>
                {
            ...
            AddStep(
                ...
                () =>
                    InputManager.MoveMouseTo(
                        textContainer.ScreenSpaceDrawQuad.BottomRight
                            - new Vector2(textContainer.ScreenSpaceDrawQuad.Width / 2 + 1f, 0)
                    )
            );
        ...
        private void clearTextboxes(IEnumerable<OsuTextBox> textBoxes) =>
            AddStep("clear textbox", () => textBoxes.ForEach(textBox => textBox.Text = null));

        private void expectedValue(IEnumerable<OsuTextBox> textBoxes, string value) =>
            AddAssert(
                "expected textbox value",
                () => textBoxes.All(textBox => textBox.Text == value)
            );
    }
}
