...
namespace osu.Game.Tests.Visual.UserInterface
{
    ...
    public partial class TestSceneShearedButtons : OsuManualInputManagerTestScene
    {
        ...
        public void TestSizing()
        {
            ...
            AddAssert(
                "draw width is 200",
                () => toggleButton.DrawWidth,
                () => Is.EqualTo(200).Within(Precision.FLOAT_EPSILON)
            );
            ...
            AddAssert(
                "button is wider than text",
                () => toggleButton.DrawWidth,
                () => Is.GreaterThan(toggleButton.ChildrenOfType<SpriteText>().Single().DrawWidth)
            );
            ...
            AddAssert(
                "button width decreased",
                () => toggleButton.DrawWidth,
                () => Is.LessThan(originalDrawWidth)
            );
        }
        ...
        public void TestDisabledState()
        {
            ...
            void assertToggleState(bool active) =>
                AddAssert(
                    $"toggle is {(active ? "" : "not ")}active",
                    () => button.Active.Value == active
                );

            void setToggleDisabledState(bool disabled) =>
                AddStep(
                    $"{(disabled ? "disable" : "enable")} toggle",
