    ...
    public partial class TestSceneMultiplayer : ScreenTestScene
    {
        ...
        public void TestLobbyEvents()
        {
                ...
                new Room
                {
                    ...
                    [
                        new PlaylistItem(
                            beatmaps
                                .GetWorkingBeatmap(
                                    importedSet.Beatmaps.First(b => b.Ruleset.OnlineID == 0)
                                )
                                .BeatmapInfo
                        )
                        {
            ...
            AddUntilStep(
                ...
                () =>
                {
                    var nextUnready = multiplayerClient.ClientRoom?.Users.FirstOrDefault(c =>
                        c.State == MultiplayerUserState.Idle
                    );
                    if (nextUnready != null)
                    ...
                    return multiplayerClient.ClientRoom?.Users.All(u =>
            ...
            AddStep(
                "unready all players at once",
                () =>
                {
                    Debug.Assert(multiplayerClient.ServerRoom != null);

                    foreach (var u in multiplayerClient.ServerRoom.Users)
                        multiplayerClient.ChangeUserState(u.UserID, MultiplayerUserState.Idle);
                }
            );

            AddStep(
                "ready all players at once",
                () =>
                {
                    Debug.Assert(multiplayerClient.ServerRoom != null);

                    foreach (var u in multiplayerClient.ServerRoom.Users)
                        multiplayerClient.ChangeUserState(u.UserID, MultiplayerUserState.Ready);
                }
            );
        }
        ...
        {
            var nextUnready = multiplayerClient.ServerRoom?.Users.FirstOrDefault(c =>
                c.State == MultiplayerUserState.Idle
            );
            if (nextUnready != null)
        ...
        {
            var nextUnready = multiplayerClient.ServerRoom?.Users.FirstOrDefault(c =>
                c.State == MultiplayerUserState.Ready
            );
            if (nextUnready != null)
        ...
        public void TestCreateRoomViaKeyboard()
        {
            ...
            AddUntilStep(
                "wait for settings",
                () =>
                    InputManager.ChildrenOfType<MultiplayerMatchSettingsOverlay>().FirstOrDefault()
                    != null
            );
            ...
            AddUntilStep(
                ...
                () =>
                    InputManager
                        .ChildrenOfType<MultiplayerMatchSongSelect>()
                        .FirstOrDefault()
            ...
            AddStep("Press select", () => InputManager.Key(Key.Enter));
            AddUntilStep(
                "wait for return to screen",
                () =>
                    InputManager.ChildrenOfType<MultiplayerMatchSongSelect>().FirstOrDefault()
                    == null
            );
        ...
        public void TestCreateRoomWithoutPassword()
        {
            ...
            AddUntilStep(
                "Check participant count correct",
                () => multiplayerClient.ClientAPIRoom?.ParticipantCount == 1
            );
            AddUntilStep(
                "Check participant list contains user",
                () =>
                    multiplayerClient.ClientAPIRoom?.RecentParticipants.Count(u =>
                        u.Id == API.LocalUser.Value.Id
                    ) == 1
            );
        }
        ...
        public void TestExitMidJoin()
        {
            AddStep(
                ...
                () =>
                {
                    multiplayerClient.AddServerSideRoom(
                        new Room
                        {
                            Name = "Test Room",
                            Playlist =
            ...
            AddStep(
                "refresh rooms",
                () => this.ChildrenOfType<LoungeSubScreen>().Single().UpdateFilter()
            );
            AddUntilStep("wait for room", () => this.ChildrenOfType<RoomPanel>().Any());

            AddStep("select room", () => InputManager.Key(Key.Down));
            AddStep(
                "join room and immediately exit select",
                () =>
                {
        ...
        public void TestCreateRoomWithPassword()
        {
            ...
            AddUntilStep(
                "room has password",
                () => multiplayerClient.ClientAPIRoom?.Password == "password"
            );
        }
        ...
        public void TestLocalPasswordUpdatedWhenMultiplayerSettingsChange()
        {
            ...
            AddStep(
                "change password",
                () => multiplayerClient.ChangeSettings(password: "password2")
            );
            AddUntilStep(
                "local password changed",
                () => multiplayerClient.ClientAPIRoom?.Password == "password2"
            );
        }
        ...
        public void TestPlayStartsWithCorrectBeatmapWhileAtSongSelect()
        {
            ...
            createRoom(() =>
            {
                item = new PlaylistItem(
                    beatmaps
                        .GetWorkingBeatmap(importedSet.Beatmaps.First(b => b.Ruleset.OnlineID == 0))
                        .BeatmapInfo
                )
                {
                    RulesetID = new OsuRuleset().RulesetInfo.OnlineID,
                };
                return new Room { Name = "Test Room", Playlist = [item] };
            });
            ...
            AddStep(
                ...
                () =>
                {
                    var currentSubScreen = (
                        (Screens.OnlinePlay.Multiplayer.Multiplayer)
                            multiplayerComponents.CurrentScreen
                    ).CurrentSubScreen;
            ...
            AddUntilStep(
                "wait for song select",
                () =>
                    this.ChildrenOfType<MultiplayerMatchSongSelect>()
                        .FirstOrDefault()
                        ?.BeatmapSetsLoaded == true
            );

            AddUntilStep(
                "Beatmap matches current item",
                () =>
                    Beatmap.Value.BeatmapInfo.OnlineID
                    == multiplayerClient.ClientRoom?.Playlist.First().BeatmapID
            );

            AddStep("Select next beatmap", () => InputManager.Key(Key.Down));

            AddUntilStep(
                "Beatmap doesn't match current item",
                () =>
                    Beatmap.Value.BeatmapInfo.OnlineID
                    != multiplayerClient.ClientRoom?.Playlist.First().BeatmapID
            );
        ...
        public void TestPlayStartsWithCorrectRulesetWhileAtSongSelect()
        {
            ...
            AddUntilStep(
                "Ruleset matches current item",
                () =>
                    Ruleset.Value.OnlineID
                    == multiplayerClient.ClientRoom?.Playlist.First().RulesetID
            );
            ...
            AddStep(
                ...
                () =>
                    (
                        ...
                        .Ruleset
                        .Value = new CatchRuleset().RulesetInfo
            );

            AddUntilStep(
                "Ruleset doesn't match current item",
                () =>
                    Ruleset.Value.OnlineID
                    != multiplayerClient.ClientRoom?.Playlist.First().RulesetID
            );
        ...
        public void TestPlayStartsWithCorrectModsWhileAtSongSelect()
        {
            ...
            AddUntilStep(
                "Mods match current item",
                () =>
                    SelectedMods
                        .Value.Select(m => m.Acronym)
                        .SequenceEqual(
                            multiplayerClient
                                .ClientRoom.AsNonNull()
                                .Playlist.First()
                                .RequiredMods.Select(m => m.Acronym)
                        )
            );

            AddStep(
                "Switch required mods",
                () =>
                    (
                        (MultiplayerMatchSongSelect)
                            multiplayerComponents.MultiplayerScreen.CurrentSubScreen
                        ...
                        .Mods
                        .Value = new Mod[] { new OsuModDoubleTime() }
            );

            AddUntilStep(
                "Mods don't match current item",
                () =>
                    !SelectedMods
                        .Value.Select(m => m.Acronym)
                        .SequenceEqual(
        ...
        public void TestLocalPlayDoesNotStartWhileSpectatingWithNoBeatmap()
        {
            ...
            AddStep(
                "join other user (ready, host)",
                () =>
                {
                    multiplayerClient.AddUser(
                        new APIUser { Id = MultiplayerTestScene.PLAYER_1_ID, Username = "Other" }
                    );
            ...
            AddUntilStep(
                "wait for spectating user state",
        ...
        public void TestLocalPlayStartsWhileSpectatingWhenBeatmapBecomesAvailable()
        {
            ...
            AddUntilStep(
                "play started",
                () => multiplayerComponents.CurrentScreen is SpectatorScreen
            );
        }
        ...
        public void TestSubScreenExitedWhenDisconnectedFromMultiplayerServer()
        {
            ...
            AddUntilStep(
                "back in lounge",
                () =>
                    this.ChildrenOfType<LoungeSubScreen>().FirstOrDefault()?.IsCurrentScreen()
                    == true
            );
        }
        ...
        public void TestLeaveNavigation()
        {
            ...
            AddStep(
                "open mod overlay",
                () => this.ChildrenOfType<UserModSelectButton>().Single().TriggerClick()
            );

            AddStep("invoke on back button", () => multiplayerComponents.OnBackButton());

            AddAssert(
                "mod overlay is hidden",
                () =>
                    this.ChildrenOfType<MultiplayerUserModSelectOverlay>().Single().State.Value
                    == Visibility.Hidden
            );

            AddAssert(
                "dialog overlay is hidden",
                () => DialogOverlay.State.Value == Visibility.Hidden
            );
            ...
            void testLeave(string actionName, Action action)
            {
                ...
                AddAssert(
                    ...
                    () => DialogOverlay.State.Value == Visibility.Visible
                );
        ...
        public void TestGameplayFlow()
        {
            ...
            for (double i = 1000; i < TestResources.QUICK_BEATMAP_LENGTH; i += 1000)
            {
                ...
                AddUntilStep(
                    $"wait for time > {i}",
                    () =>
                        this.ChildrenOfType<GameplayClockContainer>().SingleOrDefault()?.CurrentTime
                        > time
                );
            }
            ...
            AddUntilStep(
                "wait for results",
                () => multiplayerComponents.CurrentScreen is ResultsScreen
            );
        ...
        public void TestGameplayExitFlow()
        {
            ...
            AddUntilStep(
                "wait for playing",
                () => this.ChildrenOfType<Player>().FirstOrDefault()?.LocalUserPlaying.Value == true
            );
            ...
            AddUntilStep(
                ...
                () =>
                    multiplayerComponents.CurrentScreen
                    is Screens.OnlinePlay.Multiplayer.Multiplayer
            );
        ...
        public void TestGameplayDoesntStartWithNonLoadedUser()
        {
            ...
            AddAssert(
                "ensure gameplay hasn't started",
                () =>
                    this.ChildrenOfType<GameplayClockContainer>().SingleOrDefault()?.IsRunning
                    == false
            );
        }
        ...
        public void TestRoomSettingsReQueriedWhenJoiningRoom()
        {
            ...
            AddStep(
                "disable polling",
                () => this.ChildrenOfType<LoungeListingPoller>().Single().TimeBetweenPolls.Value = 0
            );
            AddStep(
                "change server-side settings",
                () =>
                {
            ...
            AddAssert(
                "local room has correct name",
                () => this.ChildrenOfType<MultiplayerRoomPanel>().Single().Room.Name,
                () => Is.EqualTo(multiplayerClient.ServerSideRooms[0].Name)
            );
            AddAssert(
                "local room has correct playlist",
                () => this.ChildrenOfType<MultiplayerQueueList>().Single().Items.Single().ID,
                () => Is.EqualTo(2)
            );
        }
        ...
        public void TestSpectatingStateResetOnBackButtonDuringGameplay()
        {
            ...
            AddUntilStep(
                "wait for gameplay",
                () =>
                    (multiplayerComponents.CurrentScreen as MultiSpectatorScreen)?.IsLoaded == true
            );

            AddStep(
                "press back button and exit",
                () =>
                {
            ...
            AddUntilStep(
                "wait for return to match subscreen",
                () => multiplayerComponents.MultiplayerScreen.IsCurrentScreen()
            );
            AddUntilStep(
                "user state is idle",
                () => multiplayerClient.LocalUser?.State == MultiplayerUserState.Idle
        ...
        public void TestSpectatingStateNotResetOnBackButtonOutsideOfGameplay()
        {
            ...
            AddStep(
                "set other user loaded",
                () => multiplayerClient.ChangeUserState(1234, MultiplayerUserState.Loaded)
            );
            AddStep(
                "set other user finished play",
                () => multiplayerClient.ChangeUserState(1234, MultiplayerUserState.FinishedPlay)
            );
        ...
        public void TestItemAddedByOtherUserDuringGameplay()
        {
            ...
            AddUntilStep(
                "item arrived in playlist",
                () => multiplayerClient.ClientRoom?.Playlist.Count == 2
            );

            AddStep(
                "exit gameplay as initial user",
                () => multiplayerComponents.MultiplayerScreen.MakeCurrent()
            );
            AddUntilStep(
                "queue contains item",
                () => this.ChildrenOfType<MultiplayerQueueList>().Single().Items.Single().ID == 2
            );
        }
        ...
        public void TestItemAddedAndDeletedByOtherUserDuringGameplay()
        {
            ...
            AddStep(
                "delete item as other user",
                () => multiplayerClient.RemoveUserPlaylistItem(1234, 2).WaitSafely()
            );
            AddUntilStep(
                "item removed from playlist",
                () => multiplayerClient.ClientRoom?.Playlist.Count == 1
            );
            ...
            AddUntilStep(
                "queue is empty",
                () => this.ChildrenOfType<MultiplayerQueueList>().Single().Items.Count == 0
            );
        }
        ...
        public void TestGameplayStartsWhileInSpectatorScreen()
        {
            ...
            AddStep(
                ...
                () => multiplayerClient.ChangeUserState(1234, MultiplayerUserState.Idle)
            );
            AddUntilStep(
                "wait for room to be idle",
                () => multiplayerClient.ClientRoom?.State == MultiplayerRoomState.Open
            );
            ...
            void runGameplay()
            {
                ...
                AddUntilStep(
                    "wait for loading",
                    () => multiplayerClient.ClientRoom?.State == MultiplayerRoomState.WaitingForLoad
                );
                AddStep(
                    "set player loaded",
                    () => multiplayerClient.ChangeUserState(1234, MultiplayerUserState.Loaded)
                ...
                AddUntilStep(
                    "wait for local user to enter spectator",
                    () => multiplayerComponents.CurrentScreen is MultiSpectatorScreen
                );
            }
        }
        ...
        public void TestGameplayStartsWhileInSongSelectWithDifferentRuleset()
        {
                ...
                new Room
                {
                    ...
                    [
                        ...
                        new PlaylistItem(
                            beatmaps
                                .GetWorkingBeatmap(
                                    importedSet.Beatmaps.First(b => b.Ruleset.OnlineID == 1)
                                )
                                .BeatmapInfo
                        )
                        {
            ...
            AddStep(
                "select hidden",
                () => multiplayerClient.ChangeUserMods(new[] { new APIMod { Acronym = "HD" } })
            );
            AddStep(
                "make user ready",
            ...
            AddStep(
                "press edit on second item",
                () =>
                    this.ChildrenOfType<DrawableRoomPlaylistItem>()
                        .Single(i => i.Item.RulesetID == 1)
                        .ChildrenOfType<DrawableRoomPlaylistItem.PlaylistEditButton>()
                        .Single()
                        .TriggerClick()
            );
            ...
            AddAssert(
                "hidden is selected",
                () => SelectedMods.Value,
                () => Has.One.TypeOf(typeof(OsuModHidden))
            );
        }
        ...
        public void TestGlobalBeatmapDoesNotChangeAtResults()
        {
            ...
            AddAssert(
                "global beatmap still matches first playlist item",
                () => Beatmap.Value.BeatmapInfo.OnlineID,
                () => Is.EqualTo(multiplayerClient.ClientRoom!.Playlist[0].BeatmapID)
            );
            AddStep("return to match", () => multiplayerComponents.Exit());
            AddAssert(
                "global beatmap matches second playlist item",
                () => Beatmap.Value.BeatmapInfo.OnlineID,
                () => Is.EqualTo(multiplayerClient.ClientRoom!.Playlist[1].BeatmapID)
            );
        }
        ...
        private void pressReadyButton(int? playingUserId = null)
        {
            ...
            AddStep(
                ...
                () =>
                {
                    user =
                        playingUserId == null
                            ? multiplayerClient.LocalUser
                            : multiplayerClient.ServerRoom?.Users.Single(u =>
                                u.UserID == playingUserId
                            );
        ...
        private void createRoom(Func<Room> room)
        {
            ...
            AddStep(
                "open room",
                () => multiplayerComponents.ChildrenOfType<LoungeSubScreen>().Single().Open(room())
            );
