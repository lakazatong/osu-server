    ...
    public partial class TestSceneMultiplayerMatchSubScreen : MultiplayerTestScene
    {
        ...
        public override void SetUpSteps()
        {
            ...
            AddUntilStep(
                ...
                () => this.ChildrenOfType<MultiplayerUserModSelectOverlay>().Count(),
                () => Is.Zero
            );
        ...
        public void TestTaikoOnlyMod()
        {
            AddStep(
                ...
                () =>
                {
                    ...
                    [
                        new PlaylistItem(
                            new TestBeatmap(new TaikoRuleset().RulesetInfo).BeatmapInfo
                        )
                        {
            ...
            AddStep(
                ...
                () =>
                    MultiplayerClient.ChangeUserMods(
                        ...
                        new[] { new TaikoModSwap() }
                    )
            );
            AddUntilStep(
                "participant panel has mod",
                () =>
                    this.ChildrenOfType<ParticipantPanel>()
                        .Any(p => p.ChildrenOfType<ModIcon>().Any(m => m.Mod is TaikoModSwap))
            );
        }
        ...
        {
            AddAssert(
                "create button not enabled",
                () =>
                    !this.ChildrenOfType<MultiplayerMatchSettingsOverlay.CreateOrUpdateButton>()
                        .Single()
                        .Enabled.Value
            );
        ...
        public void TestStartMatchWhileSpectating()
        {
            AddStep(
                ...
                () =>
                {
                    ...
                    [
                        new PlaylistItem(
                            beatmaps.GetWorkingBeatmap(importedSet.Beatmaps.First()).BeatmapInfo
                        )
                        {
                            RulesetID = new OsuRuleset().RulesetInfo.OnlineID,
                        },
            ...
            AddStep(
                "join other user",
                void () => MultiplayerClient.AddUser(new APIUser { Id = PLAYER_1_ID })
            );
            AddUntilStep(
                "wait for user populated",
                () => MultiplayerClient.ClientRoom!.Users.Single(u => u.UserID == PLAYER_1_ID).User,
                () => Is.Not.Null
            );
            AddStep(
                "other user ready",
        ...
        {
            AddStep(
                "add playlist item with allowed mod",
                () =>
                {
                    room.Playlist =
                    [
            ...
            AddUntilStep(
                "mod select contents loaded",
                () =>
                    this.ChildrenOfType<ModColumn>().Any()
                    && this.ChildrenOfType<ModColumn>().All(col => col.IsLoaded && col.ItemsLoaded)
            );
            AddUntilStep(
                "mod select contains only double time mod",
                () =>
                    this.ChildrenOfType<MultiplayerUserModSelectOverlay>()
                        .Single()
                        .ChildrenOfType<ModPanel>()
                        .SingleOrDefault(panel => panel.Visible)
                        ?.Mod is OsuModDoubleTime
            );
        }
        ...
        {
            AddStep(
                "add playlist item with no allowed mods",
                () =>
                {
                    room.Playlist =
                    [
            ...
            AddUntilStep(
                ...
                () =>
                {
                    var lastItem = this.ChildrenOfType<MultiplayerQueueList>()
                        ...
                        .Single(p =>
                            p.Item.ID == MultiplayerClient.ServerAPIRoom?.Playlist.Last().ID
                        );
        ...
        public void TestModSelectOverlay()
        {
            ...
            ClickButtonWhenEnabled<UserModSelectButton>();
            AddUntilStep(
                "mod select shows unranked",
                () =>
                    this.ChildrenOfType<RankingInformationDisplay>().Single().Ranked.Value == false
            );
            AddAssert(
                "score multiplier = 1.20",
                () => this.ChildrenOfType<RankingInformationDisplay>().Single().ModMultiplier.Value,
                () => Is.EqualTo(1.2).Within(0.01)
            );
            ...
            AddStep(
                ...
                () =>
                    this.ChildrenOfType<MultiplayerUserModSelectOverlay>()
                        .Single()
                        .ChildrenOfType<ModPanel>()
                        .Single(m => m.Mod is ModFlashlight)
                        .TriggerClick()
            );
            AddAssert(
                "score multiplier = 1.35",
                () => this.ChildrenOfType<RankingInformationDisplay>().Single().ModMultiplier.Value,
                () => Is.EqualTo(1.35).Within(0.01)
            );

            AddStep(
                "change flashlight setting",
                () =>
                    (
                        (OsuModFlashlight)
                            this.ChildrenOfType<MultiplayerUserModSelectOverlay>()
                                .Single()
                                .SelectedMods.Value.Single()
                    )
                        .FollowDelay
                        .Value = 1200
            );
        ...
        public void TestChangeSettingsButtonVisibleForHost()
        {
            ...
            AddUntilStep(
                "button visible",
                () =>
                    this.ChildrenOfType<MultiplayerRoomPanel>().Single().ChangeSettingsButton.Alpha,
                () => Is.GreaterThan(0)
            );
            AddStep(
                "join other user",
        ...
        public void TestUserModSelectUpdatesWhenNotVisible()
        {
            ...
            AddStep(
                "click flashlight panel",
                () =>
                {
                    ModPanel panel = this.ChildrenOfType<ModPanel>()
                        .Single(p => p.Mod is OsuModFlashlight);
                    InputManager.MoveMouseTo(panel);
                    InputManager.Click(MouseButton.Left);
                }
            );
            AddUntilStep(
                "flashlight mod enabled",
                () => MultiplayerClient.ClientRoom!.Users[0].Mods.Any()
            );
            ...
            AddStep(
                "close mod select overlay",
                () => this.ChildrenOfType<MultiplayerUserModSelectOverlay>().Single().Hide()
            );
            AddUntilStep(
                "mod select overlay not present",
                () => !this.ChildrenOfType<MultiplayerUserModSelectOverlay>().Single().IsPresent
            );
            AddStep(
                "disable allowed mods",
                () =>
                    MultiplayerClient.EditPlaylistItem(
                        new MultiplayerPlaylistItem(
                            new PlaylistItem(MultiplayerClient.ServerRoom!.Playlist[0])
                            {
                                AllowedMods = [],
                            }
                        )
            ...
            AddUntilStep(
                "flashlight mod disabled",
                () => !MultiplayerClient.ClientRoom!.Users[0].Mods.Any()
            );
            AddStep(
                "re-enable allowed mods",
                () =>
                    MultiplayerClient.EditPlaylistItem(
                        new MultiplayerPlaylistItem(
                            new PlaylistItem(MultiplayerClient.ServerRoom!.Playlist[0])
            ...
            AddAssert(
                "flashlight mod panel not activated",
                () =>
        ...
        public void TestStartCountdown()
        {
            ...
            AddStep(
                ...
                () =>
                {
                    InputManager.MoveMouseTo(
                        this.ChildrenOfType<MultiplayerCountdownButton>().Single()
                    );
                    InputManager.Click(MouseButton.Left);
                }
            );
            ...
            AddStep(
                ...
                {
                    InputManager.MoveMouseTo(
                        this.ChildrenOfType<Popover>().Single().ChildrenOfType<Button>().First()
                    );
                    InputManager.Click(MouseButton.Left);
                }
            );
            ...
            AddUntilStep(
                "countdown started",
                () => MultiplayerClient.ServerRoom!.ActiveCountdowns.Any()
            );
        }
        ...
        public void TestSettingsRemainsOpenOnRoomUpdate()
        {
            ...
            AddStep(
                "open settings",
                () => this.ChildrenOfType<MultiplayerMatchSettingsOverlay>().Single().Show()
            );
            AddAssert(
                "settings opened",
                () => this.ChildrenOfType<MultiplayerMatchSettingsOverlay>().Single().State.Value,
            ...
            AddStep(
                "trigger room update",
                () =>
                    MultiplayerClient.AddPlaylistItem(
                        MultiplayerClient.ServerRoom!.Playlist[0].Clone()
                    )
            );
            AddAssert(
                "settings still open",
