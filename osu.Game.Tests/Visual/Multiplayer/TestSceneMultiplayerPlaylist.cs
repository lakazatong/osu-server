...
namespace osu.Game.Tests.Visual.Multiplayer
{
    public partial class TestSceneMultiplayerPlaylist : MultiplayerTestScene
    {
        ...
        {
            AddStep(
                "change to round robin queue mode",
                () =>
                    MultiplayerClient
                        .ChangeSettings(
            ...
            AddStep(
                "get drawable items",
                () => drawableItems = this.ChildrenOfType<DrawableRoomPlaylistItem>().ToArray()
            );
            ...
            AddAssert(
                "all previous playlist items remained",
                () => drawableItems.All(this.ChildrenOfType<DrawableRoomPlaylistItem>().Contains)
            );
        }
        ...
        /// </summary>
        private void addItemStep(bool expired = false, int? userId = null) =>
            AddStep(
                "add item",
                () =>
                {
                    MultiplayerClient
                        .AddUserPlaylistItem(
                            userId ?? API.LocalUser.Value.OnlineID,
        ...
        private void assertItemInQueueListStep(int playlistItemId, int visualIndex)
        {
            ...
            AddUntilStep(
                $"{playlistItemId} in queue at pos = {visualIndex}",
                () =>
                {
        ...
        private void assertItemInHistoryListStep(int playlistItemId, int visualIndex)
        {
            ...
            AddUntilStep(
                $"{playlistItemId} in history at pos = {visualIndex}",
                () =>
                {
        ...
        private void changeDisplayModeStep(MultiplayerPlaylistDisplayMode mode) =>
            AddStep($"change list to {mode}", () => list.DisplayMode.Value = mode);
