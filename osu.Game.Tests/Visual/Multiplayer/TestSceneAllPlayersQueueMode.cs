...
namespace osu.Game.Tests.Visual.Multiplayer
{
    public partial class TestSceneAllPlayersQueueMode : QueueModeTestScene
    {
        ...
        public void TestSingleItemExpiredAfterGameplay()
        {
            ...
            AddUntilStep(
                "playlist has only one item",
                () => MultiplayerClient.ClientAPIRoom?.Playlist.Count == 1
            );
            AddUntilStep(
        ...
        public void TestItemAddedToTheEndOfQueue()
        {
            ...
            AddUntilStep(
                ...
                () => MultiplayerClient.ClientAPIRoom?.Playlist.Count == 3
            );

            AddUntilStep(
                "first item still selected",
                () =>
                    MultiplayerClient.ClientRoom?.Settings.PlaylistItemId
                    == MultiplayerClient.ClientAPIRoom?.Playlist[0].ID
            );
        ...
        public void TestNextItemSelectedAfterGameplayFinish()
        {
            ...
            AddUntilStep(
                "second item expired",
                () => MultiplayerClient.ClientAPIRoom?.Playlist[1].Expired == true
            );
            AddUntilStep(
                "next item selected",
                () =>
                    MultiplayerClient.ClientRoom?.Settings.PlaylistItemId
                    == MultiplayerClient.ClientAPIRoom?.Playlist[2].ID
            );
        }
        ...
        public void TestItemsNotClearedWhenSwitchToHostOnlyMode()
        {
            ...
            AddStep(
                "change queue mode",
                () => MultiplayerClient.ChangeSettings(queueMode: QueueMode.HostOnly)
            );
            AddUntilStep(
                "playlist has 3 items",
                () => MultiplayerClient.ClientAPIRoom?.Playlist.Count == 3
            ...
            AddUntilStep(
                "item 2 is not expired",
                () => MultiplayerClient.ClientAPIRoom?.Playlist[1].Expired == false
            );
            AddUntilStep(
                "current item is the other beatmap",
                () => MultiplayerClient.ClientRoom?.Settings.PlaylistItemId == 2
            );
        }
        ...
        public void TestCorrectItemSelectedAfterNewItemAdded()
        {
            addItem(() => OtherBeatmap);
            AddUntilStep(
                "selected beatmap is initial beatmap",
                () => Beatmap.Value.BeatmapInfo.OnlineID == InitialBeatmap.OnlineID
            );
        }
        ...
        public void TestCorrectRulesetSelectedAfterNewItemAdded()
        {
            ...
            AddUntilStep(
                "wait for player",
                () => CurrentScreen is Player player && player.IsLoaded
            );
            AddAssert(
                "ruleset is correct",
                () => ((Player)CurrentScreen).Ruleset.Value.Equals(new OsuRuleset().RulesetInfo)
            );
        ...
        )
        {
            ...
            AddUntilStep(
                ...
                () => (songSelect = CurrentSubScreen as Screens.Select.SongSelect) != null
            );
            AddUntilStep("wait for loaded", () => songSelect.AsNonNull().BeatmapSetsLoaded);

            if (ruleset != null)
                AddStep(
                    $"set {ruleset.Name} ruleset",
                    () => songSelect.AsNonNull().Ruleset.Value = ruleset
                );

            if (mods != null)
                AddStep(
                    $"set mods to {string.Join(",", mods.Select(m => m.Acronym))}",
                    () => songSelect.AsNonNull().Mods.Value = mods
                );

            AddStep(
                "select other beatmap",
                () => songSelect.AsNonNull().FinaliseSelection(beatmap())
            );
            AddUntilStep(
