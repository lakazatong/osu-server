...
namespace osu.Game.Tests.Visual.Multiplayer
{
    public partial class TestSceneMultiplayerQueueList : MultiplayerTestScene
    {
        ...
        public override void SetUpSteps()
        {
            ...
            AddStep(
                ...
                () =>
                {
                    ...
                    playlist.Items.ReplaceRange(
                        0,
                        playlist.Items.Count,
                        MultiplayerClient.ClientAPIRoom!.Playlist
                    );
        ...
        public void TestDeleteButtonOnlyVisibleForItemOwnerIfNotHost()
        {
            ...
            AddStep(
                "set local user as host",
                () => MultiplayerClient.TransferHost(API.LocalUser.Value.OnlineID)
            );
            assertDeleteButtonVisibility(1, true);
            assertDeleteButtonVisibility(2, true);
        }
        ...
        public void TestCurrentItemHasDeleteButtonIfNotSingle()
        {
            ...
            AddStep(
                "finish current item",
                () => MultiplayerClient.FinishCurrentItem().WaitSafely()
            );
            AddUntilStep(
                "wait for next item to be selected",
                () => MultiplayerClient.ClientRoom?.Settings.PlaylistItemId == 2
            );
            AddUntilStep(
                "wait for two items in playlist",
                () => playlist.ChildrenOfType<DrawableRoomPlaylistItem>().Count() == 2
            );
        ...
        public void TestChangeExistingItem()
        {
            ...
            AddUntilStep(
                "first playlist item has new beatmap",
                () => playlist.Items[0].Beatmap.OnlineID,
                () => Is.EqualTo(1337)
            );
        }
        ...
        private void addPlaylistItem(Func<int> userId)
        {
            ...
            AddStep(
                ...
                () =>
                {
                    MultiplayerPlaylistItem item = new MultiplayerPlaylistItem(
                        new PlaylistItem(importedBeatmap)
                    );
            ...
            AddUntilStep(
                ...
                () =>
                    playlist
                        .ChildrenOfType<RearrangeableListItem<PlaylistItem>>()
                        .Any(i => i.Model.ID == itemId)
            );
        }
