...
namespace osu.Game.Tests.Visual.Gameplay
{
    public partial class TestSceneGameplayRewinding : OsuPlayerTestScene
    {
        ...
        public void TestNoJudgementsOnRewind()
        {
            ...
            AddAssert(
                "all judged",
                () => Player.DrawableRuleset.Playfield.AllHitObjects.All(h => h.Judged)
            );
            AddUntilStep(
                "key counter counted keys",
                () =>
                    Player
                        .HUDOverlay.InputCountController.Triggers.Select(kc =>
                            kc.ActivationCount.Value
                        )
                        .Sum() == 15
            );
        ...
        {
            var beatmap = new Beatmap { Difficulty = { ApproachRate = 9 } };

            for (int i = 0; i < 15; i++)
            {
                beatmap.HitObjects.Add(
                    new HitCircle { Position = new Vector2(256, 192), StartTime = 1000 + 30 * i }
                );
            }
