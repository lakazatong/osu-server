...
namespace osu.Game.Tests.Visual.Gameplay
{
    ...
    public partial class TestSceneGameplayLeaderboard : OsuTestScene
    {
        ...
        private TestGameplayLeaderboardProvider leaderboardProvider =
            ...
            AddStep(
                ...
                () =>
                {
                    if (leaderboard.IsNotNull())
                        leaderboard.CollapseDuringGameplay.Value = !leaderboard
                            .CollapseDuringGameplay
                            .Value;
                }
            );
        ...
        public void TestLayoutWithManyScores()
        {
            ...
            AddStep(
                "add many scores in one go",
                () =>
                {
                    for (int i = 0; i < 32; i++)
                        leaderboardProvider.CreateRandomScore(
                            new APIUser { Username = $"Player {i + 1}" }
                        );

                    // Add player at end to force an animation down the whole list.
                    playerScore.Value = 0;
                    leaderboardProvider.CreateLeaderboardScore(
                        playerScore,
                        new APIUser { Username = "You", Id = 3 },
                        true
                    );
                }
            );
            ...
            AddUntilStep(
                "wait for fill flow layout",
                () =>
                    leaderboard
                        .ChildrenOfType<FillFlowContainer<DrawableGameplayLeaderboardScore>>()
                        .First()
                        .ScreenSpaceDrawQuad.Intersects(leaderboard.ScreenSpaceDrawQuad)
            );

            AddUntilStep(
                "wait for some scores not masked away",
                () =>
                    leaderboard
                        .ChildrenOfType<DrawableGameplayLeaderboardScore>()
                        .Any(s =>
                            leaderboard.ScreenSpaceDrawQuad.Contains(s.ScreenSpaceDrawQuad.Centre)
                        )
            );

            AddUntilStep(
                "wait for tracked score fully visible",
                () =>
                    leaderboard.ScreenSpaceDrawQuad.Intersects(
                        leaderboard.TrackedScore!.ScreenSpaceDrawQuad
                    )
            );
        ...
        public void TestRandomScores()
        {
            ...
            int playerNumber = 1;
            AddRepeatStep(
                "add player with random score",
                () =>
                    leaderboardProvider.CreateRandomScore(
                        new APIUser { Username = $"Player {playerNumber++}" }
                    ),
                10
            );
        }
        ...
        public void TestExistingUsers()
        {
            ...
            AddStep(
                ...
                () =>
                    leaderboardProvider.CreateRandomScore(
                        new APIUser { Username = "peppy", Id = 2 }
                    )
            );
            AddStep(
                "add smoogipoo",
                () =>
                    leaderboardProvider.CreateRandomScore(
                        new APIUser { Username = "smoogipoo", Id = 1040328 }
                    )
            );
            AddStep(
                "add flyte",
                () =>
                    leaderboardProvider.CreateRandomScore(
                        new APIUser { Username = "flyte", Id = 3103765 }
                    )
            );
            AddStep(
                "add frenzibyte",
                () =>
                    leaderboardProvider.CreateRandomScore(
                        new APIUser { Username = "frenzibyte", Id = 14210502 }
                    )
            );
        }
        ...
        public void TestFriendScore()
        {
            ...
            AddRepeatStep(
                "add 3 friend score",
                () => leaderboardProvider.CreateRandomScore(friend),
                3
            );
            AddUntilStep(
                "at least one friend score is pink",
                () =>
                    leaderboard
                        .GetAllScoresForUsername("my friend")
        ...
        private partial class TestDrawableGameplayLeaderboard : DrawableGameplayLeaderboard
        {
            ...
            public IEnumerable<DrawableGameplayLeaderboardScore> GetAllScoresForUsername(
                string username
        ...
        public class TestGameplayLeaderboardProvider : IGameplayLeaderboardProvider
        {
            ...
            public GameplayLeaderboardScore CreateLeaderboardScore(
                BindableLong totalScore,
                APIUser user,
