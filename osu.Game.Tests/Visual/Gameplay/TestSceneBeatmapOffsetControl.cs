    ...
    public partial class TestSceneBeatmapOffsetControl : OsuTestScene
    {
        ...
        public void TestTooShortToDisplay()
        {
            ...
            AddAssert(
                ...
                () => !offsetControl.ChildrenOfType<SettingsButton>().Any()
            );
        }
        ...
        public void TestTooShortToDisplay_HasPreviousValidScore()
        {
            ...
            AddStep(
                "Press button",
                () => offsetControl.ChildrenOfType<SettingsButton>().Single().TriggerClick()
            );
            AddAssert(
                "Offset is adjusted",
                () => offsetControl.Current.Value == initial_offset - average_error
            );
        }
                ...
                () =>
                {
                    ...
                    [
                        new HitEvent(
                            ...
                            new SliderHeadCircle { ClassicSliderBehaviour = true },
                            null,
                            null
                        ),
                    ];
        ...
        public void TestScoreFromDifferentBeatmap()
        {
            AddStep(
                ...
                () =>
                {
                    ...
                    {
                        HitEvents =
                            TestSceneHitEventTimingDistributionGraph.CreateDistributedHitEvents(10),
        ...
        {
            AddStep(
                "Set score with mod removing timed inputs",
                () =>
                {
                    offsetControl.ReferenceScore.Value = new ScoreInfo
                    {
        ...
        public void TestCalibrationFromZero()
        {
            ...
            AddUntilStep(
                "Button is disabled",
                () => !offsetControl.ChildrenOfType<SettingsButton>().Single().Enabled.Value
            );

            recreateControl();
            AddStep(
                "Set same reference score",
                () => offsetControl.ReferenceScore.Value = referenceScore
            );
            AddAssert(
                "No calibration button",
                () => !offsetControl.ChildrenOfType<SettingsButton>().Any()
        ...
        public void TestCalibrationFromNonZeroWithImmediateReferenceScore()
        {
            ...
            AddStep(
                "Set beatmap offset non-neutral",
                () =>
                    Realm.Write(r =>
                    {
                        r.Add(
                            new BeatmapInfo
                            {
                                ...
                                UserSettings = { Offset = initial_offset },
                            }
                        );
                    })
            );
            ...
            AddStep(
                "Create control with preloaded reference score",
                () =>
                {
            ...
            AddAssert(
                "Offset is adjusted",
                () => offsetControl.Current.Value,
                () => Is.EqualTo(initial_offset - average_error)
            );
            AddUntilStep(
                "Button is disabled",
                () => !offsetControl.ChildrenOfType<SettingsButton>().Single().Enabled.Value
            ...
            AddStep(
                ...
                () =>
                {
                    Child = new PlayerSettingsGroup("Some settings")
                    {
                        Anchor = Anchor.Centre,
                        Origin = Anchor.Centre,
                        Children = new Drawable[] { offsetControl = new BeatmapOffsetControl() },
                    };
                }
            );
        }
