...
namespace osu.Game.Tests.Visual.SongSelect
{
    ...
    public partial class TestSceneAdvancedStats : OsuTestScene
    {
        ...
        [SetUp]
        public void Setup() =>
            Schedule(() => Child = advancedStats = new TestAdvancedStats { Width = 500 });

        private BeatmapInfo exampleBeatmapInfo =>
            new BeatmapInfo
            {
        ...
        public void TestNoMod()
        {
            ...
            AddAssert(
                "first bar text is correct",
                () =>
                    advancedStats.ChildrenOfType<SpriteText>().First().Text
        ...
        {
            AddStep(
                "set ruleset to mania",
                () => advancedStats.Ruleset.Value = new ManiaRuleset().RulesetInfo
            );
            AddAssert(
                "first bar text is correct",
                () =>
            ...
            AddStep(
                "set ruleset to osu",
                () => advancedStats.Ruleset.Value = new OsuRuleset().RulesetInfo
            );
            AddAssert(
                "first bar text is correct",
                () =>
        ...
        public void TestUnchangedDifficultyAdjustMod()
        {
            ...
            AddStep(
                "select unchanged Difficulty Adjust mod",
                () =>
                {
                    var ruleset = advancedStats.BeatmapInfo.Ruleset.CreateInstance().AsNonNull();
        ...
        public void TestChangedDifficultyAdjustMod()
        {
            ...
            AddStep(
                "select changed Difficulty Adjust mod",
                () =>
                {
                    var ruleset = advancedStats.BeatmapInfo.Ruleset.CreateInstance().AsNonNull();
                    var difficultyAdjustMod = ruleset
                        .CreateMod<OsuModDifficultyAdjust>()
                        .AsNonNull();
        ...
        private bool barIsWhite(AdvancedStats.StatisticRow row) =>
            row.ModBar.AccentColour == Color4.White;

        private bool barIsBlue(AdvancedStats.StatisticRow row) =>
            row.ModBar.AccentColour == colours.BlueDark;

        private bool barIsRed(AdvancedStats.StatisticRow row) =>
            row.ModBar.AccentColour == colours.Red;
