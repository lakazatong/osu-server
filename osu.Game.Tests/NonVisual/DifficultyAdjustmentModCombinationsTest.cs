...
namespace osu.Game.Tests.NonVisual
{
    ...
    public class DifficultyAdjustmentModCombinationsTest
    {
        ...
        {
            var combinations =
                new TestLegacyDifficultyCalculator().CreateDifficultyAdjustmentModCombinations();

            assertCombinations(new[] { new[] { typeof(ModNoMod) } }, combinations);
        }

        [Test]
        public void TestSingleMod()
        {
            var combinations = new TestLegacyDifficultyCalculator(
                new ModA()
            ).CreateDifficultyAdjustmentModCombinations();

            assertCombinations(
                new[] { new[] { typeof(ModNoMod) }, new[] { typeof(ModA) } },
                combinations
            );
        }
        ...
        public void TestMultiModFlattening()
        {
            var combinations = new TestLegacyDifficultyCalculator(
                new ModA(),
                new MultiMod(new ModB(), new ModC())
            ).CreateDifficultyAdjustmentModCombinations();
        ...
        public void TestIncompatibleThroughMultiMod()
        {
            var combinations = new TestLegacyDifficultyCalculator(
                new ModA(),
                new MultiMod(new ModB(), new ModIncompatibleWithA())
            ).CreateDifficultyAdjustmentModCombinations();
        ...
        public void TestIncompatibleWithSameInstanceViaMultiMod()
        {
            var combinations = new TestLegacyDifficultyCalculator(
                new ModA(),
                new MultiMod(new ModA(), new ModB())
            ).CreateDifficultyAdjustmentModCombinations();
        ...
        private void assertCombinations(Type[][] expectedCombinations, Mod[] actualCombinations)
        {
            ...
            Assert.Multiple(() =>
            {
                for (int i = 0; i < expectedCombinations.Length; ++i)
                {
                    ...
                    Type[] actualTypes = ModUtils
                        .FlattenMod(actualCombinations[i])
                        .Select(m => m.GetType())
                        .ToArray();
        ...
        private class ModIncompatibleWithAAndB : Mod
        {
            ...
            public override string Acronym =>
                $"Incompatible With {nameof(ModA)} and {nameof(ModB)}";
            public override LocalisableString Description => string.Empty;
            public override double ScoreMultiplier => 1;
