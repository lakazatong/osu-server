    ...
    public class RealmKeyBindingStoreTest
    {
        ...
        public void TestBindingsWithoutDuplicatesAreNotModified()
        {
            var bindings = new List<RealmKeyBinding>
            {
                ...
                new RealmKeyBinding(
                    GlobalAction.Back,
                    KeyCombination.FromMouseButton(MouseButton.Button1)
                ),
            ...
            Assert.Multiple(() =>
            {
                ...
                Assert.That(
                    bindings[0].KeyCombination,
                    Is.EqualTo(new KeyCombination(InputKey.Escape))
                );

                Assert.That(bindings[1].Action, Is.EqualTo((int)GlobalAction.Back));
                Assert.That(
                    bindings[1].KeyCombination,
                    Is.EqualTo(new KeyCombination(InputKey.ExtraMouseButton1))
                );

                Assert.That(bindings[2].Action, Is.EqualTo((int)GlobalAction.MusicPrev));
                Assert.That(
                    bindings[2].KeyCombination,
                    Is.EqualTo(new KeyCombination(InputKey.F1))
                );

                Assert.That(bindings[3].Action, Is.EqualTo((int)GlobalAction.MusicNext));
                Assert.That(
                    bindings[3].KeyCombination,
                    Is.EqualTo(new KeyCombination(InputKey.F5))
                );
            });
        }
        ...
        public void TestDuplicateBindingsAreCleared()
        {
            ...
            Assert.Multiple(() =>
            {
                ...
                Assert.That(
                    bindings[0].KeyCombination,
                    Is.EqualTo(new KeyCombination(InputKey.None))
                );
