...
namespace osu.Game.Tests.Editing.Checks
{
    ...
    public class CheckUnsnappedObjectsTest
    {
        ...
        {
            assertOk(new List<HitObject> { new HitCircle { StartTime = 100 } });
        }

        [Test]
        public void TestCircleUnsnapped1Ms()
        {
            assert1Ms(new List<HitObject> { new HitCircle { StartTime = 101 } });

            assert1Ms(new List<HitObject> { new HitCircle { StartTime = 99 } });
        }

        [Test]
        public void TestCircleUnsnapped2Ms()
        {
            assert2Ms(new List<HitObject> { new HitCircle { StartTime = 102 } });

            assert2Ms(new List<HitObject> { new HitCircle { StartTime = 98 } });
        }
        ...
        public void TestSliderSnapped()
        {
            ...
            assertOk(
                new List<HitObject> { getSliderMock(startTime: 100, endTime: 400.75d).Object }
            );
        }
        ...
        {
            assert1Ms(
                new List<HitObject> { getSliderMock(startTime: 101, endTime: 401.75d).Object },
                count: 2
            );

            // End is only off by 0.25 ms, hence count 1.
            assert1Ms(
                new List<HitObject> { getSliderMock(startTime: 99, endTime: 399.75d).Object },
                count: 1
            );
        }
        ...
        {
            assert2Ms(
                new List<HitObject> { getSliderMock(startTime: 102, endTime: 402.75d).Object },
                count: 2
            );
            ...
            Assert.That(
                issues.Any(issue =>
                    issue.Template is CheckUnsnappedObjects.IssueTemplateSmallUnsnap
                )
            );
            Assert.That(
                issues.Any(issue =>
                    issue.Template is CheckUnsnappedObjects.IssueTemplateLargeUnsnap
                )
            );
        }
