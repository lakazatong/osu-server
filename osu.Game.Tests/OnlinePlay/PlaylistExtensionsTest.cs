...
namespace osu.Game.Tests.OnlinePlay
{
    ...
    public class PlaylistExtensionsTest
    {
        ...
        public void TestPlaylistItemsOutOfOrder()
        {
            ...
            Assert.Multiple(() =>
            {
                ...
                Assert.That(
                    ...
                    Is.EquivalentTo(new[] { items[1], items[0], items[2] })
                );
            });
        }
        ...
        public void TestExpiredPlaylistItemsSkipped()
        {
            var items = new[]
            {
                new PlaylistItem(new APIBeatmap { OnlineID = 1001 })
                {
                    ID = 1,
                    Expired = true,
                    PlayedAt = new DateTimeOffset(2021, 12, 21, 7, 55, 0, TimeSpan.Zero),
                },
                new PlaylistItem(new APIBeatmap { OnlineID = 1002 })
                {
                    ID = 2,
                    Expired = true,
                    PlayedAt = new DateTimeOffset(2021, 12, 21, 7, 53, 0, TimeSpan.Zero),
                },
                new PlaylistItem(new APIBeatmap { OnlineID = 1003 }) { ID = 3, PlaylistOrder = 3 },
            };
            ...
            {
                Assert.That(
                    items.GetHistoricalItems(),
                    Is.EquivalentTo(new[] { items[1], items[0] })
                );
        ...
        public void TestAllItemsExpired()
        {
            var items = new[]
            {
                ...
                new PlaylistItem(new APIBeatmap { OnlineID = 1002 })
                {
                    ID = 3,
                    Expired = true,
                    PlayedAt = new DateTimeOffset(2021, 12, 21, 7, 57, 0, TimeSpan.Zero),
                },
            };
