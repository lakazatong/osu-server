    ...
    public class MultiplayerPlaylistItemTest
    {
        ...
        public void TestCloneMultiplayerPlaylistItem()
        {
            var faker = new Faker<MultiplayerPlaylistItem>()
                ...
                .RuleFor(
                    o => o.RequiredMods,
                    f => f.Make(5, _ => new APIMod { Acronym = f.Random.String2(3) })
                )
                .RuleFor(
                    o => o.AllowedMods,
                    f => f.Make(5, _ => new APIMod { Acronym = f.Random.String2(3) })
            ...
            for (int i = 0; i < 100; i++)
            {
                ...
                Assert.That(
                    MessagePackSerializer.SerializeToJson(item.Clone()),
                    Is.EqualTo(MessagePackSerializer.SerializeToJson(item))
                );
            }
        }
        ...
        public void TestConstructFromAPIModel()
        {
            ...
            for (int i = 0; i < 100; i++)
            {
                ...
                MultiplayerPlaylistItem copiedItem = new MultiplayerPlaylistItem(
                    new PlaylistItem(initialItem)
                );
                Assert.That(
                    MessagePackSerializer.SerializeToJson(copiedItem),
                    Is.EqualTo(MessagePackSerializer.SerializeToJson(initialItem))
                );
            }
        }
    }
