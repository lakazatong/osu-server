...
namespace osu.Game.Rulesets.Osu.Skinning.Legacy
{
    public abstract partial class LegacySpinner : CompositeDrawable, IHasApproachCircle
    {
        ...
        protected override void LoadComplete()
        {
            ...
            completedSpins.BindValueChanged(bonus =>
            {
                ...
                else
                {
                    ...
                    bonusCounter
                        .ScaleTo(SPRITE_SCALE * 2f)
                        .Then()
                        .ScaleTo(SPRITE_SCALE * 1.28f, 800, Easing.Out);
                }
            });
        ...
        private void onCompletedChanged(ValueChangedEvent<bool> completed)
        {
            if (completed.NewValue)
            {
                ...
                using (BeginAbsoluteSequence(startTime))
                {
                    ...
                    clear
                        .ScaleTo(SPRITE_SCALE * 2)
                        .Then()
                        .ScaleTo(SPRITE_SCALE * 0.8f, 240, Easing.Out)
                        .Then()
                        .ScaleTo(SPRITE_SCALE, 160);
                }

                const double fade_out_duration = 50;
                using (
                    BeginAbsoluteSequence(DrawableSpinner.HitStateUpdateTime - fade_out_duration)
                )
        ...
        protected virtual void UpdateStateTransforms(
        ...
        )
        {
            switch (drawableHitObject)
            {
                case DrawableSpinner d:
                    ...
                    {
                        spmBackground.MoveToOffset(
                            new Vector2(0, -spm_hide_offset),
                            d.HitObject.TimeFadeIn,
                            Easing.Out
                        );
                        spmCounter.MoveToOffset(
                            new Vector2(0, -spm_hide_offset),
                            d.HitObject.TimeFadeIn,
                            Easing.Out
                        );
                    ...
                    using (
                        BeginAbsoluteSequence(d.HitObject.StartTime - d.HitObject.TimeFadeIn / 2)
                    )
                    ...
                    using (
                        BeginAbsoluteSequence(
                            drawableHitObject.HitStateUpdateTime - spinFadeOutLength
                        )
                    )
