...
{
    public partial class OsuModFlashlight
        : ModFlashlight<OsuHitObject>,
            IApplicableToDrawableHitObject
    {
        public override double ScoreMultiplier => UsesDefaultConfiguration ? 1.12 : 1;
        public override Type[] IncompatibleMods =>
            base
                .IncompatibleMods.Concat(new[] { typeof(OsuModBloom), typeof(OsuModBlinds) })
                .ToArray();

        private const double default_follow_delay = 120;

        [SettingSource("Follow delay", "Milliseconds until the flashlight reaches the cursor")]
        public BindableNumber<double> FollowDelay { get; } =
            new BindableDouble(default_follow_delay)
            {
        ...
        private partial class OsuFlashlight : Flashlight, IRequireHighFrequencyMousePosition
        {
            ...
            public void OnSliderTrackingChange(DrawableSlider e)
            {
                ...
                FlashlightDim =
                    Time.Current >= e.HitObject.StartTime && e.Tracking.Value ? 0.8f : 0.0f;
            }
            ...
            protected override bool OnMouseMove(MouseMoveEvent e)
            {
                ...
                FlashlightPosition = Interpolation.ValueAt(
                    Math.Min(Math.Abs(Clock.ElapsedFrameTime), followDelay),
                    position,
                    destination,
                    0,
                    followDelay,
