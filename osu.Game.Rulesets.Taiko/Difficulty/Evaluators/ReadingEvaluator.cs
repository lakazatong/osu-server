...
namespace osu.Game.Rulesets.Taiko.Difficulty.Evaluators
{
    public static class ReadingEvaluator
    {
        ...
        public static double EvaluateDifficultyOf(TaikoDifficultyHitObject noteObject)
        {
            ...
            double midVelocityDifficulty =
                0.5
                * DifficultyCalculationUtils.Logistic(
                    effectiveBPM,
                    midVelocity.Center,
                    1.0 / (midVelocity.Range / 10)
                );
            ...
            double highVelocityDifficulty =
                (1.0 - 0.33 * densityPenalty)
                * DifficultyCalculationUtils.Logistic(
                    effectiveBPM,
                    highVelocity.Center + 8 * densityPenalty,
                    (1.0 + 0.5 * densityPenalty) / (highVelocity.Range / 10)
                );
