...
namespace osu.Game.Rulesets.Taiko.Beatmaps
{
    internal class TaikoBeatmapConverter : BeatmapConverter<TaikoHitObject>
    {
        ...
        protected override Beatmap<TaikoHitObject> ConvertBeatmap(
            IBeatmap original,
        ...
        )
        {
            ...
            if (original.BeatmapInfo.Ruleset.OnlineID == 0)
            {
                ...
                {
                    if (hitObject is not IHasSliderVelocity hasSliderVelocity)
                        continue;

                    double nextScrollSpeed = hasSliderVelocity.SliderVelocityMultiplier;
                    EffectControlPoint currentEffectPoint =
                        converted.ControlPointInfo.EffectPointAt(hitObject.StartTime);

                    if (
                        !Precision.AlmostEquals(
                            lastScrollSpeed,
                            nextScrollSpeed,
                            acceptableDifference: currentEffectPoint.ScrollSpeedBindable.Precision
                        )
                    )
                    {
            ...
            if (original.BeatmapInfo.Ruleset.OnlineID == 3)
            {
                ...
                converted.HitObjects = converted
                    .HitObjects.GroupBy(t => t.StartTime)
                    .Select(x =>
                    {
        ...
        protected override IEnumerable<TaikoHitObject> ConvertHitObject(
            HitObject obj,
            IBeatmap beatmap,
            CancellationToken cancellationToken
        )
            ...
            switch (obj)
            {
                case IHasPath pathData:
                {
                    if (
                        shouldConvertSliderToHits(
                            ...
                            beatmap,
                            pathData,
                            out int taikoDuration,
                            out double tickSpacing
                        )
                    )
                    {
                        IList<IList<HitSampleInfo>> allSamples = obj
                            is IHasPathWithRepeats curveData
                            ? curveData.NodeSamples
                            : new List<IList<HitSampleInfo>>(new[] { samples });

                        int i = 0;

                        for (
                            double j = obj.StartTime;
                            j <= obj.StartTime + taikoDuration + tickSpacing / 8;
                            j += tickSpacing
                        )
                        {
                            IList<HitSampleInfo> currentSamples = allSamples[i];

                            yield return new Hit { StartTime = j, Samples = currentSamples };
                ...
                case IHasDuration endTimeData:
                {
                    ...
                    yield return new Swell
                    {
                        ...
                        RequiredHits = (int)
                            Math.Max(1, endTimeData.Duration / 1000 * hitMultiplier),
                    };
                ...
                default:
                {
                    yield return new Hit { StartTime = obj.StartTime, Samples = samples };
        ...
        private bool shouldConvertSliderToHits(
            HitObject obj,
            IBeatmap beatmap,
            IHasPath pathData,
            out int taikoDuration,
        ...
        )
        {
            ...
            if (obj is IHasSliderVelocity hasSliderVelocity)
                beatLength = LegacyRulesetExtensions.GetPrecisionAdjustedBeatLength(
                    hasSliderVelocity,
                    timingPoint,
                    TaikoRuleset.SHORT_NAME
                );
            else
            ...
            tickSpacing = Math.Min(
                beatLength / beatmap.Difficulty.SliderTickRate,
                (double)taikoDuration / spans
            );

            return tickSpacing > 0 && distance / osuVelocity * 1000 < 2 * beatLength;
        }
