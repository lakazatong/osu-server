    ...
    public partial class DataLoadTest : TournamentHostTest
    {
        ...
        public void TestRulesetGetsValidOnlineID()
        {
            using (HeadlessGameHost host = new CleanRunHeadlessGameHost())
            {
                try
                {
                    ...
                    {
                        var storage = host.Storage.GetStorageForDirectory(
                            Path.Combine("tournaments", "default")
                        );
                    ...
                    Assert.That(
                        osu.Dependencies.Get<IBindable<RulesetInfo>>().Value.OnlineID,
                        Is.EqualTo(1)
                    );
                }
                finally
        ...
        public partial class TestTournament : TournamentGameBase
        {
            ...
            public TestTournament(
                bool resetRuleset = false,
                [InstantHandle] Action? runOnLoadComplete = null
            )
            {
