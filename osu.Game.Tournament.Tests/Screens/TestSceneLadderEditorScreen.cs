    ...
    public partial class TestSceneLadderEditorScreen : TournamentScreenTestScene
    {
        ...
        public void Setup() =>
            Schedule(() =>
            {
                ...
                Add(
                    osuContextMenuContainer = new OsuContextMenuContainer
                    {
        ...
        public void TestResetBracketTeamsCancelled()
        {
            ...
            AddStep(
                "click Reset teams button",
                () =>
                {
                    InputManager.MoveMouseTo(
                        osuContextMenuContainer
                            .ChildrenOfType<DrawableOsuMenuItem>()
                            .Last(p => ((OsuMenuItem)p.Item).Type == MenuItemType.Destructive),
                        new Vector2(5, 0)
                    );
            ...
            AddAssert(
                "dialog displayed",
                () => DialogOverlay.CurrentDialog is LadderResetTeamsDialog
            );
            AddStep(
                "click cancel",
                () =>
            ...
            AddUntilStep(
                "dialog dismissed",
                () => DialogOverlay.CurrentDialog is not LadderResetTeamsDialog
            );

            AddAssert(
                "assert ladder teams unchanged",
                () =>
                    string.Equals(
                        matchBeforeReset.Value,
                        JsonConvert.SerializeObject(Ladder.CurrentMatch.Value),
        ...
        public void TestResetBracketTeams()
        {
            ...
            AddAssert(
                "assert ladder teams reset",
                () =>
                    Ladder.CurrentMatch.Value?.Team1.Value == null
                    && Ladder.CurrentMatch.Value?.Team2.Value == null
            );
        }
    }
}
