...
namespace osu.Game.Rulesets.Catch.Tests
{
    ...
    public partial class TestSceneCatcher : OsuTestScene
    {
        ...
        public void SetUp() =>
            Schedule(() =>
            {
                var difficulty = new BeatmapDifficulty { CircleSize = 0 };
        ...
        {
            float halfWidth =
                Catcher.CalculateCatchWidth(new BeatmapDifficulty { CircleSize = 0 }) / 2;
        ...
        public void TestFruitClampedToCatchableRegion()
        {
            AddStep(
                "catch fruit left",
                () => attemptCatch(new Fruit { X = -CatchPlayfield.WIDTH })
            );
            checkPlate(1);
            AddStep("move catcher to right", () => catcher.X = CatchPlayfield.WIDTH);
            AddStep(
                "catch fruit right",
                () => attemptCatch(new Fruit { X = CatchPlayfield.WIDTH * 2 })
            );
            checkPlate(2);
        }
        ...
        public void TestNormalFruitResetsHyperDashState()
        {
            AddStep(
                "catch hyper fruit",
                () => attemptCatch(new Fruit { HyperDashTarget = new Fruit { X = 100 } })
            );
            checkHyperDash(true);
        ...
        {
            AddStep(
                "catch hyper kiai fruit",
                () => attemptCatch(new TestKiaiFruit { HyperDashTarget = new Fruit { X = 100 } })
            );
        ...
        public void TestLastBananaShouldClearPlateOnMiss()
        {
            ...
            AddStep(
                "miss last banana",
                () => attemptCatch(new Banana { LastInCombo = true, X = 100 })
            );
            checkPlate(0);
        }
                    ...
                    attemptCatch(
                        () =>
                            new Fruit
                            {
                                X =
                                    (RNG.NextSingle() - 0.5f)
                                    * Catcher.CalculateCatchWidth(Vector2.One),
                            },
                        50
                    )
            );
        ...
        public void TestCatcherExplosionAndDropping()
        {
            ...
            AddStep("catch tiny droplet", () => attemptCatch(new TinyDroplet()));
            AddAssert(
                "tiny droplet is exploded",
                () => catcher.CaughtObjects.Count() == 1 && droppedObjectContainer.Count == 1
            );
            ...
            AddAssert(
                "fruits are exploded",
                () => !catcher.CaughtObjects.Any() && droppedObjectContainer.Count == 10
            );
            AddUntilStep("wait explosion", () => !droppedObjectContainer.Any());
        ...
        public void TestHitLightingColour()
        {
            ...
            AddStep("catch fruit", () => attemptCatch(new Fruit()));
            AddAssert(
                "correct hit lighting colour",
                () =>
                    catcher.ChildrenOfType<HitExplosion>().First()?.Entry?.ObjectColour
                    == this.ChildrenOfType<DrawableCatchHitObject>().First().AccentColour.Value
            );
        }
        ...
        public void TestAllExplodedObjectsAtUniquePositions()
        {
            ...
            AddAssert(
                "two fruit at distinct x coordinates",
                () => this.ChildrenOfType<CaughtFruit>().Select(f => f.DrawPosition.X).Distinct(),
                () => Has.Exactly(2).Items
            );
        }

        private void checkPlate(int count) =>
            AddAssert(
                $"{count} objects on the plate",
                () => catcher.CaughtObjects.Count() == count
            );

        private void checkState(CatcherAnimationState state) =>
            AddAssert($"catcher state is {state}", () => catcher.CurrentState == state);

        private void checkHyperDash(bool state) =>
            AddAssert(
                $"catcher is {(state ? "" : "not ")}hyper dashing",
                () => catcher.HyperDashing == state
            );
        ...
        public partial class TestCatcher : Catcher
        {
            ...
            public TestCatcher(
                DroppedObjectContainer droppedObjectTarget,
