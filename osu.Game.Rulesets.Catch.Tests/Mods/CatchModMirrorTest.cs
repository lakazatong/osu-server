    ...
    public class CatchModMirrorTest
    {
        ...
        private static IBeatmap createRawBeatmap() =>
            new Beatmap
            {
                ...
                {
                    new Fruit { OriginalX = 150, StartTime = 0 },
                    new Fruit { OriginalX = 450, StartTime = 500 },
                    new JuiceStream
                    {
                        OriginalX = 250,
                        Path = new SliderPath
                    ...
                    new BananaShower { StartTime = 5000, Duration = 5000 },
                },
            };
        ...
        private static void assertEffectivePositionsMirrored(IBeatmap original, IBeatmap mirrored)
        {
            if (original.HitObjects.Count != mirrored.HitObjects.Count)
                Assert.Fail(
                    $"Top-level object count mismatch (original: {original.HitObjects.Count}, mirrored: {mirrored.HitObjects.Count})"
                );
            ...
            for (int i = 0; i < original.HitObjects.Count; ++i)
            {
                ...
                if (
                    !effectivePositionMirrored(originalObject, mirroredObject)
                    && !(originalObject is BananaShower)
                )
                    Assert.Fail(
                        $"{originalObject.GetType().Name} at time {originalObject.StartTime} is not mirrored ({printEffectivePositions(originalObject, mirroredObject)})"
                    );

                if (originalObject.NestedHitObjects.Count != mirroredObject.NestedHitObjects.Count)
                    Assert.Fail(
                        $"{originalObject.GetType().Name} nested object count mismatch (original: {originalObject.NestedHitObjects.Count}, mirrored: {mirroredObject.NestedHitObjects.Count})"
                    );
        ...
        private static string printEffectivePositions(
            CatchHitObject original,
            CatchHitObject mirrored
        ) =>
        ...
        private static bool effectivePositionMirrored(
            CatchHitObject original,
            CatchHitObject mirrored
        ) =>
