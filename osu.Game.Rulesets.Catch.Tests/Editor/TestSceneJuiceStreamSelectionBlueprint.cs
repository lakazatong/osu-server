...
namespace osu.Game.Rulesets.Catch.Tests.Editor
{
    public partial class TestSceneJuiceStreamSelectionBlueprint : CatchSelectionBlueprintTestScene
    {
        ...
        public void TestBasicComponentLayout()
        {
            ...
            AddAssert(
                "correct vertex piece count",
                () => this.ChildrenOfType<VertexPiece>().Count() == times.Length
            );

            AddAssert(
                "first vertex is semitransparent",
                () =>
        ...
        public void TestSliderVelocityChange()
        {
            ...
            AddAssert(
                ...
                () => hitObject.SliderVelocityMultiplierBindable.IsDefault
            );
        ...
        public void TestUpdateFromHitObject()
        {
            ...
            AddStep(
                "update hit object path",
                () =>
                {
                    hitObject.Path = new SliderPath(
                        PathType.PERFECT_CURVE,
                        new[] { Vector2.Zero, new Vector2(100, 100), new Vector2(0, 200) }
                    );
        ...
        public void TestAddVertex()
        {
            ...
            AddAssert(
                "duration is changed",
                () => Precision.AlmostEquals(hitObject.Duration, 800 - times[0], 1e-3)
            );
        }
        ...
        public void TestVertexResampling()
        {
            addBlueprintStep(
                ...
                new SliderPath(
                    PathType.PERFECT_CURVE,
                    new[] { Vector2.Zero, new Vector2(100, 100), new Vector2(50, 200) }
                ),
                0.5
            );
            AddAssert(
                "1 vertex per 1 nested HO",
                () => getVertices().Count == hitObject.NestedHitObjects.Count
            );
            AddAssert(
                "slider path not yet changed",
                () => hitObject.Path.ControlPoints[0].Type == PathType.PERFECT_CURVE
            );
            addAddVertexSteps(150, 150);
            AddAssert(
                "slider path change to linear",
                () => hitObject.Path.ControlPoints[0].Type == PathType.LINEAR
            );
        }

        private void addBlueprintStep(
            double time,
            float x,
        ...
        private IReadOnlyList<JuiceStreamPathVertex> getVertices() =>
            this.ChildrenOfType<EditablePath>().Single().Vertices;

        private void addVertexCheckStep(int count, int index, double time, float x) =>
            AddAssert(
                $"vertex {index} of {count} at {time}, {x}",
                () =>
                {
