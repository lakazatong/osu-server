...
namespace osu.Game.Rulesets.Pippidon.Beatmaps
{
    public class PippidonBeatmapConverter : BeatmapConverter<PippidonHitObject>
    {
        ...
        public override bool CanConvert() =>
            Beatmap.HitObjects.All(h => h is IHasXPosition && h is IHasYPosition);
        ...
        private int getLane(HitObject hitObject) =>
            (int)
                Math.Clamp(
                    (getUsablePosition(hitObject) - minPosition)
                        / (maxPosition - minPosition)
                        * PippidonPlayfield.LANE_COUNT,
                    0,
                    PippidonPlayfield.LANE_COUNT - 1
                );

        private float getUsablePosition(HitObject h) =>
            (h as IHasYPosition)?.Y ?? ((IHasXPosition)h).X;
    }
}
