...
namespace osu.Game.Rulesets.Mania.Skinning.Legacy
{
    public partial class LegacyHitExplosion : LegacyManiaColumnElement, IHitExplosion
    {
        ...
        private void load(ISkinSource skin, IScrollingInfo scrollingInfo)
        {
            string imageName =
                GetColumnSkinConfig<string>(
                    skin,
                    LegacyManiaSkinConfigurationLookups.ExplosionImage
                )?.Value ?? "lightingN";

            float explosionScale =
                GetColumnSkinConfig<float>(
                    skin,
                    LegacyManiaSkinConfigurationLookups.ExplosionScale
                )?.Value ?? 1;
            ...
            explosion = skin.GetAnimation(imageName, true, false, frameLength: frameLength)
                ?.With(d =>
                {
        ...
        public void Animate(JudgementResult result)
        {
            ...
            explosion?.FadeInFromZero(FADE_IN_DURATION).Then().FadeOut(120);
        }
    }
}
