...
namespace osu.Game.Rulesets.Osu.Tests
{
    public partial class TestSceneResume : PlayerTestScene
    {
        ...
        protected override TestPlayer CreatePlayer(Ruleset ruleset) =>
            new TestPlayer(true, false, AllowBackwardsSeeks);
        ...
        public void TestPauseViaKeyboard()
        {
            ...
            AddUntilStep(
                "wait for pause overlay",
                () => Player.ChildrenOfType<PauseOverlay>().Single().State.Value,
            ...
            AddUntilStep(
                "pause overlay present",
                () => Player.DrawableRuleset.ResumeOverlay.State.Value,
                () => Is.EqualTo(Visibility.Visible)
            );
        }
        ...
        {
            AddStep(
                "move mouse outside playfield",
                () =>
                    InputManager.MoveMouseTo(
                        Player.DrawableRuleset.Playfield.ScreenSpaceDrawQuad.BottomRight
                            + new Vector2(1)
                    )
            );
            AddUntilStep("wait for gameplay start", () => Player.LocalUserPlaying.Value);
        ...
        public void TestPauseViaKeyboardWhenMouseOutsideScreen()
        {
            AddStep(
                "move mouse outside playfield",
                () => InputManager.MoveMouseTo(new Vector2(-20))
            );
            AddUntilStep("wait for gameplay start", () => Player.LocalUserPlaying.Value);
            AddStep("press escape", () => InputManager.PressKey(Key.Escape));
            AddUntilStep(
