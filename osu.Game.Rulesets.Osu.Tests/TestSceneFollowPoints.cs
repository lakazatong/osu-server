    ...
    public partial class TestSceneFollowPoints : OsuTestScene
    {
        ...
        public void Setup() =>
            Schedule(() =>
            {
                ...
                {
                    hitObjectContainer = new TestHitObjectContainer
                    {
        ...
        {
            addObjectsStep(() =>
                new OsuHitObject[] { new HitCircle { Position = new Vector2(100, 100) } }
            );
        ...
        public void TestStackedObjects()
        {
            addObjectsStep(() =>
                new OsuHitObject[]
                {
                    ...
                    new HitCircle { Position = new Vector2(300, 300), StackHeight = 20 },
                }
            );
        ...
        private void addMultipleObjectsStep() =>
            addObjectsStep(() =>
                new OsuHitObject[]
                {
        ...
        {
            AddStep(
                $"move object {startIndex} to {endIndex}",
                () =>
                {
                    ...
                    if (endIndex < hitObjectContainer.Count)
                        ...
                        targetTime =
                            getObject(hitObjectContainer.Count - 1).HitObject.StartTime + 1;
        ...
        {
            AddAssert(
                "has correct group count",
                () => followPointRenderer.Entries.Count == hitObjectContainer.Count
            );
            AddAssert(
                "group endpoints are correct",
                () =>
                {
                    for (int i = 0; i < hitObjectContainer.Count; i++)
                    {
                        DrawableOsuHitObject expectedStart = getObject(i);
                        DrawableOsuHitObject expectedEnd =
                            i < hitObjectContainer.Count - 1 ? getObject(i + 1) : null;

                        if (getEntry(i).Start != expectedStart.HitObject)
                            throw new AssertionException(
                                $"Object {i} expected to be the start of group {i}."
                            );

                        if (getEntry(i).End != expectedEnd?.HitObject)
                            throw new AssertionException(
                                $"Object {(expectedEnd == null ? "null" : i.ToString())} expected to be the end of group {i}."
                            );
                    }
            ...
            AddAssert(
                ...
                () =>
                {
                    for (int i = 0; i < hitObjectContainer.Count; i++)
                    {
                        ...
                        float expectedDirection = MathF.Atan2(
                            expectedStart.Position.Y - expectedEnd.Position.Y,
                            expectedStart.Position.X - expectedEnd.Position.X
                        );
                        float realDirection = MathF.Atan2(
                            expectedStart.Position.Y - points[^1].Position.Y,
                            expectedStart.Position.X - points[^1].Position.X
                        );

                        if (!Precision.AlmostEquals(expectedDirection, realDirection))
                            throw new AssertionException(
                                $"Expected group {i} in direction {expectedDirection}, but was {realDirection}."
                            );
                    }
        ...
        private FollowPointConnection getGroup(int index) =>
            followPointRenderer
                .ChildrenOfType<FollowPointConnection>()
                .Single(c => c.Entry == getEntry(index));
