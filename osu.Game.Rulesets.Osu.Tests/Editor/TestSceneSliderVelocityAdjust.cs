...
namespace osu.Game.Rulesets.Osu.Tests.Editor
{
    ...
    {
        private Screens.Edit.Editor? editor =>
            Game.ScreenStack.CurrentScreen as Screens.Edit.Editor;

        private EditorBeatmap editorBeatmap =>
            editor.ChildrenOfType<EditorBeatmap>().FirstOrDefault()!;
        ...
        private TimelineHitObjectBlueprint blueprint =>
            editor.ChildrenOfType<TimelineHitObjectBlueprint>().FirstOrDefault()!;

        private DifficultyPointPiece difficultyPointPiece =>
            blueprint.ChildrenOfType<DifficultyPointPiece>().First();

        private IndeterminateSliderWithTextBoxInput<double> velocityTextBox =>
            Game.ChildrenOfType<DifficultyPointPiece.DifficultyEditPopover>()
                .First()
        ...
        public void TestVelocityChangeSavesCorrectly(bool adjustVelocity)
        {
            ...
            AddStep(
                "seek to first control point",
                () => editorClock.Seek(editorBeatmap.ControlPointInfo.TimingPoints.First().Time)
            );
            ...
            AddStep(
                ...
                () =>
                    InputManager.MoveMouseTo(
                        editor.ChildrenOfType<Playfield>().First().ScreenSpaceDrawQuad.Centre
                    )
            );
            ...
            AddStep(
                ...
                () =>
                    InputManager.MoveMouseTo(
                        editor.ChildrenOfType<Playfield>().First().ScreenSpaceDrawQuad.BottomRight
                            - new Vector2(10)
                    )
            );
        ...
        public void TestVelocityUndo()
        {
            ...
            AddAssert(
                "velocity adjusted",
                () => slider!.Velocity,
                () => Is.EqualTo(velocityBefore!.Value * 2).Within(Precision.DOUBLE_EPSILON)
            );
            ...
            AddAssert(
                "slider has correct duration",
                () => slider!.Duration,
                () => Is.EqualTo(durationBefore)
            );
        }
    }
}
