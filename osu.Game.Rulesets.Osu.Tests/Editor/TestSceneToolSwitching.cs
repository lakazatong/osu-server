    ...
    public partial class TestSceneToolSwitching : EditorTestScene
    {
        ...
        public void TestSliderAnchorMoveOperationEndsOnSwitchingTool()
        {
            ...
            AddStep(
                ...
                () =>
                    initialPosition = EditorBeatmap
                        .HitObjects.OfType<Slider>()
                        .First()
                        .Path.ControlPoints.ElementAt(1)
                        .Position
            );
            AddStep(
                "select first slider",
                () =>
                    EditorBeatmap.SelectedHitObjects.Add(
                        EditorBeatmap.HitObjects.OfType<Slider>().First()
                    )
            );
            AddStep(
            ...
            AddStep(
                ...
                () =>
                    InputManager.MoveMouseTo(
                        InputManager.CurrentState.Mouse.Position + new Vector2(0, -200)
                    )
            );
            AddStep("switch tool", () => InputManager.PressButton(MouseButton.Button1));
            AddStep("undo", () => Editor.Undo());
            AddAssert(
                "anchor back at original position",
                () =>
                    EditorBeatmap
                        .HitObjects.OfType<Slider>()
                        .First()
        ...
        public void TestSliderAnchorCreationOperationEndsOnSwitchingTool()
        {
            ...
            AddStep("undo", () => Editor.Undo());
            AddAssert(
                "slider has three anchors again",
                () => EditorBeatmap.HitObjects.OfType<Slider>().First().Path.ControlPoints,
                () => Has.Count.EqualTo(3)
            );
        }
    }
