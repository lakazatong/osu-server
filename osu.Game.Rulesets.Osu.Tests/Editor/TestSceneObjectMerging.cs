...
namespace osu.Game.Rulesets.Osu.Tests.Editor
{
    ...
    {
        private OsuSelectionHandler selectionHandler =>
            Editor.ChildrenOfType<OsuSelectionHandler>().First();
        ...
        public void TestSimpleMerge()
        {
            ...
            AddStep(
                "select first two circles",
                () =>
                {
                    circle1 = (HitCircle)EditorBeatmap.HitObjects.First(h => h is HitCircle);
                    circle2 = (HitCircle)
                        EditorBeatmap.HitObjects.First(h => h is HitCircle && h != circle1);
            ...
            AddAssert(
                ...
                () => selectionHandler.ContextMenuItems.Any(o => o.Text.Value == "Merge selection")
            );
            ...
            AddAssert(
                "merged objects restored",
                () =>
                    circle1 is not null && circle2 is not null && objectsRestored(circle1, circle2)
            );
        }
        ...
        public void TestMergeCircleSlider()
        {
            ...
            AddStep(
                "select a circle, slider, circle",
                () =>
                {
                    circle1 = (HitCircle)EditorBeatmap.HitObjects.First(h => h is HitCircle);
                    slider = (Slider)
                        EditorBeatmap.HitObjects.First(h =>
                            h is Slider && h.StartTime > circle1.StartTime
                        );
                    circle2 = (HitCircle)
                        EditorBeatmap.HitObjects.First(h =>
                            h is HitCircle && h.StartTime > slider.StartTime
                        );
                    EditorClock.Seek(circle1.StartTime);
                    EditorBeatmap.SelectedHitObjects.Add(circle1);
            ...
            AddAssert(
                ...
                () =>
                {
                    ...
                    {
                        args[i + 1] = (
                            controlPoints[i].Position + slider.Position,
                            i == controlPoints.Count - 1 ? PathType.LINEAR : controlPoints[i].Type
                        );
                    }
            ...
            AddAssert(
                ...
                () =>
                    ...
                    && circle2 is not null
                    && slider is not null
                    && objectsRestored(circle1, slider, circle2)
            );
        }
        ...
        public void TestMergeSliderSlider()
        {
            ...
            AddStep(
                ...
                () =>
                {
                    ...
                    slider1Path = new SliderPath(
                        slider1
                            .Path.ControlPoints.Select(p => new PathControlPoint(
                                p.Position,
                                p.Type
                            ))
                            .ToArray(),
                        slider1.Path.ExpectedDistance.Value
                    );
                    slider2 = (Slider)
                        EditorBeatmap.HitObjects.First(h =>
                            h is Slider && h.StartTime > slider1.StartTime
                        );
            ...
            AddAssert(
                ...
                () =>
                {
                    ...
                    (Vector2, PathType?)[] args = new (Vector2, PathType?)[
                        controlPoints1.Count + controlPoints2.Count - 1
                    ];

                    for (int i = 0; i < controlPoints1.Count - 1; i++)
                    {
                        args[i] = (
                            controlPoints1[i].Position + slider1.Position,
                            controlPoints1[i].Type
                        );
                    }

                    for (int i = 0; i < controlPoints2.Count; i++)
                    {
                        args[i + controlPoints1.Count - 1] = (
                            controlPoints2[i].Position
                                + controlPoints1[^1].Position
                                + slider1.Position,
                            controlPoints2[i].Type
                        );
                    }
            ...
            AddAssert(
                "merged slider matches first slider",
                () =>
                {
                    var mergedSlider = (Slider)EditorBeatmap.SelectedHitObjects.First();
                    return slider1 is not null
                        && mergedSlider.HeadCircle.Samples.SequenceEqual(slider1.HeadCircle.Samples)
            ...
            AddAssert(
                "slider end is at same completion for last slider",
                () =>
                {
                    ...
                    return Precision.AlmostEquals(
                        mergedSlider.Path.Distance,
                        slider1Path.CalculatedDistance + slider2.Path.Distance
                    );
                }
            );
        }
        ...
        public void TestNonMerge()
        {
            ...
            AddStep(
                "select first two circles and spinner",
                () =>
                {
                    circle1 = (HitCircle)EditorBeatmap.HitObjects.First(h => h is HitCircle);
                    circle2 = (HitCircle)
        ...
        public void TestIllegalMerge()
        {
            ...
            AddStep(
                "add two circles on the same position",
                () =>
                {
            ...
            AddAssert(
                ...
                () =>
                    selectionHandler.ContextMenuItems.Length > 0
                    && selectionHandler.ContextMenuItems.All(o => o.Text.Value != "Merge selection")
            );
            mergeSelection();
            AddAssert(
                "circles not merged",
                () =>
                    circle1 is not null
                    && circle2 is not null
                    && EditorBeatmap.HitObjects.Contains(circle1)
                    && EditorBeatmap.HitObjects.Contains(circle2)
            );
        }
        ...
        public void TestMergeSliderSliderSameStartTime()
        {
            ...
            AddStep(
                "move sliders to the same start time",
                () =>
                {
        ...
        public void TestMergeSliderSliderSameStartAndEndTime()
        {
            ...
            AddStep(
                "move sliders to the same start & end time",
                () =>
                {
                    slider2!.StartTime = slider1!.StartTime;
        ...
        )
        {
            ...
            foreach ((Vector2 pos, PathType? pathType) in expectedControlPoints)
            {
                ...
                if (
                    !Precision.AlmostEquals(controlPoint.Position + mergedSlider.Position, pos)
        ...
        {
            AddStep(
                $"hover mouse over hit object {index}",
                () =>
                {
