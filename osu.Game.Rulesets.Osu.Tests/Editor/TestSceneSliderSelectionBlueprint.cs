...
namespace osu.Game.Rulesets.Osu.Tests.Editor
{
    public partial class TestSceneSliderSelectionBlueprint : SelectionBlueprintTestScene
    {
        ...
        public void Setup() =>
            Schedule(() =>
            {
                ...
                slider = new Slider
                {
                    ...
                    Path = new SliderPath(
                        PathType.BEZIER,
                        new[] { Vector2.Zero, new Vector2(150, 150), new Vector2(300, 0) }
                    ),
                };
        ...
        public void TestSingleControlPointDeselectionViaClickOutside()
        {
            ...
            AddStep(
                "move mouse outside control point",
                () => InputManager.MoveMouseTo(drawableObject)
            );
            AddStep("click", () => InputManager.Click(MouseButton.Left));
            checkControlPointSelected(0, false);
            checkControlPointSelected(1, false);
        ...
        {
            AddStep(
                "move mouse to drag marker",
                () =>
                {
                    Vector2 position =
                        slider.Position + slider.Path.PositionAt(1) + new Vector2(60, 0);
                    InputManager.MoveMouseTo(drawableObject.Parent!.ToScreenSpace(position));
                }
            );
            AddStep("start drag", () => InputManager.PressButton(MouseButton.Left));
            AddStep(
                "move mouse to control point 1",
                () =>
                {
                    Vector2 position =
                        slider.Position
                        + slider.Path.ControlPoints[1].Position
                        + new Vector2(60, 0);
                    InputManager.MoveMouseTo(drawableObject.Parent!.ToScreenSpace(position));
                }
            );
            ...
            AddStep("start drag", () => InputManager.PressButton(MouseButton.Left));
            AddStep(
                "move mouse beyond last control point",
                () =>
                {
                    Vector2 position =
                        slider.Position
                        + slider.Path.ControlPoints[2].Position
                        + new Vector2(100, 0);
                    InputManager.MoveMouseTo(drawableObject.Parent!.ToScreenSpace(position));
                }
            );
            AddStep("end adjust length", () => InputManager.ReleaseButton(MouseButton.Left));
            AddAssert(
                "expected distance is calculated distance",
                () =>
            ...
            moveMouseToControlPoint(1);
            AddAssert(
                "expected distance is unchanged",
                () =>
                    Precision.AlmostEquals(
                        slider.Path.Distance,
                        slider.Path.CalculatedDistance,
        ...
        private void checkPositions()
        {
            AddAssert(
                ...
                () => blueprint.BodyPiece.Position == slider.StackedPosition
            );

            AddAssert(
                "head positioned correctly",
                () =>
                    Precision.AlmostEquals(
                        blueprint.HeadOverlay.CirclePiece.ScreenSpaceDrawQuad.Centre,
                        drawableObject.HeadCircle.ScreenSpaceDrawQuad.Centre
                    )
            );
            ...
            AddAssert(
                ...
                () =>
                    Precision.AlmostEquals(
                        blueprint.TailOverlay.CirclePiece.ScreenSpaceDrawQuad.Centre,
                        drawableObject.TailCircle.ScreenSpaceDrawQuad.Centre
                    )
            );

            AddAssert(
                "end drag marker positioned correctly",
                () =>
                    Precision.AlmostEquals(
                        blueprint.TailOverlay.EndDragMarker!.ToScreenSpace(
                            blueprint.TailOverlay.EndDragMarker.OriginPosition
                        ),
                        drawableObject.TailCircle.ScreenSpaceDrawQuad.Centre,
                        2
                    )
        ...
        private void checkControlPointSelected(int index, bool selected) =>
            AddAssert(
                $"control point {index} {(selected ? "selected" : "not selected")}",
                () => blueprint.ControlPointVisualiser!.Pieces[index].IsSelected.Value == selected
            );
