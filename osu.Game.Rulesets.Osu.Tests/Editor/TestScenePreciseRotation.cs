...
namespace osu.Game.Rulesets.Osu.Tests.Editor
{
    public partial class TestScenePreciseRotation : TestSceneOsuEditor
    {
        ...
        public void TestHotkeyHandling()
        {
            ...
            AddStep(
                "select single circle",
                () =>
                    EditorBeatmap.SelectedHitObjects.Add(
                        EditorBeatmap.HitObjects.OfType<HitCircle>().First()
                    )
            );
            AddStep(
                "press rotate hotkey",
            ...
            AddAssert(
                "only playfield centre origin rotation available",
                () =>
                {
            ...
            AddUntilStep("popover present", getPopover, () => Is.Not.Null);
            AddAssert(
                "both origin rotation available",
                () =>
                {
                    var popover = getPopover();
                    var buttons = popover.ChildrenOfType<EditorRadioButton>();
            ...
            PreciseRotationPopover? getPopover() =>
                this.ChildrenOfType<PreciseRotationPopover>().SingleOrDefault();
        }
        ...
        public void TestRotateCorrectness()
        {
            ...
            AddStep(
                "rotate by 180deg",
                () => getPopover().ChildrenOfType<TextBox>().Single().Current.Value = "180"
            );
            AddAssert(
                "first object rotated 180deg around playfield centre",
                () => EditorBeatmap.HitObjects.OfType<HitCircle>().ElementAt(0).Position,
                () => Is.EqualTo(OsuPlayfield.BASE_SIZE - new Vector2(100))
            );
            AddAssert(
                "second object rotated 180deg around playfield centre",
            ...
            AddStep(
                ...
                () => getPopover().ChildrenOfType<EditorRadioButton>().ElementAt(2).TriggerClick()
            );
            AddAssert(
                "first object rotated 90deg around selection centre",
                () => EditorBeatmap.HitObjects.OfType<HitCircle>().ElementAt(0).Position,
                () => Is.EqualTo(new Vector2(200, 200))
            );
            AddAssert(
                "second object rotated 90deg around selection centre",
                () => EditorBeatmap.HitObjects.OfType<HitCircle>().ElementAt(1).Position,
                () => Is.EqualTo(new Vector2(100, 100))
            );
