    ...
    public partial class TestSceneSliderDrawing : TestSceneOsuEditor
    {
        ...
        public void TestTouchInputPlaceHitCircleDirectly()
        {
            AddStep(
                ...
                () =>
                    tap(
                        this.ChildrenOfType<EditorRadioButton>()
                            .Single(b => b.Button.Label == "HitCircle")
                    )
            );

            AddStep("tap to place circle", () => tap(this.ChildrenOfType<Playfield>().Single()));
            AddAssert(
                "circle placed correctly",
                () =>
                {
                    var circle = (HitCircle)
                        EditorBeatmap.HitObjects.Single(h =>
                            h.StartTime == EditorClock.CurrentTimeAccurate
                        );
                    Assert.Multiple(() =>
                    {
        ...
        public void TestTouchInputPlaceSliderDirectly()
        {
            AddStep(
                ...
                () =>
                    tap(
                        this.ChildrenOfType<EditorRadioButton>()
                            .Single(b => b.Button.Label == "Slider")
                    )
            );
            ...
            AddStep(
                ...
                () =>
                    InputManager.BeginTouch(
                        new Touch(
                            TouchSource.Touch1,
                            this.ChildrenOfType<Playfield>()
                                .Single()
                                .ToScreenSpace(new Vector2(50, 20))
                        )
                    )
            );
            AddStep(
                ...
                () =>
                    InputManager.MoveTouchTo(
                        new Touch(
                            ...
                            this.ChildrenOfType<Playfield>()
                                .Single()
                                .ToScreenSpace(new Vector2(200, 50))
                        )
                    )
            );
            AddAssert(
                "selection not initiated",
                () => this.ChildrenOfType<DragBox>().All(d => d.State == Visibility.Hidden)
            );
            AddAssert(
                "blueprint visible",
                () => this.ChildrenOfType<SliderPlacementBlueprint>().Single().Alpha > 0
            );
            AddStep(
                "end",
                () =>
            ...
            AddAssert(
                ...
                () =>
                {
                    ...
                    Assert.Multiple(() =>
                    {
                        ...
                        Assert.That(
                            slider.Path.ControlPoints[0].Position,
                            Is.EqualTo(Vector2.Zero)
                        );

                        // the final position may be slightly off from the mouse position when drawing, account for that.
                        Assert.That(
                            slider.Path.ControlPoints[1].Position.X,
                            Is.EqualTo(150).Within(5)
                        );
                        Assert.That(
                            slider.Path.ControlPoints[1].Position.Y,
                            Is.EqualTo(30).Within(5)
                        );
                    });
        ...
        public void TestTouchInputPlaceSliderAfterTouchingComposeArea()
        {
            ...
            AddStep("tap playfield", () => tap(this.ChildrenOfType<Playfield>().Single()));
            AddStep(
                "tap and hold another spot",
                () => hold(this.ChildrenOfType<Playfield>().Single(), new Vector2(50, 0))
            );
            AddUntilStep(
                "wait for slider placement",
                () =>
                    EditorBeatmap.HitObjects.SingleOrDefault(h =>
                        h.StartTime == EditorClock.CurrentTimeAccurate
                    ) is Slider
            );
            AddStep(
            ...
            AddAssert(
                "blueprint visible",
                () => this.ChildrenOfType<SliderPlacementBlueprint>().Single().IsPresent
            );
            AddStep(
                "end",
                () =>
        ...
        private void tap(Drawable drawable, Vector2 offset = default) =>
            tap(drawable.ToScreenSpace(drawable.LayoutRectangle.Centre + offset));
        ...
        private void hold(Drawable drawable, Vector2 offset = default) =>
            hold(drawable.ToScreenSpace(drawable.LayoutRectangle.Centre + offset));
