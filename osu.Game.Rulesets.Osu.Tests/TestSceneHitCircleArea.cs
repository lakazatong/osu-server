    ...
    public partial class TestSceneHitCircleArea : OsuManualInputManagerTestScene
    {
            ...
            Schedule(() =>
            {
                ...
                Child = new SkinProvidingContainer(new TrianglesSkin(null!))
                {
                    ...
                    Child = drawableHitCircle =
                        new DrawableHitCircle(hitCircle) { Size = new Vector2(100) },
                };
            });
        ...
        public void TestCircleHitCentre()
        {
            AddStep(
                ...
                () => InputManager.MoveMouseTo(hitAreaReceptor.ScreenSpaceDrawQuad.Centre)
            );
            scheduleHit();
        ...
        {
            AddStep(
                "move mouse to top left circle edge",
                () =>
                {
                    var drawQuad = hitAreaReceptor.ScreenSpaceDrawQuad;
                    ...
                    float correction =
                        relativeDistanceFromCentre * (float)Math.Sqrt(2) / 2 * (drawQuad.Width / 2);
                    var mousePosition = new Vector2(
                        drawQuad.Centre.X - correction,
                        drawQuad.Centre.Y - correction
                    );
            ...
            AddAssert(
                $"hit {(expectedAction == null ? "not " : string.Empty)}registered",
                () => hitAreaReceptor.HitAction == expectedAction
            );
        }
        ...
        {
            AddStep(
                "move mouse to top left corner of bounding box",
                () => InputManager.MoveMouseTo(hitAreaReceptor.ScreenSpaceDrawQuad.TopLeft)
            );
            scheduleHit();
        ...
        private void scheduleHit() =>
            AddStep(
                ...
                () =>
                {
                    double delay =
                        hitCircle.StartTime
                        - hitCircle.HitWindows.WindowFor(HitResult.Great)
                        - Time.Current;
                    Scheduler.AddDelayed(
                        () =>
                            hitAreaReceptor.OnPressed(
