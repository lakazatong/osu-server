...
namespace osu.Game.Rulesets.Osu.Tests
{
    public partial class TestSceneSpinnerRotation : TestSceneOsuPlayer
    {
        ...
        public void TestSpinnerRewindingRotation()
        {
            ...
            addSeekStep(5000);
            AddStep(
                "calculate rotation tolerance",
                () =>
                {
                    trackerRotationTolerance = Math.Abs(
                        drawableSpinner.RotationTracker.Rotation * 0.1f
                    );
                }
            );
            AddAssert(
                "is disc rotation not almost 0",
                () => drawableSpinner.RotationTracker.Rotation,
                () => Is.Not.EqualTo(0).Within(100)
            );
            AddAssert(
                "is disc rotation absolute not almost 0",
            ...
            addSeekStep(0);
            AddAssert(
                "is disc rotation almost 0",
                () => drawableSpinner.RotationTracker.Rotation,
                () => Is.EqualTo(0).Within(trackerRotationTolerance)
            );
            AddAssert(
                "is disc rotation absolute almost 0",
                () => drawableSpinner.Result.TotalRotation,
                () => Is.EqualTo(0).Within(100)
            );
        }
        ...
        public void TestSpinnerMiddleRewindingRotation()
        {
            ...
            double finalTrackerRotation = 0,
                trackerRotationTolerance = 0;

            addSeekStep(spinner_start_time + 5000);
            AddStep(
                "retrieve disc rotation",
                () =>
                {
            ...
            AddStep(
                "retrieve cumulative disc rotation",
                () => finalCumulativeTrackerRotation = drawableSpinner.Result.TotalRotation
            );

            addSeekStep(spinner_start_time + 2500);
            AddAssert(
                "disc rotation rewound",
                // we want to make sure that the rotation at time 2500 is in the same direction as at time 5000, but about half-way in.
                // due to the exponential damping applied we're allowing a larger margin of error of about 10%
                // (5% relative to the final rotation value, but we're half-way through the spin).
                () => drawableSpinner.RotationTracker.Rotation,
                () => Is.EqualTo(finalTrackerRotation / 2).Within(trackerRotationTolerance)
            );
            AddAssert(
                "is cumulative rotation rewound",
                // cumulative rotation is not damped, so we're treating it as the "ground truth" and allowing a comparatively smaller margin of error.
                () => drawableSpinner.Result.TotalRotation,
                () => Is.EqualTo(finalCumulativeTrackerRotation / 2).Within(100)
            );

            addSeekStep(spinner_start_time + 5000);
            AddAssert(
                "is disc rotation almost same",
                () => drawableSpinner.RotationTracker.Rotation,
                () => Is.EqualTo(finalTrackerRotation).Within(trackerRotationTolerance)
            );
            AddAssert(
                "is cumulative rotation almost same",
                () => drawableSpinner.Result.TotalRotation,
                () => Is.EqualTo(finalCumulativeTrackerRotation).Within(100)
            );
        }
        ...
        public void TestRotationDirection([Values(true, false)] bool clockwise)
        {
            ...
            AddAssert(
                "disc spin direction correct",
                () =>
                    clockwise
                        ? drawableSpinner.RotationTracker.Rotation > 0
                        : drawableSpinner.RotationTracker.Rotation < 0
            );
        }
        ...
        public void TestSpinnerNormalBonusRewinding()
        {
            ...
            AddAssert(
                ...
                () =>
                {
                    ...
                    return totalScore
                        == (int)(drawableSpinner.Result.TotalRotation / 360)
                            * scoreProcessor.GetBaseScoreForResult(
                                new SpinnerTick().Judgement.MaxResult
                            );
                }
            );
            ...
            AddAssert(
                "player score is 0",
                () => ((ScoreExposedPlayer)Player).ScoreProcessor.TotalScore.Value == 0
            );
        }
        ...
        public void TestSpinPerMinuteOnRewind()
        {
            ...
            AddAssert(
                "spm still valid",
                () => drawableSpinner.SpinsPerMinute.Value,
                () => Is.EqualTo(estimatedSpm).Within(1.0)
            );
        ...
        public void TestSpinUnaffectedByClockRate(double rate)
        {
            ...
            AddStep(
                "adjust track rate",
                () =>
                    ((MasterGameplayClockContainer)Player.GameplayClockContainer)
                        .UserPlaybackRate
                        .Value = rate
            );
            ...
            addSeekStep(1000);
            AddAssert(
                "progress almost same",
                () => drawableSpinner.Progress,
                () => Is.EqualTo(expectedProgress).Within(0.05)
            );
            AddAssert(
                "spm almost same",
                () => drawableSpinner.SpinsPerMinute.Value,
                () => Is.EqualTo(expectedSpm).Within(2.0)
            );
        }
